...
(function ($) {
    ...
    zuul = {
        ...
        schedule: function () {
            if (!zuul.enabled) {
                setTimeout(zuul.schedule, 5000000);
                return;
            }
        ...
        format: {
            ...
            change_status_icon: function(change) {
                var icon_class = 'text-success glyphicon-ok-sign';
                var icon_title = 'Succeeding';

                if (change.active != true) {
                    // Grey icon
                    icon_class = 'text-muted glyphicon-minus-sign';
            ...
            change_tree_branch: function(change, change_queue) {
                $tracks = $('<tr />')
                    .css('height', '100%')
                    .css('padding', '0')
                    .css('margin', '0');
                ...
                for (var i = 0; i < change_queue._tree_columns; i++) {
                    ...
                    if (i < change._tree.length && change._tree[i] !== null) {
                        a = '<span>|</span>';
                        $rail.append($(a));
                        $rail.css('background', 'green');
                    }

                    if (i == change._tree_index) {
                        $rail.append(zuul.format.change_status_icon(change));
            ...
            change_with_status_tree: function(change, change_queue) {
                var $tree_column = $('<div />')
                    ...
                    .css('float', 'none')
                    .append(
                        zuul.format.change_tree_branch(change, change_queue)
                    );
                var $change_column = $('<div />')
                    .addClass('col-xs-11')
                    .css('display', 'table-cell')
                    .css('float', 'none')
                    .css('padding', '0')
                ...
                var $change_box = $('<div />')
                    .addClass('row zuul-change-box')
                    .css('display', 'table')
                    .append($tree_column)
                    .append($change_column);
