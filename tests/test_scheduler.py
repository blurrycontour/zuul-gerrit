...
jobs:
  - name: job_name
    ...
    """.strip()
        ...
        jf = zuul.change_matcher
        expected = jf.MatchOnAny([
            jf.MatchOnAll([
                jf.ProjectMatcher('^project_name$'),
                jf.MatchAllFiles([jf.FileMatcher('^filename$')]),
            ]),
            jf.MatchOnAll([
                jf.ProjectMatcher('^project2_name$'),
                jf.MatchAllFiles([jf.FileMatcher('^filename2$')]),
            ]),
        ])
