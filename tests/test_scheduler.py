...
For CI problems and help debugging, contact ci@example.org"""
    ...
    def test_dependeny_graph_dispatch_jobs_once(self):
        ...
        self.waitUntilSettled()
        self.assertEqual(len(self.builds), 2)
        self.assertEqual({b.name for b in self.builds},
                         {'A-flat', 'A-deep'})
        ...
        self.assertEqual(len(self.builds), 4)
        self.assertEqual({b.name for b in self.builds},
                         {'B-flat', 'B-deep', 'C-flat', 'C-deep'})

        self.worker.release('B-.*')
        self.waitUntilSettled()
        self.assertEqual(len(self.builds), 4)
        self.assertEqual({b.name for b in self.builds},
                         {'C-flat', 'C-deep', 'D-flat', 'D-deep'})
        ...
        self.assertEqual(len(self.builds), 4)
        self.assertEqual({b.name for b in self.builds},
                         {'E-flat', 'E-deep', 'F-flat', 'F-deep'})

        self.worker.release('F-.*')
        self.waitUntilSettled()
        self.assertEqual(len(self.builds), 4)
        self.assertEqual({b.name for b in self.builds},
                         {'E-flat', 'E-deep', 'G-flat', 'G-deep'})
