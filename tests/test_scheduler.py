...
class TestScheduler(testtools.TestCase):
    ...
    def test_swift_instructions(self):
        ...
        self.assertEqual(1, len(self.builds[0].parameters['ZUUL_SWIFT']))
        self.assertEqual("logs",
                         self.builds[0].parameters['ZUUL_SWIFT'][0]['NAME'])
        self.assertEqual(
            "https://storage.example.org/V1/AUTH_account/merge_logs/1/1/1/"
            "gate/test-merge/",
            self.builds[0].parameters['ZUUL_SWIFT'][0]['URL'][:-32])
        self.assertEqual(5,
                         len(self.builds[0].parameters['ZUUL_SWIFT'][0]
                         ['HMAC_BODY'].split('\n')))
        self.assertIn('SIGNATURE', self.builds[0].parameters['ZUUL_SWIFT'][0])

        self.assertEqual(2, len(self.builds[1].parameters['ZUUL_SWIFT']))
        self.assertEqual("logs",
                         self.builds[1].parameters['ZUUL_SWIFT'][0]['NAME'])
        self.assertEqual(
            "https://storage.example.org/V1/AUTH_account/logs/1/1/1/"
            "gate/test-test/",
            self.builds[1].parameters['ZUUL_SWIFT'][0]['URL'][:-32])
        self.assertEqual(5,
                         len(self.builds[1].parameters['ZUUL_SWIFT'][0]
                         ['HMAC_BODY'].split('\n')))
        self.assertIn('SIGNATURE', self.builds[1].parameters['ZUUL_SWIFT'][0])
        self.assertEqual("mostly",
                         self.builds[1].parameters['ZUUL_SWIFT'][1]['NAME'])
        self.assertEqual(
            "https://storage.example.org/V1/AUTH_account/stash/1/1/1/"
            "gate/test-test/",
            self.builds[1].parameters['ZUUL_SWIFT'][1]['URL'][:-32])
        self.assertEqual(5,
                         len(self.builds[1].parameters['ZUUL_SWIFT'][1]
                         ['HMAC_BODY'].split('\n')))
        self.assertIn('SIGNATURE', self.builds[1].parameters['ZUUL_SWIFT'][0])
