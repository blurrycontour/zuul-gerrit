...
class TestScheduler(testtools.TestCase):
    ...
    def test_zuul_url_negative_return(self):
        "Negative test to ZUUL_URL"
        if self.sched.config.has_option('zuul', 'zuul_url'):
            self.sched.config.remove_option('zuul', 'zuul_url')

        self.assertFalse(self.sched.config.has_option('zuul', 'zuul_url'))
        ...
        for build in self.builds:
            self.assertFalse('ZUUL_URL' in build.parameters)
