...
jobs:
  - name: job_name
    ...
    def test_mutex(self):
        ...
        self.waitUntilSettled()
        self.assertEqual(len(self.builds), 3)
        self.assertEqual(self.builds[0].name, 'mutex-one')
...
For CI problems and help debugging, contact ci@example.org"""
    ...
    def test_jobs_launched_only_if_all_dependencies_are_successful_deep(self):
        self._test_jobs_launched_only_if_all_dependencies_are_successful(
            'deep')
    ...
    def _test_jobs_launched_only_if_all_dependencies_are_successful(
            self, type):
