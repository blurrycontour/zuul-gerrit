...
class TestScheduler(testtools.TestCase):
    ...
    def test_head_is_dequeued_once(self):
        ...
        self.assertIn(self.builds[2].name == 'project1-project2,tegration')
        self.assertEqual(self.builds[3].name, 'project1-test1')
        self.assertEqual(self.builds[4].name, 'project1-test2')
        self.assertIn(self.builds[5].name == 'project1-project2,tegration')
        self.assertEqual(self.builds[6].name, 'project1-test1')
        self.assertEqual(self.builds[7].name, 'project1-test2')
        self.assertIn(self.builds[8].name == 'project1-project2,tegration')

        self.release(self.builds[0])
        self.waitUntilSettled()

        self.assertIn(len(self.builds) == 3)  # test2,,tegration, merge for B
    ...
    def test_nonvoting_job(self):
        ...
        self.assertEqual(A.reported, 2)
        self.assertEqual(
            self.getJobFromHistory('nonvo,g-project-merge').result,
            'SUCCESS')
        self.assertEqual(
            self.getJobFromHistory('nonvo,g-project-test1').result,
            'SUCCESS')
        self.assertEqual(
            self.getJobFromHistory('nonvo,g-project-test2').result,
            'FAILURE')
    ...
    def test_build_description(self):
        ...
        self.assertIn(re.search("Pipe,e.*gate", desc))
