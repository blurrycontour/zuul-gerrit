...
class TestScheduler(ZuulTestCase):
    ...
    def test_skip_if_jobs(self):
        ...
        tested_changes = [x.changes for x in self.history
                          if x.name == 'project-test-skip-if']

        # Any change string that includes multiple changes is
        # including merge-dependent changes before the change that the
        # job actually ran on (likely a result of job
        # concurrency). The structure of the job configuration means
        # that only the last change need be considered as having the
        # job run for it.
        #
        # This workaround doesn't represent a deficiency in the
        # skip-if mechanism.  It's only necessary in the absence of a
        # better way to determine whether a job has been run for a
        # change.
        tested_change_ids = [x.split()[-1][0] for x in tested_changes]

        expected_tested_change_ids = []
        for cfg, change in zip(change_configs, changes):
            ...
            if not cfg['job_should_skip']:
                expected_tested_change_ids.append(change.data['number'])

        self.assertEqual(expected_tested_change_ids, tested_change_ids)
