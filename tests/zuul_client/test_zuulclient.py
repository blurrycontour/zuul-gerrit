...
class TestZuulClientAuthToken(BaseTestWeb):
    ...
    def test_token_generation_RS256(self):
        ...
        old_conf = io.StringIO()
        self.config.write(old_conf)
        self.config.add_section('auth zuul_operator_2')
        self.config.set(
            'auth zuul_operator_2', 'driver', 'RS256')
        self.config.set(
            'auth zuul_operator_2', 'allow_authz_override', 'true')
        self.config.set(
            'auth zuul_operator_2', 'realm', 'zuul.example.com')
        self.config.set(
            'auth zuul_operator_2', 'client_id', 'zuul.example.com')
        self.config.set(
            'auth zuul_operator_2', 'issuer_id', 'zuul_operator_2')
        self.config.set(
