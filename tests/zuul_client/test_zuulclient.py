...
from tests.unit.test_streaming import WSClient, TestStreamingBase
...
class TestZuulClientConsoleStream(TestStreamingBase, BaseTestWeb):
    tenant_config_file = 'config/streamer/main.yaml'
    log = logging.getLogger("zuul.test_zc_streaming")

    def _test_console_stream(self, use_uuid=False):
        self.executor_server.hold_jobs_in_build = True
        ...
        # run another websocket client to ensure all works
        client1 = WSClient(self.web.port, build.uuid)
        client1.event.wait()
    ...
    def test_console_stream_by_change(self):
        self._test_console_stream()

    def test_console_stream_by_uuid(self):
        self._test_console_stream(use_uuid=True)
