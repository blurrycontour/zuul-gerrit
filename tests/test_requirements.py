...
class TestRequirements(ZuulTestCase):
    ...
    def _test_require_any(self, project, job):
        "Test an approval matches all of the given requirements"
        self.config.set(
            'zuul', 'layout_config',
        ...
        # A +1 from jenkins should not allow it to be enqueued
        A.addApproval('VRFY', 1, username='jenkins')
        self.fake_gerrit.addEvent(comment)
        self.waitUntilSettled()
        self.assertEqual(len(self.history), 0)
        ...
        A.addApproval('CRVW', 2, username='core-reviewer')
        self.fake_gerrit.addEvent(comment)
        self.waitUntilSettled()
        self.assertEqual(len(self.history), 1)
        self.assertEqual(self.history[0].name, job)
        ...
        # A non-negative from a core member should not queue (without jenkins)
        B.addApproval('CRVW', 2, username='core-reviewer')
        self.fake_gerrit.addEvent(comment)
        self.waitUntilSettled()
        self.assertEqual(len(self.history), 1)

        # A +2 from jenkins should queue
        B.addApproval('VRFY', 2, username='jenkins')
        self.fake_gerrit.addEvent(comment)
        self.waitUntilSettled()
        self.assertEqual(len(self.history), 2)
    ...
    def _test_require_all(self, project, job):
        "Test all of the approvals match all of the requirements"
        self.config.set(
            'zuul', 'layout_config',
