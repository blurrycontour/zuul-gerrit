...
from zuul.driver.bitbucket.bitbucketconnection import BitbucketClient, BitbucketConnection, BitbucketConnectionError

class TestBitbucketClient(unittest.TestCase):

    def _broken_client(self):
        bc = BitbucketClient("https://bitbucket.smaato.net", 443)
    ...
    @unittest.skipUnless('BITBUCKET_USER' in environ, True) 
...
class TestBitbucketConnection(CommonConnectionTest):
    ...
    def test_getBitbucketClient(self):
        ...
        self.assertEqual(client.server, 'https://bitbucket.glumfungl.net')
