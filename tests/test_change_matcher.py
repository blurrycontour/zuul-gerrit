...
class TestAbstractMatcherCollection(BaseTestMatcher):
    ...
    def test_str(self):
        matcher = cm.MatchOnAll([cm.FileMatcher('foo')])
        self.assertEqual(str(matcher), '{MatchOnAll:{FileMatcher:foo}}')

    def test_repr(self):
        matcher = cm.MatchOnAll([])
        self.assertEqual(repr(matcher), '<MatchOnAll>')
...
class TestMatchOnAll(BaseTestMatcher):
    ...
    def test_matches_returns_true(self):
        matcher = cm.MatchOnAll([cm.ProjectMatcher(self.project)])
        self.assertTrue(matcher.matches(self.change))

    def test_matches_returns_false_for_missing_matcher(self):
        matcher = cm.MatchOnAll([cm.ProjectMatcher('not_project')])
        self.assertFalse(matcher.matches(self.change))
...
class TestMatchOnAny(BaseTestMatcher):
    ...
    def test_matches_returns_true(self):
        matcher = cm.MatchOnAny([cm.ProjectMatcher(self.project)])
        self.assertTrue(matcher.matches(self.change))

    def test_matches_returns_false(self):
        matcher = cm.MatchOnAny([cm.ProjectMatcher('not_project')])
        self.assertFalse(matcher.matches(self.change))
