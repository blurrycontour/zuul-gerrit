...
class TestZuulJSON(AnsibleZuulTestCase):
    ...
    def test_json_task_action(self):
        ...
        with self.jobLog(job):
            ...
            self.assertEquals('debug', tasks[0]['action'])
            self.assertEquals('debug', tasks[1]['action'])
            self.assertEquals('debug', tasks[2]['action'])
            self.assertEquals('copy', tasks[3]['action'])
            self.assertEquals('find', tasks[4]['action'])
            self.assertEquals('stat', tasks[5]['action'])
            self.assertEquals('debug', tasks[6]['action'])
