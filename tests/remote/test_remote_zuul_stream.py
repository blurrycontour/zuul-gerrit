...
class TestZuulStream(AnsibleZuulTestCase):
    ...
    def assertLogLine(self, line, log):
        pattern = '^\d\d\d\d-\d\d-\d\d \d\d:\d\d\:\d\d\.\d\d\d\d\d\d \| %s$' % line
    ...
    def test_command(self):
        ...
        with self.jobLog(job):
            ...
            self.assertLogLine('RUN START: \[untrusted : review.example.com/org/project/'
            ...
            self.assertLogLine('controller \| ok: Runtime: \d:\d\d:\d\d\.\d\d\d\d\d\d', text)
            self.assertLogLine('TASK \[Show contents of second file\]', text)
            self.assertLogLine('controller \| command test one', text)
            self.assertLogLine('controller \| ok: Runtime: \d:\d\d:\d\d\.\d\d\d\d\d\d', text)
            self.assertLogLine('PLAY RECAP', text)
            self.assertLogLine('controller \| ok: 5 changed: 2 unreachable: 0 failed: 0', text)
            self.assertLogLine('RUN END RESULT_NORMAL: \[untrusted : review.example.com/'
                               'org/project/playbooks/command.yaml@master]x', text)
    ...
    def test_module_failure(self):
        ...
        with self.jobLog(job):
            ...
            self.assertLogLine('controller \| MODULE FAILURE: This module is broken', text)
