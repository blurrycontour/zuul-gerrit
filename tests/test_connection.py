...
def _get_reporter_from_connection_name(action_reporters, connection_name):
    ...
    # name
    for ac in action_reporters:
        if hasattr(ac.reporter, 'connection'):
            if ac.reporter.connection.connection_name == connection_name:
                return ac.reporter
...
class TestMultipleGerrits(ZuulTestCase):
    ...
    def test_multiple_project_separate_gerrits(self):
        ...
        A = self.fake_review_gerrit.addFakeChange('org/project1', 'master', 'A')
        self.fake_review_gerrit.addEvent(A.getPatchsetCreatedEvent(1))

        self.waitUntilSettled()

        self.assertEqual(1, len(self.builds))
        self.assertEqual('project1-another-gerrit', self.builds[0].name)
