...
class TestConnections(ZuulDBTestCase):
    ...
    def _test_sql_tables_created(self, build_table=None, metadata_table=None):
        ...
        self.assertEqual([{
            'column_names': ['uuid', 'job_name'],
            'name': 'zuul_build_uuid_job_name',
            'duplicates_index': 'zuul_build_uuid_job_name'}],
            insp.get_unique_constraints(build_table))

        self.assertEqual(4, len(insp.get_columns(metadata_table)))
        self.assertEqual([{
            'column_names': ['build_uuid', 'build_job_name', 'key'],
            'name': 'zuul_build_metadata_build_uuid_build_job_name_key',
            'duplicates_index':
                'zuul_build_metadata_build_uuid_build_job_name_key'}],
    ...
    def _test_sql_results(self):
        ...
        result = conn.execute(
            sqlalchemy.sql.select([reporter.connection.zuul_build]))
        ...
        metadata = conn.execute(
            sqlalchemy.sql.select([reporter.connection.zuul_build_metadata]).
            where(sqlalchemy.sql.and_(
                reporter.connection.zuul_build_metadata.c.build_uuid ==
                reporter.connection.zuul_build.c.uuid,
                reporter.connection.zuul_build_metadata.c.build_job_name ==
                reporter.connection.zuul_build.c.job_name,
                reporter.connection.zuul_build_metadata.c.build_uuid ==
                row['uuid']))
        ).fetchall()
        metadata_dict = {}
    ...
    def notest_multiple_sql_connections(self):
        ...
        result = conn.execute(sqlalchemy.sql.select(
            [reporter1.connection.zuul_build]))
        ...
        result = conn.execute(sqlalchemy.sql.select(
            [reporter2.connection.zuul_build]))
        rows = result.fetchall()
