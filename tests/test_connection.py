...
class TestConnections(ZuulDBTestCase):
    ...
    def _test_sql_results(self):
        ...
        buildset0_builds = conn.execute(
            ...
            where(
                reporter.connection.zuul_build_table.c.buildset_id == buildset0['id']
            )
        ).fetchall()
        ...
        buildset1_builds = conn.execute(
            ...
            where(
                reporter.connection.zuul_buildset.c.buildset_id ==
                buildset1['id']
            )
        ).fetchall()
    ...
    def test_multiple_sql_connections(self):
        ...
        # Should have been 1 buildset reported to the resultsdb
        self.assertEqual(1, len(buildsets_resultsdb))

        self.assertEqual('gate', buildsets_resultsdb[0]['pipeline'])
        ...
        self.assertEqual('gate', buildsets_resultsdb_failures[0]['pipeline'])
        self.assertEqual(
