...
class TestCloner(ZuulTestCase):
    ...
    def test_post(self):
        ...
        for number, build in enumerate(builds):
            ...
            for project in projects:
                self.log.debug("Project %s HEAD: %s" % (project, work[project].commit('HEAD')))
                self.log.debug("Project %s master: %s" % (project, work[project].commit('master')))
                self.log.debug("Project %s stable/kilo: %s" % (project, work[project].commit('stable/kilo')))
                self.assertEquals(state[project],
                                  str(work[project].commit('HEAD')),
                                  'Project %s commit for build %s should '
                                  'be correct' % (project, number))
    ...
    def test_release(self):
        ...
        for number, build in enumerate(builds):
            ...
            for project in projects:
                self.log.debug("Project %s HEAD: %s" % (project, work[project].commit('HEAD')))
                self.log.debug("Project %s master: %s" % (project, work[project].commit('master')))
                if project is 'org/project2':
                    self.log.debug("Project %s 0.0.0: %s" % (project, work[project].tag('refs/tags/0.0.0').commit))
                self.assertEquals(state[project],
                                  str(work[project].commit('HEAD')),
                                  'Project %s commit for build %s should '
                                  'be correct' % (project, number))
