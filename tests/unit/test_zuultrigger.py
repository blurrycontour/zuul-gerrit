...
class TestZuulTriggerParentChangeEnqueuedGithub(ZuulGithubAppTestCase):
    ...
    def test_zuul_trigger_parent_change_enqueued(self):
        ...
        # After starting recording installation 2 should be untouched
        inst_two_clients = [x for x in self.fake_github.recorded_clients
                            if x._inst_id == 2]
        self.assertEqual(len(inst_two_clients), 0)
