...
class TestGerritCircularDependencies(ZuulTestCase):
    ...
    def test_build_set_info_in_zuul_var(self):
        A = self.fake_gerrit.addFakeChange("org/project1", "master", "A")
        B = self.fake_gerrit.addFakeChange("org/project1", "master", "B")
        C = self.fake_gerrit.addFakeChange("org/project1", "master", "C")
        ...
        time_before_check_AB = time.time()
        self.fake_gerrit.addEvent(A.getPatchsetCreatedEvent(1))
        self.waitUntilSettled()
        time_after_check_AB = time.time()
        var_zuul_items = self.builds[0].parameters["zuul"]["items"]
        # queue_item_uuid and enqueue_time should be the same for A,B.
        self.assertEqual(len(var_zuul_items), 2)
        self.assertEqual(
            var_zuul_items[0]["queue_item_uuid"],
            var_zuul_items[1]["queue_item_uuid"],
        )
        queue_item_uuid_check_AB = var_zuul_items[0]["queue_item_uuid"]
        self.assertEqual(
            var_zuul_items[0]["enqueue_time"],
            var_zuul_items[1]["enqueue_time"],
        )
        self.assertTrue(
            time_before_check_AB <= var_zuul_items[0]["enqueue_time"],
        )
        self.assertTrue(
            var_zuul_items[0]["enqueue_time"] <= time_after_check_AB,
        )
        self.executor_server.release()
        self.waitUntilSettled()
        self.assertEqual(len(A.patchsets[-1]["approvals"]), 1)
        ...
        # queue_item_uuid and enqueue_time should different for C.
        time_before_check_C = time.time()
        self.fake_gerrit.addEvent(C.getPatchsetCreatedEvent(1))
        self.waitUntilSettled()
        time_after_check_C = time.time()
        var_zuul_items = self.builds[0].parameters["zuul"]["items"]
        self.assertEqual(len(var_zuul_items), 1)
        self.assertNotEqual(
            var_zuul_items[0]["queue_item_uuid"],
            queue_item_uuid_check_AB,
        )
        self.assertTrue(
            time_before_check_C < var_zuul_items[0]["enqueue_time"],
        )
        self.assertTrue(
            var_zuul_items[0]["enqueue_time"] < time_after_check_C,
        )
        self.executor_server.release()
        self.waitUntilSettled()
        self.assertEqual(len(C.patchsets[-1]["approvals"]), 1)
        ...
        self.fake_gerrit.addEvent(C.addApproval("Approved", 1))
        self.waitUntilSettled()
        time_after_gate = time.time()
        # The first few jobs uses A and B, the last ones use C as well.
        var_zuul_items_AB = self.builds[0].parameters["zuul"]["items"]
        var_zuul_items_ABC = self.builds[-1].parameters["zuul"]["items"]
        self.assertEqual(len(var_zuul_items_AB), 2)
        self.assertEqual(len(var_zuul_items_ABC), 3)
        # The first queue item, containing A and B, should have the same
        # var_zuul_items as the second queue item, apart from C.
        self.assertEqual(var_zuul_items_AB[0], var_zuul_items_ABC[0])
        self.assertEqual(var_zuul_items_AB[1], var_zuul_items_ABC[1])
        # A and B should have the same queue_item_uuid and enqueue_time.
        self.assertEqual(
            var_zuul_items_AB[0]["queue_item_uuid"],
            var_zuul_items_AB[1]["queue_item_uuid"],
        )
        self.assertEqual(
            var_zuul_items_AB[0]["enqueue_time"],
            var_zuul_items_AB[1]["enqueue_time"],
        )
        # C should be different.
        self.assertNotEqual(
            var_zuul_items_AB[0]["queue_item_uuid"],
            var_zuul_items_ABC[2]["queue_item_uuid"],
        )
        time_gate_AB = var_zuul_items_AB[0]["enqueue_time"]
        time_gate_C = var_zuul_items_ABC[2]["enqueue_time"]
        self.assertTrue(time_before_gate < time_gate_AB)
        self.assertTrue(time_gate_AB < time_gate_C)
        self.assertTrue(time_gate_C < time_after_gate)
