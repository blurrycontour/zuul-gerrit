...
class TestReporting(ZuulTestCase):
    ...
    @simple_layout("layouts/abort-reporting.yaml")
    def test_abort_reporting(self):
        """Check that explicitly dequeued items are reported as aborted"""
    ...
    @simple_layout("layouts/abort-reporting.yaml")
    def test_abort_reporting_gate_reset(self):
        """Check that a gate reset is not reported as aborted"""
        ...
        # Release project-test1 for A which will fail. This will abort both
        # running B jobs and re-execute project-merge for B.
        self.builds[0].release()
        self.waitUntilSettled()
        self.orderedRelease()

        # None of the items should be reported as abort, only success/failure
    ...
    @simple_layout("layouts/abort-reporting.yaml")
    def test_abort_reporting_supercedes(self):
        """Test that a superceeded change is reported as aborted"""
    ...
    @simple_layout("layouts/abort-reporting.yaml")
    def test_abort_reporting_new_patchset(self):
        "Test that change superceeded by a new patchset is reported as aborted"
