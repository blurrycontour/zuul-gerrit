...
class TestGitLabAPIClient(BaseTestCase):
    ...
    def test_approve_updated_merge_request(self):
        values = (
            {'message': 'SHA does not match HEAD of source branch: <updated_sha>'},
            409,
            'https://gitlab.local/api/v4/projects/test/merge_requests' +
                '/1/approve',
            'POST'
        )

        with patch.object(GitlabAPIClient, 'post', return_value=values) as post:
