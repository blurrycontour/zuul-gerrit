    ...
    def test_sql_results(self):
        ...
        def check_results():
            ...
            with self.scheds.first.connections.getSqlConnection().\
                ...
                self.assertEqual(len(paused_build_events), 1)
                self.assertEqual(
                    paused_build_events[0]["event_type"], "paused")
                self.assertIsNotNone(paused_build_events[0]["event_time"])
                self.assertIsNone(paused_build_events[0]["event_description"])
