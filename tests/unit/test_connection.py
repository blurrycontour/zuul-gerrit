...
class TestSQLConnection(ZuulDBTestCase):
    ...
    def test_sql_results_reset_builds(self):
        "Test that gate resets are entered into an sql table correctly"
        ...
        def check_results(connection_name):
            ...
            with self.connections.connections[
                ...
                buildsets = result.fetchall()
                # We have two FakeChanges, so we should get two buildset
                # entries
            ...
            self.assertTrue(buildset0_builds[1]['final'])
