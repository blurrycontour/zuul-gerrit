...
class TestSQLConnection(ZuulDBTestCase):
    ...
    def test_multiple_sql_connections(self):
        ...
        def check_results(connection_name_1, connection_name_2):
            ...
            self.assertEqual(
                'check', buildsets_resultsdb_mysql_failures[0]['pipeline'])
            self.assertEqual('org/project',
                             buildsets_resultsdb_mysql_failures[0]['project'])
            self.assertEqual(2,
                             buildsets_resultsdb_mysql_failures[0]['change'])
            self.assertEqual(
                '1', buildsets_resultsdb_mysql_failures[0]['patchset'])
            self.assertEqual(
                'FAILURE', buildsets_resultsdb_mysql_failures[0]['result'])
            self.assertEqual('Build failed.',
                             buildsets_resultsdb_mysql_failures[0]['message'])
