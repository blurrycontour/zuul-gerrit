...
class TestElasticsearchConnection(AnsibleZuulTestCase):
    ...
    def test_elastic_reporter_with_vars_indexation(self):
        ...
        # Check variables passed via zuul_return are there
        self.assertEqual(build_doc['job_param.ir_var3'], 'tonkatsu')
        self.assertEqual(build_doc['job_param.ir_foo'], 'bar')
        # Did not match the regexp
        self.assertNotIn(build_doc, "job_param.var3")
        self.assertNotIn(build_doc, "job_param.not")
