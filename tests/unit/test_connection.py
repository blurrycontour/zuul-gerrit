...
class TestSQLConnection(ZuulDBTestCase):
    ...
    def _sql_indexes_created(self, connection_name):
        ...
        self.assertEqual(3, len(insp.get_indexes(buildset_table)))
        self.assertEqual(1, len(insp.get_indexes(build_table)))

        # check if all indexes are prefixed
        if table_prefix:
            indexes = insp.get_indexes(buildset_table) + \
                      insp.get_indexes(build_table)
            for index in indexes:
                self.assertTrue(index['name'].startswith(table_prefix))

    def _sql_indexes_created(self):
