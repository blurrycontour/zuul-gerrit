...
class TestMergerRepo(ZuulTestCase):
    ...
    def test_files_changes(self):
        ...
        self.create_commit('org/project1', head='feature', files={"file1.txt": "file1\n"})
        self.create_commit('org/project1', head='feature', files={"file2.txt": "file2\n"})

        work_repo = Repo(parent_path, self.workspace_root,
                         'none@example.org', 'User Name', '0', '0')
        changed_files = work_repo.getFilesChanges('feature', 'master')
        self.assertEqual(['README', 'file1.txt', 'file2.txt'], sorted(changed_files))
