...
class TestMerger(ZuulTestCase):
    ...
    def test_lost_merge_request_result(self):
        ...
        # client.  This is normally called from apsched from the
        # scheduler.  To exercise it, we need to produce a fake lost
        # merge result and then invoke it ourselves.
        ...
        a = merger_api.get(f"{merger_api.MERGE_REQUEST_ROOT}/A")
        # Merger reports result
        result_data = {'result': 'ok'}
        merger_api.reportResult(a, result_data)

        # Merger removes merge request on completion
        merger_api.remove(a)
        ...
        lost_merge_results = list(merger_api.lostMergeResults())
