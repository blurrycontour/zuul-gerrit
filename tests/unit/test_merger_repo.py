...
class TestMergerRepo(ZuulTestCase):
    ...
    def test_files_changes_criss_cross_merges(self):
        ...
        work_repo = Repo(parent_path, self.workspace_root,
                         'none@example.org', 'User Name', '0', '0')
        # ancestor
        changed_files = work_repo.getFilesChanges(commitAhexsha, commitFhexsha)
        self.assertEqual(sorted(['B', 'D', 'F']), sorted(changed_files))
        # ancestor
        changed_files = work_repo.getFilesChanges(commitBhexsha, commitFhexsha)
        ...
        # ancestor
        changed_files = work_repo.getFilesChanges(commitDhexsha, commitFhexsha)
        ...
        # G includes file 'A', check that it is present  in E/G files changes
        changed_files = work_repo.getFilesChanges(commitEhexsha, commitGhexsha)
        self.assertEqual(sorted(['A', 'D', 'F']), sorted(changed_files))
        ...
        # some previous tests but with using oldrev/newrev
        changed_files = work_repo.getFilesChanges(None, None,
                                                  commitEhexsha, commitFhexsha)
        self.assertEqual(sorted(['D', 'F']), sorted(changed_files))
        changed_files = work_repo.getFilesChanges(None, None,
                                                  commitEhexsha, commitGhexsha)
        self.assertEqual(sorted(['A', 'D', 'F']), sorted(changed_files))
        changed_files = work_repo.getFilesChanges(None, None,
                                                  commitChexsha, commitDhexsha)
        self.assertEqual(sorted(['D']),
                         sorted(changed_files))
        changed_files = work_repo.getFilesChanges(None, None,
                                                  commitDhexsha, commitChexsha)
        self.assertEqual(sorted(['C']), sorted(changed_files))
        changed_files = work_repo.getFilesChanges(None, None,
                                                  commitChexsha, commitDhexsha)
        self.assertEqual(sorted(['D']), sorted(changed_files))
        ...
        # when one ref is None, all tree files are returned
        changed_files = work_repo.getFilesChanges(commitFhexsha, None)
        self.assertEqual(sorted(['A', 'B', 'D', 'F', 'README']),
                         sorted(changed_files))
        # when one ref is None, all tree files are returned
        changed_files = work_repo.getFilesChanges(None, commitFhexsha)
        self.assertEqual(sorted(['A', 'B', 'D', 'F', 'README']),
                         sorted(changed_files))

        changed_files = work_repo.getFilesChanges(
            "branch1", "refs/heads/branch1",
            commitAhexsha, commitEhexsha)
        self.assertEqual(sorted(['B', 'C', 'E']), sorted(changed_files))
