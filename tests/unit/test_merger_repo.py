...
class TestMergerRepo(ZuulTestCase):
    ...
    def test_set_refs(self):
        ...
        work_repo.setRefs({'refs/heads/master':
                           repo.heads.foobar.commit.hexsha})

        self.assertEqual(work_repo.getBranchHead('master'),
                         work_repo.getBranchHead('foobar'))
        # Ensure that we keep the origin refs intact
        for ref in ['master', 'foobar']:
            self.assertIn(ref, repo.remotes.origin.refs)
