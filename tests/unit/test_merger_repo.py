...
class TestMerger(ZuulTestCase):
    ...
    def test_merge_temp_refs(self):
        ...
        if parent_repo.git.version_info[:2] < (2, 13):
            ...
            # For some reason we also have to cleanup the reflog for this test
            for root, dirs, files in os.walk(
                    os.path.join(parent_path, '.git/logs/refs'), topdown=False):
                if (
                    not os.listdir(root)
                    and not root.endswith('.git/logs/refs')
                ):
                    self.log.debug("Cleaning empty ref dir %s", root)
                    os.rmdir(root)

        cache_repo.reset()
        self.assertNotIn("foo/bar", repo.refs)

        # Create another branch foo that can't be created if the 'foo/bar'
