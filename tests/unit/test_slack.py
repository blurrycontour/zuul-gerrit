...
class TestSlack(TestCase):
    ...
    def test_slack_reporter(self, _slack_mock):
        ...
        report = dedent('fake start message\n'''
                        ...
                        'fake footer message')
        calls = [
            call("chat.postMessage", channel="#all", text="Cfakec"),
            call("chat.postMessage", channel="#all", text=report, ts="0.0"),
            call("chat.postMessage", channel="#foo", text="Cfakec"),
            call("chat.postMessage", channel="#foo", text=report, ts="0.0"),
        ]
