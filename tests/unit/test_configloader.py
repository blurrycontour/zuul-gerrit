...
# class TestMultiSchedulerSetup(TenantParserTestCase):
#     tenant_config_file = 'config/tenant-parser/extra.yaml'
#
#     def setUp(self):
#         super(TestMultiSchedulerSetup, self).setUp()
#         self.sched2_wrap = ZuulSchedulerWrapper(self.log, self.config,
#                                                 self.zk_config,
#                                                 self.connections)
#         self.sched_wraps.append(self.sched2_wrap)
#
#     def test_extra_reconfigure_2(self):
#
#
#         in_repo_conf = textwrap.dedent(
#             """
...
#             dict(name='common-config-job', result='SUCCESS', changes='2,1'),
#             dict(name='project2-job', result='SUCCESS', changes='2,1'),
#             dict(name='project2-extra-file2', result='SUCCESS',
#                  changes='2,1'),
#         ], ordered=False)
...
class TestMultiSchedulerSetup(TenantParserTestCase):
    tenant_config_file = 'config/tenant-parser/extra.yaml'
    # config_file_second_scheduler = 'zuul-second-scheduler.conf'

    def setUp(self):
        super().setUp()
        # self.sched_second = self.sched_factory.create(
        #     self.log, self.test_root, self.config, self.zk_config,
        #     self.connections)

    def test_extra_reconfigure_1(self):
        in_repo_conf = textwrap.dedent(
            """
            - job:
                name: project2-extra-file2
...
#             FIXTURE_DIR, self.config_file_second_scheduler))
#         self.shed2 = self.sched_factory.create(
