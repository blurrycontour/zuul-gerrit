...
class TestAuthorizationRuleParser(ZuulTestCase):
    ...
    def test_rules_are_loaded(self):
        rules = self.scheds.first.abide.admin_rules
        self.assertTrue('auth-rule-one' in rules, self.scheds.first.abide)
        self.assertTrue('auth-rule-two' in rules, self.scheds.first.abide)
