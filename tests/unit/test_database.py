...
class TestMysqlDatabase(DBBaseTestCase):
    ...
    def test_sanitize_subtring_query(self):
        ...
        self.assertEqual(None,
                         session._sanitizeSubstringQuery(None, escape_char))
        self.assertEqual("job-name", session._sanitizeSubstringQuery(
            "job-name", escape_char))
        self.assertEqual("$%job-$%name$%", session._sanitizeSubstringQuery(
            "%job-%name%", escape_char))
        self.assertEqual("job$_-$%name$%", session._sanitizeSubstringQuery(
            "job_-%name%", escape_char))
        self.assertEqual("job$_-$%name$%", session._sanitizeSubstringQuery(
            "job_-%name%", escape_char))
