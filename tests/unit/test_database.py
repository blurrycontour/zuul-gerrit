...
class TestMysqlDatabase(DBBaseTestCase):
    ...
    def test_sanitize_subtring_query(self):
        ...
        self.assertEqual(True, session._sanitizeSubstringQuery(True))
        self.assertEqual(
            ["foo", "bar"], session._sanitizeSubstringQuery(["foo", "bar"]))
        self.assertEqual(
    ...
    def test_fuzzy_filter_op(self):
        ...
        self.assertEqual(type(col == None), type(filter))
        self.assertEqual(sqlalchemy.sql.elements.Null, type(filter.right))

        filter = session._getFuzzyFilterOp(col, True)
        self.assertEqual(type(col == True), type(filter))
        self.assertEqual(sqlalchemy.sql.elements.True_, type(filter.right))

        filter = session._getFuzzyFilterOp(col, ["foo", "bar"])
        self.assertEqual(type(col == True), type(filter))
        self.assertEqual(["foo", "bar"], filter.right.value)
