    ...
    def setUp(self):
        ...
        self.web = self.useFixture(
            ZuulWebFixture(
                ...
                JWTalgorithm=self.zuul_ini_config.get('web', 'JWTalgorithm')))
...
class TestTenantScopedWebApi(BaseTestWeb):
    ...
    def test_invalid_claim_format(self):
        for zuul_tenants in [['tenant-one', 'tenant-two'],
                             {'tenant-one': 'org/project', }, ]:
            authz = {'iss': 'me',
                     'zuul.tenants': zuul_tenants,
                     'exp': time.time() + 3600}
    ...
    def test_valid_JWT_bad_tenants(self):
        authz = {'iss': 'me',
                 'zuul.tenants': {'tenant-six': ['org/pj1', 'org/pj2'],
                                  'tenant-ten': '*'},
                 'exp': time.time() + 3600}
        token = jwt.encode(authz, key='NoDanaOnlyZuul',
                           algorithm='HS256').decode('utf-8')
    ...
    def test_valid_JWT_bad_projects(self):
        authz = {'iss': 'me',
                 'zuul.tenants': {'tenant-one': ['org/project5',
                                                 'org/project10'], },
                 'exp': time.time() + 3600}
        token = jwt.encode(authz, key='NoDanaOnlyZuul',
                           algorithm='HS256').decode('utf-8')
    ...
    def test_enqueue(self):
        ...
        authz = {'iss': 'me',
                 'zuul.tenants': {'tenant-one': ['org/project', ], },
                 'exp': time.time() + 3600}
        token = jwt.encode(authz, key='NoDanaOnlyZuul',
                           algorithm='HS256').decode('utf-8')
