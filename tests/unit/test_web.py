...
class BaseTestWeb(ZuulTestCase):
    ...
    def setup_config(self):
        ...
        if getattr(self, 'authorization_rules_config_file') is not None:
            ...
            self.config.set('web', 'authorizations_config',
                            os.path.join(FIXTURE_DIR, value))
...
class TestTenantScopedWebApi(BaseTestWeb):
    ...
    def test_valid_JWT_bad_tenants(self):
        authz = {'iss': 'zuul_operator',
                 'sub': 'testuser',
                 'aud': 'zuul.example.com',
                 'zuul.actions': {'autohold': {'tenant-six': ['org/pj1',
                                                              'org/pj2'],
                                               'tenant-ten': '*'},
                                  'enqueue': {'tenant-six': ['org/pj1',
                                                             'org/pj2'],
                                              'tenant-ten': '*'},
                                  'dequeue': {'tenant-six': ['org/pj1',
                                                             'org/pj2'],
                                              'tenant-ten': '*'}},
                 'exp': time.time() + 3600}
        token = jwt.encode(authz, key='NoDanaOnlyZuul',
    ...
    def test_dequeue(self):
        ...
        zuul_actions = {'dequeue': {'tenant-one': '*'}, }
        authz = {'iss': 'zuul_operator',
                 'aud': 'zuul.example.com',
                 'sub': 'testuser',
                 'zuul.actions': zuul_actions,
    ...
    def test_JWT_as_query_arg(self):
        ...
        args = {"reason": "some reason",
                ...
                'ref': None,
                'node_hold_expiration': None}
        zuul_actions = {'autohold': {'tenant-one': ['org/project', ]}, }
        authz = {'iss': 'zuul_operator',
                 'aud': 'zuul.example.com',
                 'sub': 'testuser',
                 'zuul.actions': zuul_actions,
                 'exp': time.time() + 3600}
        tkn = jwt.encode(authz, key='NoDanaOnlyZuul',
                         algorithm='HS256').decode('utf-8')
        req = self.post_url(
            'api/tenant/tenant-one/project/org/project/autohold?jwt=%s' % tkn,
            json=args)
        self.assertEqual(200, req.status_code, req.text)
        data = req.json()
        self.assertEqual(True, data)
...
class TestTenantScopedWebApiWithAuthRulesOverrideAllowed(
    ...
    def test_authorization_registry_is_loaded(self):
        self.assertEqual(4, len(self.web.authorizations.ruleset),
    ...
    def test_user_actions_action_override(self):
        ...
        # The JSON returned is the same as the client's output
        data = req.json()
        self.assertTrue('zuul.actions' in data, data)
...
class TestTenantScopedWebApiWithAuthRules(BaseTestWeb):
    ...
    # TODO this should probably be tested with every action verb
    def test_tenant_level_rule(self):
        ...
        for p in ['org/project', 'org/project1', 'org/project2']:
            i += 1
            f_ch = self.fake_gerrit.addFakeChange(p, 'master',
                                                  '%s %i' % (p, i))
    ...
    # TODO this should probably be tested with every action verb
    def test_project_level_rule(self):
        """Test that authz rules defined at project level are checked"""
        i = 0
        path = "api/tenant/%(tenant)s/project/%(project)s/enqueue"
        for p in ['org/project', 'org/project1', 'org/project2']:
            i += 1
    ...
    def test_user_actions(self):
        ...
        for test_user in users:
            ...
            self.assertEqual(200, req.status_code, req.text)
            data = req.json()
            self.assertTrue('zuul.actions' in data,
                            "%s got %s" % (authz['sub'], data))
            self.assertEqual(test_user['zuul.actions'], data['zuul.actions'],
                             "%s got %s" % (authz['sub'], data))
