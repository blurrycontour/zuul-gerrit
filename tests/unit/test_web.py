...
class TestWeb(BaseTestWeb):
    ...
    def test_web_404_on_unknown_tenant(self):
        ...
        self.assertEqual(404, resp.status_code)
        error = resp.json()
        self.assertEqual(404, error['status'], error)
...
class TestTenantScopedWebApiWithAuthRulesMultiTenant(BaseTestWeb):
    config_file = 'zuul-admin-web-no-override.conf'
    tenant_config_file = 'config/authorization/multi-tenant/main.yaml'
    ...
    def test_user_actions(self):
        ...
        for test_user in users:
            ...
            req = self.get_url('/api/tenant/not-a-tenant/authorizations',
                               headers={'Authorization': 'Bearer %s' % token})
