...
class TestRoles(BaseTestWeb):
    ...
    def test_roles_list(self):
        ...
        for role in roles:
            self.assertIn(role["name"], ["ensure-python", "foo", "undocumented-role"])

        ensure_python = [r for r in roles if r["name"] == "ensure-python"][0]
        foo = [r for r in roles if r["name"] == "foo"][0]
        undocumented_role = [r for r in roles if r["name"] == "undocumented-role"][0]

        self.assertEquals(
            ["project", "changelog", "readme", "name"], list(ensure_python.keys())
        )
        self.assertEquals(
