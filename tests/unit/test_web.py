...
class TestWeb(BaseTestWeb):
        ...
        job_params = {
            ...
            'zuul': {
                '_inheritance_path': [
                    ...
                    'common-config/zuul.yaml@master#51>',
                    '<Job project-test1 branches: None source: '
                    'common-config/zuul.yaml@master#64>',
                    '<Job project-test1 branches: None source: '
                    'common-config/zuul.yaml@master#136>',
                    '<Job project-test1 branches: None source: '
                    'common-config/zuul.yaml@master#51>'],
    ...
    def test_freeze_job_with_depends_on(self):
        ...
        data = self.put_url(
            "api/tenant/tenant-one/pipeline/check"
            "/project/org/project1/branch/master/freeze-job/"
            "project-test1", json=["http://review.example.com/1"]).json()
        self.assertEquals([{
            'branch': 'master',
            'change': '1',
...
class TestRunner(BaseTestWeb):
    ...
    def test_runner_execute(self):
        """Test runner can execute from the freeze_job api"""
        runner = self.setup_runner({
            "nodes": [{
                "label": "label1",
                "hostname": "localhost",
