...
class TestInventory(ZuulTestCase):
    ...
    def _get_setup_inventory(self, name):
        build = self.getBuildByName(name)
        setup_inv_path = os.path.join(build.jobdir.root, 'ansible', 'setup-inventory.yaml')
    ...
    def test_setup_inventory(self):
        ...
        self.assertIn('default', setup_inventory['all'])
        self.assertIn('fakeuser', setup_inventory['all'])
        self.assertIn('windows', setup_inventory['all'])
        self.assertNotIn('network', setup_inventory['all'])
        self.assertIn('default', inventory['all'])
        self.assertIn('fakeuser', inventory['all'])
        self.assertIn('windows', inventory['all'])
        self.assertIn('network', inventory['all'])
