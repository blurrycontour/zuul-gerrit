...
from tests.unit.bitbucket.mocks import BitbucketClientMock, CommonConnectionTest
from zuul.driver.bitbucket.bitbucketconnection import\
    BitbucketClient, BitbucketConnectionError, BitbucketWatcher
...
class TestBitbucketWatcher(CommonConnectionTest):

    @unittest.skip
    @patch('threading.Thread')
    def test_private_run(self):
        con = self._connection()
        watcher = BitbucketWatcher(con, 10)
        watcher._run()
