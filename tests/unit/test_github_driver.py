...
class TestCodeownersParsing(ZuulTestCase):
    ...
    @staticmethod
    def _generate_team_event(org, team_name, team_slug, action, repo,
                             permission):
        result = ('team', {
            'action': action,
            'repository': {
    ...
    @staticmethod
    def _generate_membership_event(org, team_name, team_slug, action, user):
        return ('membership', {
            'action': action,
            'member': {
                'login': user
    ...
    def setUp(self):
        super().setUp()
        testteam1 = FakeGithubTeam('team1 CI/CD', slug='team1',
                                   members={'mhosch'}, permission='push')
        testteam2 = FakeGithubTeam('team2', members={'tobiash', 'jkubovy'},
                                   permission='push')
        testteam3 = FakeGithubTeam('team3', members={'swestphahl'},
                                   permission='pull')
        testteam4 = FakeGithubTeam('team4', members={'goofy'},
                                   permission='admin')
        testteam5 = FakeGithubTeam('team5', members={'barf'},
                                   permission='pull')
        ...
        self.fake_github.github_data.organizations['org'] = testorg
    ...
    def test_review_from_team_member_after_granting_access(self):
        ...
        self.fake_github.emitEvent(
            self._generate_membership_event('org', 'team2 foo/bar', 'team2',
                                            'added', 'mhosch'))
    ...
    def test_review_from_team_member_after_removing_repo(self):
        ...
        # Change team3 permissions to push on org/project1
        client = self.fake_github.getGithubClient('org/project1')
        repo = client.repository('org', 'project1')
        repo._teams = [x for x in repo._teams
                       if x.name != 'team2']
    ...
    def test_review_from_team_member_after_deleting_team(self):
        ...
        # Remove team2 from the repo
        repo = self.fake_github.github_data.repos.get(('org', 'project1'))
        repo._teams = [team for team in repo._teams if team.slug != 'team2']
        self.fake_github.emitEvent(
            self._generate_team_event('org',
                                      'team2',
                                      'team2',
    ...
    def test_codeowners_matching(self):
        github = self.fake_github
        codeowners = github._getCodeowners('org/project1', 'master', None)

        reviewers = codeowners.getReviewersForFiles({'zuul.yaml'})
        self.assertEqual(reviewers,
                         [
                             ('*.yaml              @org/team2 @org/team3',
                              ['@org/team2', '@org/team3'])
                         ])
    ...
    def test_cache_consistency_after_revoking_repo_access(self):
        ...
        repo._teams.append(FakeGithubTeam('team5', members={'barf'},
                                          permission='push'))
        self.fake_github.emitEvent(self._generate_team_event(
            'org', 'team5 foo', 'team5', 'added_to_repository', 'project1',
            'push'))
    ...
    def test_case_sensitivity(self):
        self._test_pipeline_with_codeowners(
            {'test.java': '#include <concepts>'}, ['goofy'], True)
