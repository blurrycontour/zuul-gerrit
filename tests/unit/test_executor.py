...
class TestExecutorFacts(AnsibleZuulTestCase):
    ...
    def test_fact_exposition(self):
        ...
        date_time = j[0]['plays'][0]['tasks'][0]['hosts']['localhost'][
            'ansible_date_time'
        ]
        self.assertEqual(18, len(date_time))

        # Check timezone is UTC as expected
        self.assertEqual('UTC', date_time['tz'])
        self.assertEqual('+0000', date_time['tz_offset'])
