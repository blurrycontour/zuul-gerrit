...
class TestSchedulerZone(ZuulTestCase):
    ...
    def test_jobs_executed(self):
        ...
        queue = list(self.build_queue.all())
...
class TestScheduler(ZuulTestCase):
    ...
    def test_failed_change_at_head_with_queue(self):
        ...
        self.waitUntilSettled()
        queue = list(self.build_queue.requested())
        self.assertEqual(len(self.builds), 0)
        self.assertEqual(len(queue), 1)
        self.assertEqual(self.build_queue.DEFAULT_ZONE, queue[0].zone)
        ...
        self.build_queue.release(queue[0])
        self.waitUntilSettled()
    ...
    def test_delayed_merge_conflict(self):
        ...
        queue = list(self.build_queue.requested())
        self.build_queue.release(queue[0])
        self.build_queue.release(queue[1])
        self.waitUntilSettled()
    ...
    def test_cancel_starting_build(self):
        ...
        for _ in iterate_timeout(
            60, 'Wait for build to be in starting phase', sleep=1.0
        ):
    ...
    def test_stuck_job_cleanup(self):
        ...
        self.build_queue.release('gate-noop')
        self.waitUntilSettled()
    ...
    def test_live_reconfiguration_command_socket(self):
        ...
        # record previous tenant layout state, which may not be set
    ...
    def test_timer_template(self):
        ...
        for job in self.merge_job_history.values():
            if job.job_type == MergeJobType.REF_STATE:
...
For CI problems and help debugging, contact ci@example.org"""
    ...
    def test_pending_merge_in_reconfig(self):
        ...
        jobs = list(self.merge_job_queue.all())
        self.assertEqual(len(jobs), 1)
        self.assertEqual(jobs[0].state, MergeJobState.HOLD)
