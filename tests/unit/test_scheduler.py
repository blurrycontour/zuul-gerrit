...
class TestScheduler(ZuulTestCase):
    ...
    def test_non_enqueue(self):
        ...
        # A -> B --> D -> E   change B, C, D formed a circular
        #       \   /         dependency
        #         C

        # set change E to WIP, so request to enqueu A would fail
        # because change E violates the pipeline requirement on wip
        E.setWorkInProgress(True)
        ...
        self.assertThat(A.messages[0], MatchesRegex(
                        r'.*Change <Change 0x[0-9a-z]+ org/project 6,1> '
                        r'does not match pipeline requirement .*'
                        r'Change does not match WIP requirement'
