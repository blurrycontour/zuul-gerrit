...
class TestTenantReconfiguration(ZuulTestCase):
    ...
    def _directAdd(self, config=True):
        ...
        if config:
            newfile = os.path.join(self.upstream_root, self.project_reconfig, '.zuul.yaml')
            ...
            newfile = os.path.join(self.upstream_root, self.project_reconfig, 'file.txt')
            content = str(self.add_counter)
        self.add_counter = self.add_counter + 1

        with open(newfile, 'w') as f:
            f.write(str(self.add_counter))
