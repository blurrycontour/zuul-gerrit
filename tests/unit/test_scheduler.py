...
class TestScheduler(ZuulTestCase):
    ...
    def test_failed_change_at_head_with_queue(self):
        ...
        self.zk_builds.setHoldInQueue(False)
        self.zk_builds.release()
        self.scheds.execute(lambda app: app.sched.zk_builds
                            .waitUntilReleased())
        self.waitUntilSettled()
    ...
    def test_source_cache(self):
        ...
        self.assertEqual(A.data['status'], 'MERGED')
        self.assertEqual(B.data['status'], 'MERGED')
        # Initial, refresh in main loop and isMerged
        self.assertEqual(A.queried, 3)
        # Initial A, refresh from B, refresh in main loop and isMerged
        self.assertEqual(B.queried, 4)
    ...
    def test_delayed_merge_conflict(self):
        ...
        self.scheds.execute(lambda app: app.sched.zk_builds
                            .waitUntilReleased(queue[0][1]))
        self.zk_builds.release(queue[1][1])
        self.scheds.execute(lambda app: app.sched.zk_builds
    ...
    def test_cancel_starting_build(self):
        ...
        for _ in iterate_timeout(30, 'Wait for executor:stop request'):
            root = "%s/%s" % (self.zk_builds.ROOT, self.zk_builds.DEFAULT_ZONE)
            keys = self.zk_client.client.get_children(root)
            if len(keys) > 0:
                path = "%s/%s/cancel" % (root, keys[0])
                if self.zk_client.client.exists(path):
                    break
    ...
    def test_stuck_job_cleanup(self):
        ...
        self.assertEqual(len(self.zk_builds.inState()), 1)

        self.commitConfigUpdate('common-config', 'layouts/no-jobs.yaml')
        self.scheds.execute(lambda app: app.sched.reconfigure(app.config))
        self.waitUntilSettled()

        self.zk_builds.release('gate-noop')
        self.scheds.execute(lambda app: app.sched.zk_builds
    ...
    def test_reconfigure_merge(self):
        ...
        with self.scheds.first.sched.run_handler_lock:
            ...
            mgmt_events = list(itertools.chain.from_iterable(
                self.scheds.first.sched.management_events.values()
            ))
