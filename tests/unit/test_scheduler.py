...
class TestScheduler(ZuulTestCase):
    ...
    def test_irrelevant_files_match_skips_job(self):
        self._test_irrelevant_files_jobs({'ignoreme': 'ignored\n'}, 'project-test-irrelevant-files', should_skip=True)

    @simple_layout('layouts/irrelevant-files.yaml')
    def test_irrelevant_files_no_match_skips_job(self):
        self._test_irrelevant_files_jobs({'respectme': 'please!\n'}, 'project-test-irrelevant-files', should_skip=False)
    ...
    def test_irrelevant_files_one_not_included_one_excluded_runs_job(self):
        "Neither file is really relevant for the job but the job is NOT skipped."
        self._test_irrelevant_files_jobs({
            'B/includeme': 'not!\n',
            'A/excludeme': 'excluded\n'
    ...
    def test_fileset_one_not_included_one_excluded_skips_job(self):
        "Same changed files as in test_irrelevant_files_one_not_included_one_excluded_runs_job but job is skipped."
        self._test_irrelevant_files_jobs({
            'B/includeme': 'not!\n',
            'A/excludeme': 'excluded\n'
        }, 'project-test-fileset', should_skip=True)
