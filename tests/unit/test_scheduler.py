...
class TestSchedulerZone(ZuulTestCase):
    ...
    def setUp(self):
        ...
        self.zk.builds.register_all_zones()
    ...
    def test_jobs_executed(self):
        ...
        queue = self.zk.builds.in_state('ALL')
...
class TestScheduler(ZuulTestCase):
    ...
    def test_failed_change_at_head_with_queue(self):
        ...
        self.zk.builds.set_hold_in_queue(True)
        self.scheds.execute(lambda app: app.sched.zk.builds
                            .set_hold_in_queue(True))
        A = self.fake_gerrit.addFakeChange('org/project', 'master', 'A')
        B = self.fake_gerrit.addFakeChange('org/project', 'master', 'B')
        C = self.fake_gerrit.addFakeChange('org/project', 'master', 'C')
        A.addApproval('Code-Review', 2)
        ...
        self.waitUntilSettled()
        queue = self.zk.builds.in_state(['HOLD', 'REQUESTED'])
        self.assertEqual(len(self.builds), 0)
        self.assertEqual(len(queue), 1)
        self.assertEqual(self.zk.builds.DEFAULT_ZONE,
        ...
        self.zk.builds.release('.*-merge')
        self.scheds.execute(lambda app: app.sched.zk.builds
                            .wait_until_released('.*-merge'))
        self.waitUntilSettled()
        ...
        self.zk.builds.release(queue[0][1])
        self.scheds.execute(lambda app: app.sched.zk.builds
                            .wait_until_released(queue[0][1]))
        self.waitUntilSettled()
        ...
        self.scheds.execute(lambda app: app.sched.zk.builds
                            .set_hold_in_queue(False))
        self.zk.builds.set_hold_in_queue(False)
        self.zk.builds.release()
        self.scheds.execute(lambda app: app.sched.zk.builds
                            .wait_until_released())
        self.waitUntilSettled()
    ...
    def test_delayed_merge_conflict(self):
        ...
        self.scheds.execute(lambda app: app.sched.zk.builds
                            .wait_until_released(queue[0][1]))
        self.zk.builds.release(queue[1][1])
        self.scheds.execute(lambda app: app.sched.zk.builds
                            .wait_until_released(queue[1][1]))
        self.waitUntilSettled()
    ...
    def test_cancel_starting_build(self):
        ...
        for _ in iterate_timeout(30, 'Wait for executor:stop request'):
            root = "%s/%s" % (self.zk.builds.ROOT, self.zk.builds.DEFAULT_ZONE)
            keys = self.zk.kazoo_client.get_children(root)
            if len(keys) > 0:
                path = "%s/%s/cancel" % (root, keys[0])
                if self.zk.kazoo_client.exists(path):
                    break
    ...
    def test_stuck_job_cleanup(self):
        ...
        self.assertEqual(len(self.zk.builds.in_state('ALL')), 1)

        self.commitConfigUpdate('common-config', 'layouts/no-jobs.yaml')
        self.scheds.execute(lambda app: app.sched.reconfigure(app.config))
        self.waitUntilSettled()

        self.zk.builds.release('gate-noop')
        self.scheds.execute(lambda app: app.sched.zk.builds
                            .wait_until_released('gate-noop'))
        self.waitUntilSettled()
