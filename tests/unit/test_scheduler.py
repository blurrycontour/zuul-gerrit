...
class TestSemaphoreInRepo(ZuulTestCase):
    ...
    def test_semaphore_in_repo(self):
        ...
        # check that the layout in a queue item still has max value of 2
        # for test-semaphore
        check_pipeline = tenant.layout.pipelines.get('check')
        queue_item = check_pipeline.queues[0].queue[0]
        ...
        self.assertEqual(dynamic_test_semaphore.max, 2)

        # two build must be in queue, two semaphores acquired
        self.assertEqual(len(self.builds), 2)
        ...
        # one build must be left in queue, one semaphore acquired
        self.assertEqual(len(self.builds), 1)
        self.assertEqual(self.builds[0].name, 'project-test2')
        self.assertTrue('test-semaphore' in
                        tenant.semaphore_handler.semaphores)
