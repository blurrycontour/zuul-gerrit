...
class TestReconfigureBranch(ZuulTestCase):
    ...
    def _addConfig(self):
        ...
        self.fake_gerrit.addEvent(A.getRefUpdatedEvent())
        self.waitUntilSettled()
        self.hexsha = A.patchsets[-1]['revision']
...
class TestReconfigureBranchCreateConfDeleteSshHttp(TestReconfigureBranch):
    ...
    def test_reconfigure_cache_branch_create_update_delete(self):
        ...
        self._createBranch()
        self._expectReconfigure(True)
        # when ref-updated direct push we can only fetch the files that are
        # listed in commit (snapshot) we can't get files that were only
        # modified between oldrev and newrev, we can't do:
