...
class TestSchedulerAutoholdHoldExpiration(ZuulTestCase):
    ...
    def test_autohold_max_hold_default(self):
        ...
        request_list = self.scheds.first.zk.getHoldRequests()
        self.assertEqual(1, len(request_list))
        request = self.scheds.first.zk.getHoldRequest(request_list[0])
...
class TestScheduler(ZuulTestCase):
    ...
    def test_failed_change_in_middle(self):
        ...
        tenant = self.scheds.first.abide.tenants.get('tenant-one')
    ...
    def _test_time_database(self, iteration):
        ...
        data = json.loads(self.scheds.first.formatStatusJSON('tenant-one'))
        found_job = None
    ...
    def test_autohold(self):
        ...
        request2 = self.scheds.first.zk.getHoldRequest(request.id)
    ...
    def test_autohold_request_expiration(self):
        ...
        def reset_exp():
            self.scheds.first.EXPIRED_HOLD_REQUEST_TTL = orig_exp
        ...
        self.scheds.first.EXPIRED_HOLD_REQUEST_TTL = 1
        time.sleep(3)
    ...
    def test_noop_job(self):
        ...
        self.assertTrue(self.scheds.first._areAllBuildsComplete())
        self.assertEqual(len(self.history), 0)
        self.assertEqual(A.data['status'], 'MERGED')
        self.assertEqual(A.reported, 2)
    ...
    def test_statsd(self):
        ...
        statsd = self.scheds.first.statsd
    ...
    def test_reconfigure_merge(self):
        ...
        tenant = self.scheds.first.abide.tenants['tenant-one']
        (trusted, project) = tenant.getProject('org/project')

        self.scheds.first.run_handler_lock.acquire()
        self.assertEqual(self.scheds.first.management_event_queue.qsize(), 0)

        self.scheds.first.reconfigureTenant(tenant, project, None)
        self.assertEqual(self.scheds.first.management_event_queue.qsize(), 1)
        ...
        self.scheds.first.run_handler_lock.release()
        self.waitUntilSettled()
    ...
    def test_live_reconfiguration_command_socket(self):
        ...
        old = self.scheds.first.tenant_last_reconfigured.get('tenant-one', 0)
        self.waitUntilSettled()
        ...
        while True:
            ...
            new = self.scheds.first.tenant_last_reconfigured\
                .get('tenant-one', 0)
            if old < new:
                break
            else:
    ...
    def test_worker_update_metadata(self):
        ...
        self.assertEqual(len(self.scheds.first.executor.builds), 1)

        self.log.debug('Current builds:')
        self.log.debug(self.scheds.first.executor.builds)
        ...
        while True:
            if time.time() - start > 10:
                raise Exception("Timeout waiting for gearman server to report "
                                + "back to the client")
            build = list(self.scheds.first.executor.builds.values())[0]
...
For CI problems and help debugging, contact ci@example.org"""
    ...
    def test_zookeeper_disconnect(self):
        ...
        self.scheds.execute(lambda sched: sched.zk.client.stop())
        self.scheds.execute(lambda sched: sched.zk.client.start())
        self.fake_nodepool.unpause()
        self.waitUntilSettled()
    ...
    def test_zookeeper_disconnect2(self):
        ...
        self.fake_nodepool.unpause()
        requests = list(self.scheds.first.nodepool.requests.values())
    ...
    def test_nodepool_relative_priority_check(self):
        ...
        for x in iterate_timeout(30, 'fulfill request'):
            if len(self.scheds.first.nodepool.requests) < 4:
                break
        self.waitUntilSettled()
    ...
    def test_pending_merge_in_reconfig(self):
        ...
        self.scheds.first.reconfigureTenant(
            self.scheds.first.abide.tenants['tenant-one'], None, None)
        self.waitUntilSettled()
        # Verify the merge job is still running and that the item is
        # in the pipeline
        self.assertEqual(len(self.scheds.first.merger.jobs), 1)
        tenant = self.scheds.first.abide.tenants.get('tenant-one')
...
class TestSemaphore(ZuulTestCase):
    ...
    def test_semaphore_zk_error(self):
        ...
        orig_useNodeSet = self.scheds.first.nodepool.useNodeSet

        def broken_use_nodeset(nodeset, build_set=None, event=None):
            # restore original useNodeSet
            self.scheds.first.nodepool.useNodeSet = orig_useNodeSet
            raise NoNodeError()

        self.scheds.first.nodepool.useNodeSet = broken_use_nodeset
    ...
    def test_semaphore_abandon_pending_execution(self):
        ...
        self.fake_gerrit.addEvent(A.getPatchsetCreatedEvent(1))
        self.waitUntilSettled()
        self.assertEqual(len(self.scheds.first.nodepool.requests), 2)
        ...
        for _ in iterate_timeout(30, 'fulfill node requests'):
            if len(self.scheds.first.nodepool.requests) == 0:
                break
...
class TestSemaphoreMultiTenant(ZuulTestCase):
    ...
    def test_semaphore_tenant_isolation(self):
        ...
        self.waitUntilSettled()
        tenant_one = self.scheds.first.abide.tenants.get('tenant-one')
        tenant_two = self.scheds.first.abide.tenants.get('tenant-two')
...
class TestSchedulerSmartReconfiguration(ZuulTestCase):
    ...
    def _test_smart_reconfiguration(self, command_socket=False):
        ...
        # record previous tenant reconfiguration time, which may not be set
        old_one = self.scheds.first.tenant_last_reconfigured\
            .get('tenant-one', 0)
        old_two = self.scheds.first.tenant_last_reconfigured\
            .get('tenant-two', 0)
        self.waitUntilSettled()
        ...
        while True:
            ...
            new_two = self.scheds.first.tenant_last_reconfigured\
                .get('tenant-two', 0)
            if old_two < new_two:
                break
            else:
        ...
        self.waitUntilSettled()
        new_one = self.scheds.first.tenant_last_reconfigured\
            .get('tenant-one', 0)
        ...
        self.assertEqual(expected_tenants,
                         self.scheds.first.abide.tenants.keys())

        self.assertIsNotNone(
            self.scheds.first.tenant_last_reconfigured.get('tenant-four'),
