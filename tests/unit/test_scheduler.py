...
class TestScheduler(ZuulTestCase):
    ...
    def test_failed_change_at_head_with_queue(self):
        ...
        self.build_queue.hold_in_queue = True
        ...
        self.waitUntilSettled()
        queue = self.build_queue.requested()
        self.assertEqual(len(self.builds), 0)
        self.assertEqual(len(queue), 1)
        self.assertEqual(self.build_queue.DEFAULT_ZONE, queue[0][1].zone)
        job_args = queue[0][1].params
        self.assertEqual(job_args['job'], 'project-merge')
        self.assertEqual(job_args['items'][0]['number'], '%d' % A.number)

        self.build_queue.release('.*-merge')
        #self.scheds.execute(lambda app: app.sched.zk_builds
        #                    .waitUntilReleased('.*-merge'))
        self.waitUntilSettled()

        queue = self.zk_builds.requested()
        self.assertEqual(len(self.builds), 0)
        ...
        #                    .waitUntilReleased('.*-merge'))
        self.waitUntilSettled()
        queue = self.zk_builds.in_state(BuildState.HOLD, BuildState.REQUESTED)

        self.assertEqual(len(self.builds), 0)
        self.assertEqual(len(queue), 6)

        self.assertEqual(queue[0][1].params['job'], 'project-test1')
        self.assertEqual(queue[1][1].params['job'], 'project-test2')
        self.assertEqual(queue[2][1].params['job'], 'project-test1')
        self.assertEqual(queue[3][1].params['job'], 'project-test2')
        self.assertEqual(queue[4][1].params['job'], 'project-test1')
        self.assertEqual(queue[5][1].params['job'], 'project-test2')
    ...
    def test_stuck_job_cleanup(self):
        ...
        self.assertEqual(len(self.zk_builds.in_state()), 1)
