...
class TestBranchVariants(ZuulTestCase):
    ...
    def test_branch_variants_divergent(self):
        ...
        ipath = self.builds[1].parameters['zuul']['_inheritance_path']
...
class TestSecretInheritance(ZuulTestCase):
    ...
    def _checkTrustedSecrets(self):
        ...
        self.assertEqual(
            self._getSecrets('trusted-secrets-trusted-child', 'playbooks'),
            [{}, {'trusted-secret': secret}, {}])
        self.assertEqual(
            self._getSecrets('trusted-secrets-trusted-child', 'pre_playbooks'), [])
        self.assertEqual(
            self._getSecrets('trusted-secrets-trusted-child', 'post_playbooks'), [])

        self.assertEqual(
            self._getSecrets('trusted-secrets-untrusted-child', 'playbooks'),
            [{}, {'trusted-secret': secret}, {}])
        self.assertEqual(
            self._getSecrets('trusted-secrets-untrusted-child', 'pre_playbooks'), [])
        self.assertEqual(
            self._getSecrets('trusted-secrets-untrusted-child', 'post_playbooks'), [])
    ...
    def _checkUntrustedSecrets(self):
        ...
        self.assertEqual(
            self._getSecrets('untrusted-secrets-trusted-child', 'playbooks'),
            [{}, {'untrusted-secret': secret}, {}])
        self.assertEqual(
            self._getSecrets('untrusted-secrets-trusted-child', 'pre_playbooks'), [])
        self.assertEqual(
            self._getSecrets('untrusted-secrets-trusted-child', 'post_playbooks'), [])

        self.assertEqual(
            self._getSecrets('untrusted-secrets-untrusted-child', 'playbooks'),
            [{}, {'untrusted-secret': secret}, {}])
        self.assertEqual(
            self._getSecrets('untrusted-secrets-untrusted-child', 'pre_playbooks'), [])
        self.assertEqual(
            self._getSecrets('untrusted-secrets-untrusted-child', 'post_playbooks'), [])
