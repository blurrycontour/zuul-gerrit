...
class TestEarlyFailure(AnsibleZuulTestCase):
    ...
    def test_early_failure(self):
        ...
        for _ in iterate_timeout(10, 'job A started'):
            if len(self.builds) == 1:
                break
        A_build = self.builds[0]
        start = os.path.join(self.jobdir_root, A_build.uuid +
                             '.failure_start.flag')
        for _ in iterate_timeout(10, 'job A running'):
        ...
        for _ in iterate_timeout(10, 'job B started'):
            if len(self.builds) == 2:
                break
        B_build = self.builds[1]
        start = os.path.join(self.jobdir_root, B_build.uuid +
                             '.wait_start.flag')
        for _ in iterate_timeout(10, 'job B running'):
            if os.path.exists(start):
                break
        ...
        for _ in iterate_timeout(10, 'job B restarted'):
            if len(self.builds) == 2:
        ...
        start = os.path.join(self.jobdir_root, A_build.uuid +
                             '.wait_start.flag')
        for _ in iterate_timeout(10, 'job A post running'):
            if os.path.exists(start):
                break
        ...
        for _ in iterate_timeout(10, 'job A complete'):
            ...
            for b in self.builds[:]:
                if b.name == 'pre-failure':
                    try:
                        ...
                        self.log.debug("Unable to write flag path %s", flag_path)
        self.log.debug("Done")
