...
class TestQueueLimits(ZuulTestCase):

    @simple_layout('layouts/queue-max-changes.yaml')
    def test_queue_max_changes_check(self):
        ...
        self.assertEqual(A.reported, 1)
        self.assertEqual(B.reported, 1)
        self.assertIn('too many changes', B.messages[0])

    @simple_layout('layouts/queue-max-changes.yaml')
    def test_queue_max_changes_gate(self):
        # Max-changes is 1, so this is allowed
        A = self.fake_gerrit.addFakeChange('org/project', 'master', 'A')
        A.addApproval('Code-Review', 2)
        self.fake_gerrit.addEvent(A.addApproval('Approved', 1))
        ...
        self.assertEqual(B.reported, 0)
        self.assertEqual(C.reported, 1)
        self.assertIn('too many changes', C.messages[0])
