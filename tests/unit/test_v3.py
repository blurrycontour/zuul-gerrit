...
class TestBrokenConfig(ZuulTestCase):
    ...
    fail_cat_job = {"org/project4": True}
    ...
    def test_broken_config_on_startup(self):
        ...
        self.assertIn(
            ...
            str(loading_errors[0].error))
        self.assertIn(
            "Zuul encountered a failure of \"merger:cat\" job in\n"
            "review.example.com/org/project4@master repository.",
            str(loading_errors[1].error))
        self.assertIn(
            "Zuul encountered a syntax error",
            str(loading_errors[2].error))
