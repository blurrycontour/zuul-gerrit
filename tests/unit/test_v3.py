...
class TestUnreachableGroup(TestUnreachableBase):
    ...
    def test_group_zuul_unreachable(self):
        ...
        # Unit tests don't use any nodes which are accessible via SSH. So we force to disable
        # remote ansible.
        ...
        self.assertHistory([
            dict(name='unreachable-group-in-zuul-no-hosts-leaves', result=None, changes='1,1'),
            dict(name='unreachable-group-in-zuul-no-hosts-leaves', result=None, changes='1,1'),
            dict(name='unreachable-group-in-zuul', result="SUCCESS", changes='1,1'),
        ], ordered=False)
        ...
        for build in self.history:
            ...
            if build.name == "unreachable-group-in-zuul":
                self.assertIn("This host is not unreachable: available-host", job_output)
                self.assertIn("This host is unreachable: unreachable-host", job_output)
            elif build.name == "unreachable-group-in-zuul-no-hosts-leaves":
                self.assertIn(
                    (
                        "Specified inventory, host pattern and/or --limit leaves us with no hosts "
                        "to target"
                    ),
                    job_output,
                )
