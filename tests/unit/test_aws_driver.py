...
from zuul.driver.aws.awsmodel import AwsCreateState, AwsProviderNode
...
class TestAwsDriver(ZuulTestCase):
    ...
    def test_state_machines(self):
        ...
        with self.createZKContext(None) as ctx:
            node = AwsProviderNode.new(ctx, label="debian-normal")
            execute_future = False
            for _ in iterate_timeout(30, "create state machine to complete"):
                with node.activeContext(ctx):
                    # Re-create the SM from the state in ZK
                    sm = provider.getCreateStateMachine(node, self.log)
                    sm.advance()
            ...
            for _ in iterate_timeout(30, "delete state machine to complete"):
                with node.activeContext(ctx):
                    # Re-create the SM from the state in ZK
