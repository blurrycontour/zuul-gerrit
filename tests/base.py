...
class ZuulSchedulerWrapper:
    ...
    def fullReconfigure(self, use_zk: bool = False):
        try:
            self.sched.reconfigure(self.config, use_zk=use_zk)
        except Exception:
            self.log.exception("Reconfiguration failed:")

    def smartReconfigure(self, command_socket=False, use_zk: bool = False):
        try:
            if command_socket:
...
class ZuulTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.sched_wrap = ZuulSchedulerWrapper(self.log, self.config,
                                            self.zk_config, self.connections)
        self.sched = self.sched_wrap.sched
        self.event_queues = self.sched_wrap.event_queues + self.event_queues

        if hasattr(self, 'fake_github'):
            self.event_queues.append(
                self.fake_github.github_event_connector._event_forward_queue)

        self.executor_client = self.sched_wrap.executor_client
        self.merge_client = self.sched_wrap.merge_client
        self.merge_server = None
        self.nodepool = self.sched_wrap.nodepool
        self.zk = self.sched_wrap.zk
