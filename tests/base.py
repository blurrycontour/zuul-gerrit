...
class BaseTestCase(testtools.TestCase):
    ...
    def setUp(self):
        ...
        def git_repo_init(myself, *args, **kw):
            ...
            self.log.debug("Created git repo 0x%x %s" % (id(myself), repr(myself)))
        self.useFixture(fixtures.MonkeyPatch('git.Repo.__init__', git_repo_init))
    ...
    def assertFinalState(self):
        ...
        for obj in gc.get_objects():
            if isinstance(obj, git.Repo):
                self.log.debug("Leaked git repo object: %s %s" %
                               (id(obj), repr(obj)))
                repos.append(obj)
