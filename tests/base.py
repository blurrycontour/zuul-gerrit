...
class FakeGerritChange(object):
    ...
    def queryHTTP(self):
        ...
        for app in self.patchsets[-1]['approvals']:
            ...
            _, label_min, label_max = self.categories[label]
            val = int(app['value'])
    ...
    def queryRevisionHTTP(self, revision):
        ...
        if self.depends_on_change:
            changes.append({
                "commit": {
                    "commit": self.patchsets[self.depends_on_patchset-1]['revision'],
                }})
        for (needed_by_change, needed_by_patchset) in self.needed_by_changes:
            changes.append({
                "commit": {
                    "commit": needed_by_change.patchsets[needed_by_patchset-1]['revision'],
                }})
            ...
            def get_changes(self, query):
                ...
                if query.startswith('(') and 'OR' in query:
                    ...
                    for q in query.split(' OR '):
                        for r in fake_gerrit._simpleQuery(q):
                            if r not in results:
                                results.append(r)
                else:
                    results = fake_gerrit._simpleQuery(query)
