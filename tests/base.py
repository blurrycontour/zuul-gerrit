...
class FakePagureConnection(pagureconnection.PagureConnection):
    ...
    def emitEvent(self, event, use_zuulweb=False, project=None,
        ...
        if use_zuulweb:
            ...
            self.sched.zk_connection_event.push(self.connection_name, data)
            return data
...
class RecordingExecutorServer(zuul.executor.server.ExecutorServer):
    ...
    def executeJob(self, build_item: ZooKeeperBuildItem):
        ...
        self.zk_builds.persist(build_item)
...
class SchedulerTestApp:
    def __init__(self, log: Logger, config: ConfigParser, zk_config: str,
        ...
        self.sched = TestScheduler(self.config, self.connections, zk_client,
                                   app=self)
...
class ZuulTestCase(BaseTestCase):
    ...
    def assertEmptyBuildQueueHistory(self):
        nodes = []
        for zone in self.zk_client.client.get_children(self.zk_builds.ROOT):
            nodes.extend(self.zk_client.client.get_children(
                "%s/%s" % (self.zk_builds.ROOT, zone)))
        try:
            self.log.debug(
            ...
            for path, build in self.zk_builds.inState('ALL'):
    ...
    def __areAllBuildsWaiting(self, matcher) -> bool:
        for app in self.scheds.filter(matcher):
            ...
            for build in builds:
                ...
                # build_item = self.zk_builds.getCached(build.zookeeper_node)
                data, _ = self.zk_client.client.get(build.zookeeper_node)
                content = json.loads(data.decode('UTF-8'))
                if self.zk_builds.hold_in_queue:
                    if content['state'] == 'REQUESTED':
                        self.log.debug("%s requested while holding in queue",
                                       build)
                        return False
                    elif content['state'] != 'RUNNING':
                        continue

                if self.executor_server.hold_jobs_in_queue_sensor.pause\
                        and not self.zk_builds.isLocked(build.zookeeper_node)\
    ...
    def _releaseZookeeperLockingLocks(self, matcher=None):
        for sched in map(lambda app: app.sched, self.scheds.filter(matcher)):
            if sched.zk_client.locking_lock.locked():
                sched.zk_client.locking_lock.release()
