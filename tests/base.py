...
class ZuulTestCase(BaseTestCase):
    ...
    @property
    def event_queues(self) -> List[Queue]:
        sched_queues = map(lambda app: app.event_queues, self.scheds)
    ...
    def shutdown(self):
        ...
        self.scheds.execute(lambda sched: sched.stop())
        self.scheds.execute(lambda sched: sched.join())
        self.statsd.stop()
        self.statsd.join()
    ...
    def haveAllBuildsReported(self) -> bool:
    ...
    def areAllBuildsWaiting(self) -> bool:
        for app in self.scheds:
            executor_client = app.sched.executor
        ...
        while True:
            ...
            if self.haveAllBuildsReported():
                ...
                self.scheds.execute(
                    lambda sched: sched.run_handler_lock.release())
            self.executor_server.lock.release()
            self.scheds.execute(lambda sched: sched.wake_event.wait(0.1))
