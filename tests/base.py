...
class GerritDriverMock(GerritDriver):
    def __init__(self, registry, changes: ChangesDatabase, upstream_root: str,
                 additional_event_queues, poller_events,
        ...
        while True:
            ...
            for (fd, event) in ret:
                if fd == self.sock.fileno():
                    ...
                    if not data:
                        return
                    # self.log.debug("Appending: %s" % data[0]) TODO Uncomment
...
class FakeBuild(object):
    ...
    def __init__(self, executor_server: 'RecordingExecutorServer',
                 build_item: ZooKeeperBuildItem):
        self.daemon = True  # type: bool
        self.executor_server = executor_server  # type: RecordingExecutorServer
        self.jobdir = None  # type: Optional[JobDir]
        self.uuid = build_item.content['uuid']  # type: str
        self.parameters = build_item.content['params']  # type: Dict[str, Any]
        ...
        self.unique = self.parameters['zuul']['build']  # type: str
        self.pipeline = self.parameters['zuul']['pipeline']  # type: str
        self.project = self.parameters['zuul']['project']['name']  # type: str
        self.name = self.parameters['job']  # type: str
        self.wait_condition = threading.Condition()
        self.waiting = False  # type: bool
        self.paused = False  # type: bool
        self.aborted = False  # type: bool
        self.requeue = False  # type: bool
        self.created = time.time()  # type: float
        self.changes = None
        items = self.parameters['zuul']['items']
        self.changes = ' '.join(['%s,%s' % (x['change'], x['patchset'])
                                 for x in items if 'change' in x])  # type: str
...
class RecordingAnsibleJob(zuul.executor.server.AnsibleJob):
    ...
    def __init__(self, executor_server: 'RecordingExecutorServer',
                 build_item: ZooKeeperBuildItem):
        super().__init__(executor_server, build_item)
        self.executor_server = executor_server  # type: RecordingExecutorServer
    ...
    def recordResult(self, result):
        ...
        build = self.executor_server\
            .job_builds[self.build_item.content['uuid']]
        if not build:
            self.executor_server.lock.release()
            # Already recorded
            return
        history = BuildHistory(
            name=build.name, result=result, changes=build.changes,
        ...
        self.log.debug("Appending to build history: %s" % history)
        self.executor_server.build_history.append(history)
        self.executor_server.running_builds.remove(build)
        del self.executor_server.job_builds[self.build_item.content['uuid']]
        self.executor_server.lock.release()
    ...
    def resume(self):
        build = self.executor_server\
            .job_builds.get(self.build_item.content['uuid'])
        if build:
            build.paused = False
        super().resume()
...
class RecordingExecutorServer(zuul.executor.server.ExecutorServer):
    ...
    def __init__(self, *args, **kw):
        self._run_ansible = kw.pop('_run_ansible', False)  # type: bool
        self._test_root = kw.pop('_test_root', False)  # type: bool
        ...
        self.hold_jobs_in_build = False  # type: bool
        self.hold_jobs_in_start = False  # type: bool
        self.lock = threading.Lock()  # type: Lock
        self.running_builds = []  # type: List[FakeBuild]
        self.build_history = []  # type: List[BuildHistory]
        self.fail_tests = {}  # type: Dict[str, List[Any]]
            ...
            if build.isWaiting():
                if not regex or re.match(regex, build.name):
                    ...
                    self.log.debug("Not releasing build %s" %
                                   (build.parameters['zuul']['build']))
            self.resumed_jobs_in_queue.append(build.uuid)
        self.log.debug("Done releasing builds %s (%s)" %
                       (regex, len(builds)))

    def executeJob(self, build_item: ZooKeeperBuildItem):
        build = FakeBuild(self, build_item)
        self.log.debug("Next recording job: %s" % build)
        self.running_builds.append(build)
        self.job_builds[build_item.content['uuid']] = build
        build_item.content['params']['zuul']['_test'] = dict(
            test_root=self._test_root)
        super().executeJob(build_item)

    def stopJob(self, build_item: ZooKeeperBuildItem):
...
class ZuulWebFixture(fixtures.Fixture):
    def __init__(self, gearman_server_port, changes: ChangesDatabase,
...
class BaseTestCase(testtools.TestCase):
    ...
    def setUp(self):
        ...
        log_defaults_from_env = os.environ.get(
            'OS_LOG_DEFAULTS',
            'git.cmd=INFO,kazoo=WARNING,gear=WARNING')
...
class SchedulerTestApp:
    def __init__(self, log: Logger, config: ConfigParser, zk_config: str,
                 changes: ChangesDatabase, additional_event_queues,
                 upstream_root: str, rpcclient: RPCClient, poller_events,
        ...
        zk.connect(self.zk_config, timeout=60.0)
...
class SchedulerTestManager:
    def __init__(self, changes: ChangesDatabase):
        self.instances = []
        self.changes = changes
...
class ZuulTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.scheds = SchedulerTestManager(self.changes)
        self.scheds.create(
            self.log, self.config, self.zk_config,
    ...
    def assertEmptyBuildQueueHistory(self):
        # builds_count = -1
        try:
            for _ in iterate_timeout(10, "zk builds to become empty"):
                builds_count = len(self.zk.builds.all)
                if builds_count == 0:
                    break
        except Exception:  # as e:
            self.log.error("Uncleaned builds in Zookeeper:")
            for path, build in self.zk.builds.all:
                self.log.error("  - %s: %s" % (path, build))
            # Builds need to be cleaned from ZK
            # TODO JK: Uncomment: raise e
        # self.assertEqual(0, builds_count)

    def assertNoZkConnections(self):
        open_count = len(ZooKeeperClient.connections.values())
        if open_count > 0:
            for uid, stack in ZooKeeperClient.connections.items():
                self.log.error("ZK Not disconnected (%s): %s", uid, stack)
            raise RuntimeError("Not all ZK connections closed (%s opened)!" %
                               open_count)

    def assertFinalState(self):
        self.log.debug("Assert final state, job workers: %s" %
                       self.executor_server.job_workers)
        ...
        self.assertEmptyBuildQueueHistory()
    ...
    def shutdown(self):
        ...
        if len(threads) > 1:
            for thread in threads:
                self.log.debug("Thread name: %s" % thread.name)
            log_str = ""
    ...
    def release(self, job):
        if isinstance(job, FakeBuild):
            job.release()
        elif isinstance(job, ZooKeeperBuildItem):
            pass
        else:
            job.waiting = False
    ...
    def __areAllBuildsWaiting(self, matcher) -> bool:
        for app in self.scheds.filter(matcher):
            ...
            if not executor_client:
                raise Exception("Scheduler is missing executor client!")
            builds = executor_client.builds.values()
            seen_builds = set()
            for build in builds:
                seen_builds.add(build.uuid)
                if not build.zookeeper_node:
                    self.log.debug("%s is not known in Zookeeper" % build)
                    return False

                # if self.executor_server.hold_jobs_in_queue_sensor.pause:
                #     self.zk.pause(build.zookeeper_node)
                build_item = self.zk.builds.get(build.zookeeper_node)
                if self.zk.builds.hold_in_queue:
                    if build_item.content['state'] == 'REQUESTED':
                        self.log.debug("%s requested while holding in qeue" %
                                       build)
                        return False
                    elif build_item.content['state'] != 'RUNNING':
                        continue

                if self.executor_server.hold_jobs_in_queue_sensor.pause\
                        and not self.zk.builds.is_locked(build.zookeeper_node)\
                        and build.uuid not in self.executor_server\
                        .resumed_jobs_in_queue:
                    continue
    ...
    def __eventQueuesJoin(self, matcher) -> None:
        for app in self.scheds.filter(matcher):
            if app.sched.is_resumed:
                for event_queue in app.event_queues:
                    self.log.debug('__eventQueuesJoin[%s]: %s: %s',
                                   app.sched.name, event_queue,
                                   event_queue.qsize())
                    event_queue.join()
        for event_queue in self.additional_event_queues:
            self.log.debug('__eventQueuesJoin[additional]: %s: %s',
    ...
    def __areZookeeperEventQueuesEmpty(self, matcher=None) -> bool:
        for sched in map(lambda app: app.sched, self.scheds.filter(matcher)):
            for connection in list(sched.connections.connections.values()):
                if sched.zk.connection_event.has_events(
                        connection.connection_name, keep_locked=True):
                    return False
        return True

    def __acquireZookeeperWorkerLocks(self, matcher=None):
        for sched in map(lambda app: app.sched, self.scheds.filter(matcher)):
            for connection in list(sched.connections.connections.values()):
                if hasattr(connection, 'zookeeper_worker'):
                    connection.zookeeper_worker.lock.acquire()

    def __releaseZookeeperWorkerLocks(self, matcher=None):
        for sched in map(lambda app: app.sched, self.scheds.filter(matcher)):
            for connection in list(sched.connections.connections.values()):
                if hasattr(connection, 'zookeeper_worker'):
                    connection.zookeeper_worker.lock.release()

    def __releaseZookeeperLockingLocks(self, matcher=None):
        for sched in map(lambda app: app.sched, self.scheds.filter(matcher)):
    ...
    def waitUntilSettled(self, msg="", matcher=None) -> None:
        ...
        while True:
            ...
            self.executor_server.lock.acquire()
            self.__acquireZookeeperWorkerLocks(matcher)
            ...
            if self.__haveAllBuildsReported(matcher):
                ...
                areZookeeperEventQueuesEmpty = self\
                    .__areZookeeperEventQueuesEmpty(matcher)
                ...
                self.__releaseZookeeperLockingLocks(matcher)
                ...
                if (areZookeeperEventQueuesEmpty and
                    ...
                    self.log.debug("Scheduler run handler: Released (1)")
                    self.__releaseZookeeperWorkerLocks(matcher)
                    self.executor_server.lock.release()
                ...
                self.scheds.execute(
                    lambda app: app.sched.run_handler_lock.release())
                self.log.debug("Scheduler run handler: Released (2)")
            self.__releaseZookeeperWorkerLocks(matcher)
            self.executor_server.lock.release()
            self.scheds.execute(lambda app: app.sched.wake_event.wait(0.1))
            time.sleep(0.5)  # TODO JK: Busy loop - maybe starving?

    def __logQueueStatus(self, logger, matcher, areZookeeperEventQueuesEmpty,
                         areAllMergeJobsWaiting, haveAllBuildsReported,
                         areAllBuildsWaiting, areAllNodeRequestsComplete,
                         allEventQueuesEmpty):
        logger("Queue status:")
