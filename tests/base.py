...
class FakePagurePullRequest(object):
    ...
    def __init__(self, pagure, number, project, branch,
                 title, upstream_root, files=[], number_of_commits=1,
                 body=None):
        self.pagure = pagure
        self.source = pagure
        self.number = number
...
class FakePagureAPIClient(pagureconnection.PagureAPIClient):
    ...
    def get(self, url):
        ...
        match = re.match('.+/api/0/(.+)/pull-request/(\d+)$', url)
        if match:
        ...
        match = re.match('.+/api/0/(.+)/pull-request/(\d+)/flag$', url)
        if match:
            pr = self._get_pr(match)
        ...
        match = re.match('.+/api/0/(.+)/pull-request/(\d+)/diffstats$', url)
        if match:
            pr = self._get_pr(match)
            repo_path = os.path.join(pr.upstream_root, pr.project)
            paths = git.cmd.Git(repo_path).log(
                '--name-only', '-n1', '--pretty=format:').splitlines()
            return dict([(p, None) for p in paths])
    ...
    def post(self, url, params=None):
        ...
        match = re.match('.+/api/0/(.+)/pull-request/(\d+)/comment$', url)
        if match:
            pr = self._get_pr(match)
...
class FakePagureConnection(pagureconnection.PagureConnection):
    ...
    def emitEvent(self, event):
        name, payload = event
        # TODO(fbo): Need to also test via zuul-web
