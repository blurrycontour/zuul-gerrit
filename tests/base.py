...
class FakeBuild(object):
    ...
    def __init__(self, execute_server, job):
        self.daemon = True
        self.execute_server = execute_server
        self.job = job
        self.jobdir = None
    ...
    def run(self):
        ...
        if self.execute_server.hold_jobs_in_build:
    ...
    def shouldFail(self):
        changes = self.execute_server.fail_tests.get(self.name, [])
...
class RecordingExecuteServer(zuul.executor.server.ExecuteServer):
    ...
    def __init__(self, *args, **kw):
        ...
        super(RecordingExecuteServer, self).__init__(*args, **kw)
    ...
    def stopJob(self, job):
        ...
        for build in self.running_builds:
            if build.unique == uuid:
                build.aborted = True
                build.release()
        super(RecordingExecuteServer, self).stopJob(job)
...
class ZuulTestCase(BaseTestCase):
    ...
    :ivar RecordingExecuteServer execute_server: An instance of
        :py:class:`~tests.base.RecordingExecuteServer` which is the
    ...
    def setUp(self):
        ...
        self.execute_server = RecordingExecuteServer(
        ...
        self.history = self.execute_server.build_history
        self.builds = self.execute_server.running_builds

        self.execute_client = zuul.executor.client.ExecuteClient(
        ...
        self.sched.setExecutor(self.execute_client)
        ...
        self.webapp.start()
        self.rpc.start()
        self.execute_client.gearman.waitForServer()
    ...
    def haveAllBuildsReported(self):
        ...
        if self.execute_client.meta_jobs:
            return False
        ...
        for build in self.history:
            zbuild = self.execute_client.builds.get(build.uuid)
            if not zbuild:
        ...
        for connection in self.execute_server.worker.active_connections:
            if connection.state == 'GRAB_WAIT':
                return False
        return True

    def areAllBuildsWaiting(self):
        builds = self.execute_client.builds.values()
        for build in builds:
            client_job = None
            for conn in self.execute_client.gearman.active_connections:
            ...
            if build.url is None:
                ...
                return False
            worker_build = self.execute_server.job_builds.get(
                server_job.unique)
            if worker_build:
    ...
    def waitUntilSettled(self):
        ...
        while True:
            ...
            self.execute_server.lock.acquire()
            ...
            if self.haveAllBuildsReported():
                ...
                if (not self.merge_client.jobs and
                    ...
                    self.execute_server.lock.release()
