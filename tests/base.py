...
class ZuulTestCase(BaseTestCase):
    ...
    def assertEmptyBuildQueueHistory(self):
        ...
        try:
            ...
            for path, build in self.zk_builds.inState():
        ...
        for app in self.scheds.filter(matcher):
            ...
            for build in builds:
                ...
                if self.zk_builds.hold_in_queue:
                    if content['state'] == BuildState.REQUESTED.name:
                        self.log.debug("%s requested while holding in queue",
                                       build)
                        return False
                    elif content['state'] != BuildState.RUNNING.name:
                        continue
    ...
    def _areZookeeperEventQueuesEmpty(self, matcher=None) -> bool:
        for sched in map(lambda app: app.sched, self.scheds.filter(matcher)):
            ...
            for tenant_name in sched.abide.tenants:
                if sched.management_events[tenant_name].hasEvents():
                    return False
                if sched.trigger_events[tenant_name].hasEvents():
                    return False
        return True
