...
class MySQLSchemaFixture(fixtures.Fixture):
    def setUp(self):
        ...
        cur.execute("grant all on %s.* to 'openstack_citest'@'localhost'" %
                    (self.name))
        cur.execute("flush privileges")

        self.dburi = (
            'mysql+pymysql://openstack_citest:openstack_citest@localhost/%s' %
            (self.name))
...
class ZuulDBTestCase(ZuulTestCase):
    def setUp(self):
        ...
        self.useFixture(f)
        self.dburi = f.dburi
        super(ZuulDBTestCase, self).setUp()
        ...
        for section_name in self.config.sections():
            ...
            if self.config.get(section_name, 'driver') == 'sql':
                if (self.config.get(section_name, 'dburi') ==
                    '$MYSQL_FIXTURE_DBURI$'):
                    self.config.set(section_name, 'dburi', self.dburi)
