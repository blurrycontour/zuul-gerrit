...
class ZuulTestCase(BaseTestCase):
    ...
    def __lockAllGithubEventConnectors(self, matcher=None):
        for sched in map(lambda app: app.sched, self.scheds.filter(matcher)):
            for connection in list(sched.connections.connections.values()):
                if hasattr(connection, 'github_event_connector'):
                    connection.github_event_connector.lock_dispatcher.aquire()
                    connection.github_event_connector.lock_forwarder.aquire()

    def __unlockAllGithubEventConnectors(self, matcher=None):
        for sched in map(lambda app: app.sched, self.scheds.filter(matcher)):
            for connection in list(sched.connections.connections.values()):
                if hasattr(connection, 'github_event_connector'):
                    connection.github_event_connector.lock_forwarder.release()
                    connection.github_event_connector.lock_dispatcher.release()
