        ...
        while time.time() < (start + 5):
            for stat in self.statsd.stats:
                ...
                if key == k:
                    ...
                    # timing results into float with indeterminate
                    # 0-padding length, hence foiling straight string
                    # matching.
                    if value and kind == 'ms' and s_kind == 'ms':
                        self.assertEqual(float(value), float(s_value))
                        return True
