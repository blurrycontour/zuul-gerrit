    ...
    def copyDirToRepo(self, project, source_path):
        ...
        for (dirpath, dirnames, filenames) in os.walk(source_path):
            for filename in filenames:
                # source is 'tests/fixtures/config/in-repo/git/common-config'
                # dirpath is 'tests/fixtures/config/in-repo/git/common-config/foo'
                # filename is 'bar.yaml'
                absolute_filepath = os.path.join(dirpath, filename)
                # absolute is 'tests/fixtures/config/in-repo/git/common-config/foo/bar.yaml'
                common_path = os.path.commonprefix([absolute_filepath, source_path])
                # common is 'tests/fixtures/config/in-repo/git/common-config/foo'
                relative_filepath = absolute_filepath[len(common_path)+1:]
                # relative is 'foo/bar.yaml'
