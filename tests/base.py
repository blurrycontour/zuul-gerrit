...
class ZuulTestCase(BaseTestCase):
    ...
    def __areAllBuildsWaiting(self, matcher) -> bool:
        for app in self.scheds.filter(matcher):
            ...
            for build in builds:
                ...
                #     self.zk.pauseBuild(build.zookeeper_node)
                build_item = self.zk.getBuildItem(build.zookeeper_node)
                if self.zk.builds_hold_in_queue:
                ...
                if self.executor_server.hold_jobs_in_queue_sensor.pause\
                        and not self.zk.isBuildLocked(build.zookeeper_node)\
    ...
    def __releaseZookeeperLockingLocks(self, matcher=None):
        for sched in map(lambda app: app.sched, self.scheds.filter(matcher)):
            if sched.zk.lockingLock.locked():
                sched.zk.lockingLock.release()
