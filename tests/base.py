...
class FakeGithubEventConnector(githubconnection.GithubEventConnector):

    def __init__(self, connection):
        super().__init__(connection)
        self.lock_dispatcher = threading.Lock()
        self.lock_forwader = threading.Lock()

    def _run_event_dispatcher_inner(self):
        with self.lock_dispatcher:
            super()._run_event_dispatcher_inner()

    def _run_event_forwarder_inner(self):
        with self.lock_forwader:
            super()._run_event_forwarder_inner()


class FakeGithubConnection(githubconnection.GithubConnection):
    log = logging.getLogger("zuul.test.FakeGithubConnection")
    _event_connector_class = FakeGithubEventConnector
