...
from zuul.zk import ZooKeeperClient, connect_zookeeper
...
def iterate_timeout(max_seconds, purpose, sleep=0.01):
    start = time.time()
    count = 0
...
class FakeBuild(object):
    ...
    def __init__(self, executor_server: 'RecordingExecutorServer',
        ...
        self.uuid: str = build_item.content['uuid']
        self.parameters: Dict[str, Any] = build_item.content['params']
...
class ZuulTestCase(BaseTestCase):
    ...
    def assertEmptyBuildQueueHistory(self):
        nodes = []
        for zone in self.zk.kazoo_client.get_children(self.zk.builds.ROOT):
            nodes.extend(self.zk.kazoo_client.get_children(
                "%s/%s" % (self.zk.builds.ROOT, zone)))
        try:
            self.log.debug(
            ...
            for _ in iterate_timeout(
                ...
                nodes = []
                for zone in self.zk.kazoo_client.get_children(
                        self.zk.builds.ROOT):
                    nodes.extend(self.zk.kazoo_client.get_children(
                        "%s/%s" % (self.zk.builds.ROOT, zone)))
                if len(nodes) == 0:
            ...
            self.log.error("Remaining %s builds in Zookeeper:", nodes)
            for path, build in self.zk.builds.in_state('ALL'):
                self.log.error("  * %s: %s", path, build)
            for node in nodes:
                self.log.error("  - %s", node)
            raise e
    ...
    def __haveAllBuildsReported(self, matcher,
                                show_unreported_builds: bool = False) -> bool:
        for app in self.scheds.filter(matcher):
            ...
            if show_unreported_builds:
                self.log.debug("Unreported builds:")
                for build in self.history:
                    zbuild = executor_client.builds.get(build.uuid)
                    if zbuild:
                        self.log.debug("  - %s/%s", build, zbuild)
            for build in self.history:
                zbuild = executor_client.builds.get(build.uuid)
                if not zbuild:
        ...
        for app in self.scheds.filter(matcher):
            ...
            for build in builds:
                ...
                if build.url is None:
                    log.debug("%s has not reported start" % build)
                    return False
                ...
                if worker_build:
                    ...
                    if worker_build.isWaiting():
                        continue
                    log.debug("%s is running" % worker_build)
                    return False
                else:
                    log.debug("%s is unassigned", build)
                    return False
    ...
    def waitUntilSettled(self, msg="", matcher=None) -> None:
        ...
        while True:
            ...
            time.sleep(1.0)  # TODO JK: Busy loop - maybe starving?
