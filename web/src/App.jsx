...
class App extends React.Component {
        ...
        {this.menu.filter(item => item.title).map(item => (
          <li key={item.to} className={item === activeItem ? 'active' : ''}>
            <Link
              ...
              <Translate value={'app.' + item.title} />
            </Link>
          </li>
        ))}
      </ul>
  ...
  renderContent = () => {
    ...
    if (info.isFetching) {
      return (<h2><Translate value="app.isFetching" /></h2>)
    }
    this.menu
      ...
      <NotificationDrawer.Panel>
        <NotificationDrawer.PanelHeading>
          <NotificationDrawer.PanelTitle>
            <Translate value="app.configErrors" />
          </NotificationDrawer.PanelTitle>
          <NotificationDrawer.PanelCounter
            text={I18n.t('app.errorCount', {count: errors.length})} />
        </NotificationDrawer.PanelHeading>
        ...
        <Masthead
          ...
          <div className='collapse navbar-collapse'>
            ...
            <ul className='nav navbar-nav navbar-utility'>
              ...
              <li>
                <Link to='/openapi'><Translate value="app.api" /></Link>
              </li>
              <li>
                <a href='https://zuul-ci.org/docs'
                   rel='noopener noreferrer' target='_blank'>
                  <Translate value="app.Documentation" />
                </a>
              </li>
              {tenant.name && (
                <li>
                  <Link to={tenant.defaultRoute}>
                    <strong><Translate value="app.Tenant" /></strong> {tenant.name}
                  </Link>
                </li>
              )}
              <li>
