...
class Change extends React.Component {
  ...
  translateErrorReasons (reasons) {
    ...
    reasons.forEach(r => {
      switch (r) {
        case 'a needed change is failing':
          reason += I18n.t('statusContainer.change.failing_reasons.neededChangeFailing')
          break
        case 'it has a merge conflict':
          reason += I18n.t('statusContainer.change.failing_reasons.mergeConflict')
          break
        case 'it has an invalid configuration':
          reason += I18n.t('statusContainer.change.failing_reasons.invalidConfig')
          break
        case 'at least one job failed':
          reason += I18n.t('statusContainer.change.failing_reasons.oneJobFailed')
          break
        case 'it did not merge':
          reason += I18n.t('statusContainer.change.failing_reasons.didNotMerge')
          break
        case 'is a non-live item with no items behind':
          reason += I18n.t('statusContainer.change.failing_reasons.nonLive')
          break
        default:
          reason += r
      }
  ...
  renderStatusIcon (change) {
    ...
    let iconTitle = I18n.t('statusContainer.change.succeeding')
    if (change.active !== true) {
      iconGlyph = 'pficon pficon-pending'
      iconTitle = I18n.t('statusContainer.change.inactive')
    } else if (change.live !== true) {
      iconGlyph =  'pficon pficon-info'
      iconTitle = I18n.t('statusContainer.change.dependentChange')
    } else if (change.failing_reasons &&
               change.failing_reasons.length > 0) {
      let reason = change.failing_reasons.join(', ')
      iconTitle = I18n.t('statusContainer.change.failing') + this.translateErrorReasons(change.failing_reasons)
