...
class Pipeline extends React.Component {
  ...
  renderPipelineName() {
    ...
    let periodic_info = <></>
    let periodic_config = PIPELINE_ICONS['periodic']
    const PeriodicIcon = periodic_config.icon
    if (pipeline.triggers) {
      pipeline.triggers.forEach(trigger => {
        if (trigger.driver === 'timer') {
          periodic_info = <Tooltip
            position="bottom"
            content={<div><strong>{periodic_config.help_title}</strong><p>{periodic_config.help}</p></div>}
          >
            <PeriodicIcon />
          </Tooltip>
        }
