...
class ChangePanel extends React.Component {
  ...
  renderTimer (change) {
    ...
    if (change.remaining_time === null) {
      remainingTime = <Translate value='statusContainer.changePanel.unknown' />
    } else {
      remainingTime = this.time(change.remaining_time)
    }
    return (
      <React.Fragment>
        <small title={I18n.t('statusContainer.changePanel.remainingTime')} className='time'>
          {remainingTime}
        </small>
        <br />
        <small title={I18n.t('statusContainer.changePanel.elapsedTime')} className='time'>
  ...
  renderJobProgressBar (elapsedTime, remainingTime) {
    ...
    if (remaining !== null) {
      title = I18n.t('statusContainer.changePanel.estimatedTimeRemaining') + moment.duration(remaining).format({
  ...
  renderJobStatusLabel (result) {
    ...
    let _result = result === 'in progress' ? 'in_progress' : result
    return (
      <span className={'zuul-job-result label ' + className} title={result}>
        <Translate value={'statusContainer.changePanel.' + _result} />
      </span>
    )
  }
  ...
  renderJob (job) {
    ...
    let job_tries = <Translate value="statusContainer.changePanel.renderJob"
                               count={job.tries} />
    let name = ''
    if (job.result !== null) {
      name = <a className='zuul-job-name' href={job.report_url}>{job.name}{job.tries > 1 ? job_tries : ''}</a>
      ...
      if (job.url.match('stream/')) {
        ...
        name = <Link className='zuul-job-name' to={to}>{job.name}{job.tries > 1 ? job_tries : ''}</Link>
      } else {
        name = <a className='zuul-job-name' href={url}>{job.name}{job.tries > 1 ? job_tries : ''}</a>
      }
    } else {
      name = <span className='zuul-job-name'>{job.name}{job.tries > 1 ? job_tries : ''}</span>
    }
    let resultBar
    let result = this.jobStrResult(job)
    ...
    return (
      <span>
        ...
        {resultBar}
        {job.voting === false ? (
          <small className='zuul-non-voting-desc'> <Translate value='statusContainer.changePanel.nonvoting' /></small>) : ''}
