...
class ChangePanel extends React.Component {
  ...
  renderTimer (change) {
    ...
    if (change.remaining_time === null) {
      remainingTime = <Translate value='unknown' />
    } else {
      remainingTime = this.time(change.remaining_time)
    }
    return (
      <React.Fragment>
        <small title={I18n.t('Remaining Time')} className='time'>
          {remainingTime}
        </small>
        <br />
        <small title={I18n.t('Elapsed Time')} className='time'>
  ...
  renderJobProgressBar (elapsedTime, remainingTime) {
    ...
    if (remaining !== null) {
      title = I18n.t('Estimated time remaining: ') + moment.duration(remaining).format({
  ...
  renderJob (job) {
    ...
    let job_tries = <Translate value="jobTries"
                               count={job.tries} />
    let name = ''
    if (job.result !== null) {
    ...
    return (
      <span>
        ...
        {resultBar}
        {job.voting === false ? (
          <small className='zuul-non-voting-desc'> <Translate value='(non-voting)' /></small>) : ''}
