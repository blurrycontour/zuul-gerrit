      ...
      dequeue_ref(tenant.apiPrefix, projectName, pipeline.name, changeRef, user.token)
        .catch(error => {
          if (error.response) {
            ...
            var htmlError = parser.parseFromString(error.response.data, 'text/html')
            var error_description = htmlError.getElementsByTagName('p')[0].innerText
            this.props.dispatch(addDequeueError(error_description))
          } else {
