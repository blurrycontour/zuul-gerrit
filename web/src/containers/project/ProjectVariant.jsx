...
class ProjectVariant extends React.Component {
  ...
  render () {
    ...
    rows.push({label: <Translate value='Merge mode' />, value: variant.merge_mode})
    ...
    if (variant.templates.length > 0) {
      ...
      rows.push({label: <Translate value='Templates' />, value: templateList})
    }
    ...
    variant.pipelines.forEach(pipeline => {
      ...
      const jobList = (
        <React.Fragment>
          {pipeline.queue_name && (
            <p><strong><Translate value='Queue: ' /></strong> {pipeline.queue_name} </p>)}
          <ul className='list-group'>
      ...
      rows.push({label: <Translate value='pipelineJobs' pipeline={pipeline.name} />, value: jobList})
    })
