...
class JobsList extends React.Component {
  ...
  makeTreeView = (element, visited) => {
    ...
    return {
      ...
      'id': job.name,
      /* we use this to be able to search the description */
      '_zuul_description': job.description ? job.description : '',
  ...
  filterResults = (value) => {
    ...
    const filtered =
          allItems.map(
            opt => Object.assign({}, opt)).filter(item => this.filterItems(item, value))
  ...
  /*
   * This is a bit of a hack for really big lists like openstack.  We
   * actually do the filtering in a callback after a 1 second pause.
   * If another keystroke comes in before that time, we reset the
   * timer.  e.g. this prevents "devstack" first filtering almost
   * every job with "d", then "de", etc. until it actually gets to a
   * managable subset.
   */
  onSearch = value => {
    const { allItems, searchTimer } = this.state
    /* Value cleared; do this immediately */
    if (value === '') {
      ...
      const timer = setTimeout(() => this.filterResults(value), 1000)
      this.setState({searchValue: value, searchTimer: timer})
    }
  }
  ...
  filterItems = (item, input) => {
    ...
    if (item._zuul_description.toLowerCase().includes(input.toLowerCase())) {
      return true
    }
