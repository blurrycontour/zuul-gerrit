...
class JobsList extends React.Component {
  ...
  constructor(props) {
    ...
    /* Now we merge jobs to group their branches for display.
   ...
   *
   *  child.job -> poitner to the job defintion
...
//                    " -- it is now " + mergedChildren[child.job.name].branches.join(','))
  ...
  makeTreeView = element => {
    var branches = element.branches
    var job = element.job
    var children = []
    if (job.children.length > 0) {
      //console.log(job.name + " has " + job.children.length)
      job.children.forEach(child => {
        children.push(this.makeTreeView(child))
      })
    }

    return {
      'name': (<React.Fragment>
                 <Link to={this.props.tenant.linkPrefix + '/job/' + job.name}>{job.name}</Link>
  ...
  render () {
    ...
    return (
      <React.Fragment>
        <Title headingLevel="h2">Job definitions for {tenant.name}</Title>
        <PageSection>
          <TreeView onSearch={this.onSearch}  hasBadges data={filteredItems} allExpanded={isFiltered} />
        </PageSection>
      </React.Fragment>
    )
  }
