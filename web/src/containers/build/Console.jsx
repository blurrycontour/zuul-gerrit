    ...
    return (
      <div key='results'>
        {interesting_results.length>0 &&
         <React.Fragment>
           <h5 key='results-header'><Translate value='Results' /></h5>
...
class HostTask extends React.Component {
  ...
  render () {
    ...
    if (this.state.failed) {
      ai.push(
        <ListView.InfoItem key="failed" title={I18n.t('Click for details')}>
          <span className="task-failed" onClick={this.open}><Translate value='ansibleTasksStatus.FAILED' /></span>
        </ListView.InfoItem>)
    } else if (this.state.changed) {
      ai.push(
        <ListView.InfoItem key="changed" title={I18n.t('Click for details')}>
          <span className="task-changed" onClick={this.open}><Translate value='ansibleTasksStatus.CHANGED' /></span>
        </ListView.InfoItem>)
    } else if (this.state.skipped) {
      ai.push(
        <ListView.InfoItem key="skipped" title={I18n.t('Click for details')}>
          <span className="task-skipped" onClick={this.open}><Translate value='ansibleTasksStatus.SKIPPED' /></span>
        </ListView.InfoItem>)
    } else if (this.state.ok) {
      ai.push(
        <ListView.InfoItem key="ok" title={I18n.t('Click for details')}>
          <span className="task-ok" onClick={this.open}><Translate value='ansibleTasksStatus.OK' /></span>
        </ListView.InfoItem>)
    }
    ai.push(
            ...
            <Modal.Title>{hostname}
              <span className="zuul-console-modal-header-link">
                <a href={'#'+makeTaskPath(taskPath)}>
                  <Icon type="fa" name="link" title={I18n.t('Permalink')} />
                </a>
              </span>
            </Modal.Title>
          </Modal.Header>
...
class PlayBook extends React.Component {
  ...
  render () {
    ...
    if (playbook.trusted) {
      ai.push(
        <ListView.InfoItem key="trusted" title={I18n.t('trustedContext')}>
          <span className="additionalinfo-icon">
            <Icon type='pf' name='info' /> <Translate value='Trusted' />
          </span>
        </ListView.InfoItem>
      )
    }
      ...
      >
          {playbook.plays.map((play, idx) => (
            <React.Fragment key={idx}>
              <Row key='play'>
                <Col sm={12}>
                  <strong><Translate value='play' playname={play.play.name} /></strong>
                </Col>
              </Row>
