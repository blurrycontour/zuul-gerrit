...
class HostTask extends React.Component {
  ...
  render () {
    ...
    // alignment.  This simply pads out lines without "interesting"
    // results so they line up evenly.
    ...
    if (playbook.trusted) {
      dataListCells.push(
        <DataListCell key='trust'>
          ...
          <Label color='blue' icon={<InfoCircleIcon />} style={{cursor: 'pointer'}}>Trusted</Label></Popover></DataListCell>)
    } else {
      dataListCells.push(
        <DataListCell key='trust'>
          <Popover bodyContent={<div>This playbook runs in an untrusted execution context.</div>}>
          <Label color='grey' icon={<InfoCircleIcon />} style={{cursor: 'pointer'}}>Untrusted</Label></Popover></DataListCell>)
    }
