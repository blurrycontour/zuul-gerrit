...
class HostTask extends React.Component {
  ...
  constructor (props) {
    ...
    if (makeTaskPath(taskPath) === makeTaskPath(displayPath.slice(0, taskPath.length))) {
...
class PlayBook extends React.Component {
  ...
  render () {
    ...
    const expand = (expandAll ||
                    ...
                    makeTaskPath(taskPath) === makeTaskPath(displayPath.slice(0, taskPath.length)))
              ...
              {play.tasks.map((task, idx2) => (
                Object.entries(task.hosts).map(([hostname, host]) => (
                  <Row key={idx2+hostname}>
                    <Col sm={12}>
                      <HostTask hostname={hostname} taskPath={taskPath.concat([idx, idx2, hostname])}
      ...
      <React.Fragment>
        <ListView key="playbooks" className="zuul-console">
          {output.map((playbook, idx) => (
            <PlayBook key={idx} playbook={playbook} taskPath={[idx]}
