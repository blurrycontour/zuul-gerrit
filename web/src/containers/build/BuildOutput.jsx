      ...
      <ListView>
        {Object.entries(hosts).map(([host, values]) => (
          <ListView.Item
            ...
            heading={host}
            additionalInfo={[
              <ListView.InfoItem key="ok" title={I18n.t('buildContainer.buildOutput.taskOK')}>
                <Icon type='pf' name='ok' />
                <strong>{values.ok}</strong>
              </ListView.InfoItem>,
              <ListView.InfoItem key="changed" title={I18n.t('buildContainer.buildOutput.taskChanged')}>
                <Icon type='pf' name='on-running' color='#a28301'/>
                <strong>{values.changed}</strong>
              </ListView.InfoItem>,
              <ListView.InfoItem key="fail" title={I18n.t('buildContainer.buildOutput.taskFailure')}>
