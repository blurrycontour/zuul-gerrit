...
class BuildsPage extends TableFilters {
  ...
  autohold (rowdata) {
      ...
      autohold(this.props.token, this.props.tenant.apiPrefix, projectName, job).then(response => {
  ...
  prepareTableHeaders() {
    ...
    const linkBuildFormat = (value, rowdata) => (
      <Table.Cell>
        ...
        <Icon type='pf' name='locked' title='Hold on next failure' onClick={e => this.autohold(rowdata)} /> : <br />
        }
      </Table.Cell>
    )
...
export default connect(state => ({
    ...
    admin_tenants: state.auth.tenants}))(BuildsPage)
