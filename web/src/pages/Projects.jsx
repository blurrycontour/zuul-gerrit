...
class ProjectsPage extends React.PureComponent {
  ...
  cellFormat = (field, project) => {
    if (field === 'name') {
      return (<Link to={this.props.tenant.linkPreifx + '/project/' + project.canonical_name}>
                {project.name}
              </Link>)
      ...
      if (project.type === 'untrusted') {
        ...
        return (<Label color='blue'>trusted</Label>)
      } else {
      ...
      <React.Fragment>
        <PageSection variant={PageSectionVariants.light}>
          ...
          <SearchInput
            ...
            attributes = {[{attr:'name', display:'Name'},
                           {attr:'connection_name', display:'Connection Type'},
                ...
                <Tr key={rowIdx}>
                  {this.props.table_columns.map((colName, cellIdx) => (
                    <Td key={`${rowIdx}_${cellIdx}`}
                        dataLabel={this.props.table_columns_descriptions[cellIdx]}>
...
function selectProjects(state) {
  ...
  return projects.filter(function (project) {
    let show = false
    if (filterTerms.name) {
      show = project.name.includes(filterTerms.name)
    }
    if (filterTerms.haswords) {
      show = project.name.includes(filterTerms.haswords)
    }
    if (filterTerms.connection_name) {
      show = project.connection_name.includes(filterTerms.connection_name)
    }
    if (filterTerms.type) {
      show = project.type.includes(filterTerms.type)
    }
    return show
  })
}
