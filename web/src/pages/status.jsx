...
import {
  Alert, Badge, Checkbox, Icon, Form, FormGroup, FormControl
    ...
    if (job.result !== null) {
      ...
      if (job.url.match('stream.html')) {
        ...
        if (state.info.tenant) {
          ...
          to = '/t/' + state.tenant + '/stream/'
        }
        to += buildUuid + "?logfile=console.log"
...
class StatusPage extends React.Component {
  ...
  visibilityListener = () => {
    if (document[this.visibilityState] === 'visible') {
      this.visible = true
      this.updateData()
    } else {
      this.visible = false
  ...
  constructor () {
    ...
    document.addEventListener(
      this.visibilityChange, this.visibilityListener, false)
  }
  ...
  componentWillUnmount () {
    ...
    document.removeEventListener(
      this.visibilityChange, this.visibilityListener)
  }
  ...
  render () {
    const { error, status, filter, expanded } = this.state
    if (error) {
