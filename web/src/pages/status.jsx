  ...
  render() {
    ...
    const header = (
      <div className="panel panel-default zuul-change" onClick={this.onClick}>
        ...
        {expanded ? this.renderJobList(change.jobs) : ''}
      </div>
    )
    return (
      <React.Fragment>
...
class Change extends React.Component {
  renderStatusIcon(change) {
    ...
    return (
      <img alt="Status"
           src={iconFile} title={iconTitle} style={{display: 'block'}} />
    )
  }
  ...
  render() {
    ...
    for (i = 0; i < queue._tree_columns; i++) {
      ...
      if (i < change._tree.length && change._tree[i] !== null) {
        cellStyle.backgroundImage = "url(" + LineImage + ")"
        cellStyle.backgroundRepead = "repeat-y"
      }
      row.push(
  ...
  render() {
    ...
    queue.heads.forEach(changes => {
      changes.forEach((change, idx) => {
        changesList.push(<Change key={idx} change={change} queue={queue} />)
      })
    })
    return (
...
class PipelineTree extends React.Component {
  createTree(pipeline) {
    ...
    pipeline.change_queues.forEach((changeQueue, changeQueueIndex) => {
      ...
      changeQueue.heads.forEach((head, headIndex) => {
      ...
      changeQueue.heads.forEach((head, headIndex) => {
        head.forEach((change, changeIndex) => {
          ...
          change.items_behind.forEach((id, i) => {
            tree.push(id)
...
class StatusPage extends React.Component {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        {pipelines.map(item => (
          <PipelineTree pipeline={item} key={item.name}/>
        ))}
      </React.Fragment>)
  }
}
