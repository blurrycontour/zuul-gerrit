...
class StatusPage extends Refreshable {
  ...
  componentDidMount () {
    document.title = I18n.t('Zuul Status')
    this.loadState()
  ...
  renderStatusFooter (status) {
    return (
      <React.Fragment>
        <p><Translate value='Zuul version: ' /><span>{status.zuul_version}</span></p>
        {status.last_reconfigured ? (
          <p><Translate value='Last reconfigured: ' /><span>
    ...
    const statusControl = (
      <Form inline>
        <FormGroup controlId='status'>
          <FormControl
            type='text'
            placeholder={I18n.t('change or project name')}
            defaultValue={filter}
          ...
          <FormControl.Feedback>
            <span
              ...
              >
              <Icon type='pf' title={I18n.t('Clear filter')} name='delete' />
              &nbsp;
            </span>
          </FormControl.Feedback>
            )}
        ...
        <FormGroup controlId='status'>
          &nbsp;<Translate value='Expand by default' />&nbsp;
          <Checkbox
