...
class StatusPage extends Refreshable {
  ...
  componentDidMount () {
    document.title = I18n.t("statusPage.title")
    this.loadState()
  ...
  renderStatusHeader (status) {
    ...
    return (
      <p>
        <Translate value="statusPage.queueLengths" />
        <span>{teq_len}</span><Translate value="statusPage.events"
                   count= {teq_len} />,&nbsp;
        <span>{meq_len}</span><Translate value="statusPage.management_events"
                   count= {meq_len} />,&nbsp;
        <span>{req_len}</span><Translate value="statusPage.results"
                   count= {req_len} />.
      </p>
    )
  }
  ...
  renderStatusFooter (status) {
    return (
      <React.Fragment>
        <p><Translate value="statusPage.zuul_version" /><span>{status.zuul_version}</span></p>
        {status.last_reconfigured ? (
          <p><Translate value="statusPage.last_reconfigured" /><span>
    ...
    const statusControl = (
      <Form inline>
        <FormGroup controlId='status'>
          <FormControl
            type='text'
            placeholder={I18n.t('statusPage.formPlaceholder')}
            defaultValue={filter}
          ...
          <FormControl.Feedback>
            <span
              ...
              >
              <Icon type='pf' title={I18n.t('statusPage.clearFilter')} name='delete' />
              &nbsp;
            </span>
          </FormControl.Feedback>
            )}
        ...
        <FormGroup controlId='status'>
          &nbsp;<Translate value="statusPage.expandByDefault" />&nbsp;
          <Checkbox
      ...
      <React.Fragment>
        <div className="pull-right" style={{display: 'flex'}}>
          ...
          <Checkbox
            ...
            style={{marginTop: '0px'}}>
            <Translate value="statusPage.autoReload" />
          </Checkbox>
        </div>
