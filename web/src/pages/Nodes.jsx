...
class NodesPage extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      building: nodes.filter(n => n.state === "building").length,
      ready: nodes.filter(n => n.state === "ready").length,
      inUse: nodes.filter(n => n.state === "in-use").length,
      deleting: nodes.filter(n => n.state === "deleting").length,
      fetching: false,
    }
  ...
  render() {
    const {sortBy, rows, columns, building, ready, inUse, deleting} = this.state
