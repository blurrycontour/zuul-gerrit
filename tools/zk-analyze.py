...
class SummaryLine:
    ...
    def matchesLimit(self, limit):
        if not limit:
            return True
        if self.size >= limit:
            return True
        for child in self.children:
            if child.matchesLimit(limit):
                return True
        return False
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', help='Filesystem path for previously dumped data')
    parser.add_argument('--host', help='ZK host string (exclusive with --path)')
