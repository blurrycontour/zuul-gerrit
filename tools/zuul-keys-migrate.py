...
DESCRIPTION = """Encrypt a Zuul secret based on the Zookeeper
                 exported keys and Zuul.conf Keystore
"""
...
def serialize_zk_dump(keyfile, source_password_bytes, dest_password_bytes) -> dict:
    ...
    for key in keys:
        ...
        for encryptedKey in encryptedKeysList:
            ...
            private_key, public_key = encryption.deserialize_rsa_keypair(
                pem_private_key, source_password_bytes)
            encrypted_private_key = encryption.serialize_rsa_private_key(
                private_key, dest_password_bytes)
