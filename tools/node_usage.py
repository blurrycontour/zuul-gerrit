...
class LogScraper(object):
    ...
    r = re.compile(r'\d+-\d+-\d+ \d\d:\d\d:\d\d,\d\d\d INFO zuul.nodepool: Nodeset <.*> with (?P<nodes>\d+) nodes was in use for (?P<secs>\d+(.[\d\-e]+)?) seconds for build <Build \w+ of (?P<job>[^\s]+) voting:\w+ on .* for project (?P<repos>[^\s]+)')  # noqa
    ...
    def calculate_project_usage(self):
        if not os.path.exists('governance/reference/projects.yaml'):
            return self.sorted_projects
        with open('governance/reference/projects.yaml') as f:
            y = yaml.load(f)
...
print('Top 20 OpenStack projects by resource usage:')
