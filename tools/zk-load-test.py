...
# Whether or not to use persistent recursive watches.  Requires
# unmerged kazoo client support.
PR_WATCHES = False
...
class ZooKeeperStats:
    ...
    def _run(self):
        ...
        if self.interval_count:
            ...
            self.rate_frames.append((read_ops, write_ops, client_ops, delta))

            total_read_ops = sum([x[0] for x in self.rate_frames])
            total_write_ops = sum([x[1] for x in self.rate_frames])
            total_client_ops = sum([x[2] for x in self.rate_frames])
            total_delta = sum([x[3] for x in self.rate_frames])
            read_rate = round(total_read_ops / total_delta)
            write_rate = round(total_write_ops / total_delta)
            client_rate = round(total_client_ops / total_delta)
            self.log.info(
                f'Read {read_rate}, write {write_rate}, '
                f'client {client_rate} ops/sec')
...
class QueueSimulator:
    ...
    def establish_watches_persistent_recursive(self):
        self.client.add_watch(f'{self.path}/items',
                              self._persistent_recursive_listener,
                              kazoo.protocol.states.AddWatchMode.PERSISTENT_RECURSIVE)

    def _persistent_recursive_listener(self, event, path=None):
    ...
    def _watch_children(self, event):
        ...
        self.log.debug(f'Watch new children {len(new_children)} {event}')
    ...
    def watcher(self):
        ...
        if PR_WATCHES:
            self.establish_watches_persistent_recursive()
        else:
...
class PipelineSimulator:
    ...
    def timing_thread(self):
        while True:
            time.sleep(1)
            MON_QUEUE.put(self.read_ops)
            self.read_ops = 0
            MON_QUEUE.put(self.write_ops)
            self.write_ops = 0
