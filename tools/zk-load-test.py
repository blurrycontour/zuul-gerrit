...
ZK_CONNECTION_STRING = 'localhost:2181,localhost:2182,localhost:2183'
...
def get_client():
    client = kazoo.client.KazooClient(ZK_CONNECTION_STRING)
    client.start()
    return client
...
class ZooKeeperStats:
    ...
    def __init__(self):
        ...
        client_cert = None
        client_key = None
        self.socket = Socket(STATS_HOST, STATS_PORT,
...
class QueueSimulator:
    ...
    def check_timing(self, thread):
        ...
        if delta >= 10:
            ...
            self.log.info(f'Queue {self.name} {thread} ops/sec {rate}')
    ...
    def check_timing(self, thread, count=None):
        ...
        if delta >= 10:
            ...
            if count is None:
                ...
                count = f' count {count}'
            self.log.info(f'Cache {self.name} {thread} ops/sec {rate}{count}')
            self.chunk_start = now
            self.chunk_ops = 0
...
class PipelineSimulator:
    ...
    def check_timing(self):
        ...
        if delta >= 10:
            ...
            self.log.info(f'Pipeline {self.name} read ops/sec {read_rate}')
            self.log.info(f'Pipeline {self.name} write ops/sec {write_rate}')
            self.chunk_start = now
            self.read_ops = 0
            self.write_ops = 0
