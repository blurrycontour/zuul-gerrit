...
class ZooKeeper(object):
    ...
    def getLayoutVersion(self, tenant: Tenant) -> str:
        """
    ...
    def setLayoutVersion(self, tenant: Tenant, hash: str):
        lock_node = self._getZuulNodePath('layout', tenant.name)
    ...
    def loadConfig(self, project: str, branch: str, path: str,
                   file: str, use_lock: bool=True) -> Optional[str]:
        """
        ...
        if lock is not None:
            lock.acquire()
        try:
            node = self._getZuulNodePath('config', project, branch, path, file)
            content = "".join(
    ...
    def saveConfig(self, project: str, branch: str, path: str,
        ...
        """
        current = self.loadConfig(project, branch, path, file, use_lock=False)
