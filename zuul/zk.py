...
class ZooKeeper(object):
    ...
    LAUNCHER_ROOT = Path("/nodepool/launchers")

    def _bytesToDict(self, data):
        return json.loads(data.decode('utf8'))

    def _launcherPath(self, launcher):
        return self.LAUNCHER_ROOT / launcher

    def _nodePath(self, node):
        return self.NODE_ROOT / node
    ...
    def getRegisteredLaunchers(self):
        ...
        '''
        try:
            launcher_ids = self.client.get_children(str(self.LAUNCHER_ROOT))
        except kze.NoNodeError:
            return []
    ...
    def getNodes(self):
        ...
        '''
        try:
            return self.client.get_children(str(self.NODE_ROOT))
        except kze.NoNodeError:
            return []
