...
class LookupModule(password.LookupModule):
    ...
    def run(self, terms, variables, **kwargs):
        for term in terms:
            # term parsing taken from a loose reinterpretation of
            # ansible.lib.ansible.plugins.lookup.password._parse_parameters(),
            # with /dev/null whitelisted because it's interpreted specially
            relpath = term.split(' ')[0]
            if relpath != "/dev/null":
