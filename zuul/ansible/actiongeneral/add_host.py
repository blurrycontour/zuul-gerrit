...
from zuul.ansible.library.zuul_return import set_value
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        update_inventory = self._task.args.get('update_inventory', False)
        ...
        if update_inventory:
            path = os.path.join(
                os.environ['ZUUL_JOBDIR'], 'work', 'results.json')
            set_value(path, {
                'zuul': {
                    'inventory': {
                        str(new_name): dict(map(
