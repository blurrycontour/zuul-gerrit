...
class StreamFollower:
    ...
    def follow(self):
        self.thread = threading.Thread(target=self.follow_main)
            ...
            while True:
                ...
                for fd in rready:
                    ...
                    if not line:
                        ...
                        continue
                    log_size += len(line)
                    if log_size > self.output_max_bytes:
                        msg = (
                            '[Zuul] Log output exceeded max of '
                            f'{self.output_max_bytes}, terminating\n')
                        console.addLine(msg)
                        try:
                            ...
                            pass
                        raise Exception(msg)
                    if fd == self.cmd.stdout:
                        self.log_bytes.write(line)
                    else:
                        self.stderr_log_bytes.write(line)
                    if not line[-1] != b'\n':
                        line += b'\n'
