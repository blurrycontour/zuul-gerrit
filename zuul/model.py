...
class Pipeline(object):
    ...
    def findJobsToRun(self, item, mutex):
        ...
        # Sort the jobs before acquiring any mutex
        sorted_jobs_not_started = sorted(
            jobs_not_started, lambda a, b: cmp(a.name, b.name))
        for job in sorted_jobs_not_started:
...
class JobGraph(object):
    ...
    def __init__(self):
        ...
        self._ordered_jobs = []  # Unique Jobs in the order addJob was called

    def addJob(self, job, parent_job=None):
        # parent_job=None means no dependency
        if job.name not in self.jobs:
            self._ordered_jobs.append(job)
    ...
    def getJobs(self):
        return self._ordered_jobs  # Report in the order of the layout config
