    ...
    def addJob(self, job):
        ...
        try:
            ...
            for dependency in job.dependencies:
                ...
                ancestor_jobs = self._getParentJobNamesRecursively(dependency, soft=True)
                ancestor_jobs.add(dependency)
                if any((job.name == anc_job) for anc_job in ancestor_jobs):
                    raise Exception("Dependency cycle detected in job %s" % (job.name,))
...
class QueueItem(object):
    ...
    def findJobsToRun(self, mutex):
        ...
        if self.item_ahead:
            ...
            if self.isHoldingFollowingChanges(self.item_ahead):
                return []
    ...
    def setResult(self, build):
        if build.retry:
            ...
            for job in self.job_graph.getDependentJobsRecursively(build.job.name):
                # TODOv3(jeblair): evaluate this from the original graph change:
