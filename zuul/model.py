...
class ProviderNode(zkobject.LockableZKObject, metaclass=abc.ABCMeta):
    ...
    def _fromRaw(cls, raw_data, zstat):
        ...
        provider_id = data["_provider_id"]
        try:
            klass = cls._node_subclasses[provider_id]
        except KeyError:
            raise RuntimeError(f"Unknown node provider id: {provider_id}")
        obj = klass()
        obj._set(_zkobject_hash=None)
        # We pass the data dict to 'deserialize()' here, so the
        # JSON loading is skipped.
