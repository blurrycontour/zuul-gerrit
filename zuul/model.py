...
class PipelineState(zkobject.ZKObject):
    ...
    @classmethod
    def updateOrCreate(cls, pipeline):
...
class ChangeQueue(zkobject.ZKObject):
    ...
    def serialize(self):
        data = {
            ...
            "queue": [i.uuid for i in self.queue],
            "window": self.window,
    ...
    def deserialize(self, raw):
        ...
        queue = []
        for item_uuid in data["queue"]:
            item = existing_items.get(item_uuid)
            if item:
                item.refresh(context)
            else:
                item = QueueItem.fromZK(context, QueueItem.itemPath(
                    self.pipeline.state.getPath(), item_uuid
                ))
            queue.append(item)
    ...
    def getPath(self):
        ...
        queue_id = (self.uuid if self.dynamic or not self.name
                    else urllib.parse.quote_plus(self.name))
        layout_uuid = self.pipeline.tenant.layout.uuid
        return f"{pipeline_path}/queue/{layout_uuid}/{queue_id}"
    ...
    def matches(self, project, branch):
        return (project.canonical_name, branch) in self.project_branches
