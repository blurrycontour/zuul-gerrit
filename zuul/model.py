...
class PipelineState(zkobject.ZKObject):
    ...
    def _resurrectBuild(self, context, build_path, build_map, item_map):
        ...
        for orig_build in orig_item.current_build_set.builds.values():
            if (not isinstance(orig_build, BuildReference) and
                orig_build.getPath() == build_path):
                build_map[build_path] = orig_build
                return orig_build
        for build_list in orig_item.current_build_set.retry_builds.values():
            for orig_build in build_list:
                if (not isinstance(orig_build, BuildReference) and
                    orig_build.getPath() == build_path):
                    build_map[build_path] = orig_build
                    return orig_build
