            ...
            for job_name in data['job_graph'].jobs:
                ...
                if job_name in self.jobs:
                    ...
                    if not old_build_exists:
                        tpe_jobs.append((None, None, context.executor.submit(
                            job.refresh, context)))
                else:
                    job_path = FrozenJob.jobPath(job_name, self.getPath())
                    tpe_jobs.append(('job', None, context.executor.submit(
                ...
                if build_path:
                    ...
                    if build and build.getPath() == build_path:
                        if not build.result:
                            tpe_jobs.append((
                                None, job_name, context.executor.submit(
                        ...
                        if not self._isMyBuild(build_path):
                            ...
                            tpe_jobs.append((
                                'build', job_name, context.executor.submit(
                ...
                for retry_path in data["retry_builds"].get(job_name, []):
                    ...
                    if retry_build and retry_build.getPath() == retry_path:
                        ...
                        if not self._isMyBuild(retry_path):
                            ...
                            tpe_jobs.append((
                                'retry', job_name, context.executor.submit(
