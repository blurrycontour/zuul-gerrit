...
class Job(ConfigObject):
    ...
    def setIrrelevantFileMatcher(self, irrelevant_files):
        ...
        for fn in irrelevant_files:
            self.irrelevant_file_matcher_list.append(change_matcher.FileMatcher(ZuulRegex(fn)))
    ...
    def changeMatchesFilesSelf(self, change, irrelevantMatcherList):
        ...
        if len(irrelevantMatcherList) > 0:
            ...
            irrelevantMatcher = change_matcher.MatchAllFiles(irrelevantMatcherList)
    ...
    def changeMatchesFiles(self, change, irrelevantMatcherList=None):
        ...
        matchesSelf = self.changeMatchesFilesSelf(change, irrelevantMatcherList)

        if not self.inherit_files:
            return matchesSelf

        # We want to explicitly evaluate the files and irrelevant-files of the parent
        if not matchesSelf and not self.isBase():
            return self.parent.changeMatchesFiles(change, irrelevantMatcherList)
