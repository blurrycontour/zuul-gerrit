...
class Abide(object):
    ...
    def hasUsefulUnparsedBranchCache(self, canonical_project_name, branch, tenant=None):
        project_branch_cache = self.unparsed_project_branch_cache.setdefault(
            canonical_project_name, {})
        cache = project_branch_cache.get(branch)
        if cache is None:
        ...
        if tenant is not None:
            ...
            tpc = None
            canonical_project_name, branch, cache.isUsefulConf(tpc) if cache else 'VOID'))
