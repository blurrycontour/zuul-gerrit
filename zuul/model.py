...
class SourceContext(ConfigObject):
    ...
    def toExecutorDict(self):
        ...
        # TODO: Standardize on this and migrate rest api
        return dict(
            project=dict(
                connection=self.project_connection_name,
...
class VariableValue:
    ...
    def flattenSources(self):
        if VariableValue._isDict(self.value):
            ret = {}
...
class Job(ConfigObject):
    ...
    def _deepUpdate(a, b):
        ...
        if isinstance(a, VariableValue):
            ret = VariableValue(ret, a.source)
        elif isinstance(b, VariableValue):
            ret = VariableValue(ret, b.source)
        for k, av in a.items():
            if k not in b:
                ret[k] = av
        for k, bv in b.items():
