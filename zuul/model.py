    ...
    def deserialize(self, raw, context):
        ...
        if data['job_graph']:
            for job_name in data['job_graph'].jobs:
                # If we have a current build result before refreshing,
                # we may be able to skip refreshing some items since
                # they will not have changed.
                old_build = self.builds.get(job_name)
                old_build_result = (old_build and old_build.result)

                if job_name in self.jobs:
                    job = self.jobs[job_name]
                    if not old_build_result:
                        job.refresh(context)
                else:
