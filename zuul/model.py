...
class ChangeQueue(zkobject.ZKObject):
    ...
    def getPath(self):
        ...
        return f"{pipeline_path}/queue/{layout_uuid}/{self.uuid}"
...
class FrozenJob(object):
    ...
    @classmethod
    def deserialize(cls, data):
        # TODO (felix): Use the shortcut from Simon
        job = cls(data["name"])

        job.voting = data["voting"]
        job.hold_following_changes = data["hold_following_changes"]
        job.failure_message = data["failure_message"]
        job.success_message = data["success_message"]
        job.timeout = data["timeout"]
        job.post_timeout = data["post_timeout"]
        job.variables = data["variables"]
        job.extra_variables = data["extra_variables"]
        job.host_variables = data["host_variables"]
        job.group_variables = data["group_variables"]
        job.nodeset = NodeSet.fromDict(data["nodeset"])
        job.ansible_version = data["ansible_version"]
        job.attempts = data["attempts"]
        job.override_branch = data["override_branch"]
        job.override_checkout = data["override_checkout"]
        job.workspace_scheme = data["workspace_scheme"]
        job.inheritance_path = data["inheritance_path"]
        job.parent_data = data["parent_data"]
        job.secret_parent_data = data["secret_parent_data"]
        job.artifact_data = data["artifact_data"]
        job.queued = data["queued"]
        job.waiting_status = data["waiting_status"]
...
class JobGraph(zkobject.ZKObject):
    ...
    def getPath(self):
        ...
        return f"/zuul/jobgraphs/{self.uuid}"

    def serialize(self):
        data = {
            "uuid": self.uuid,
            # TOOD (felix): toDict -> serialize
            "jobs": {
...
class QueueItem(zkobject.ZKObject):
    ...
    def deserialize(self, raw, context):
        ...
        # TODO (felix): Disable the if block in the tests to see if the
