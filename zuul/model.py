...
class UnparsedTenantConfig(object):
    ...
    def extend(self, conf, tenant=None, canonicalize=False):
        if isinstance(conf, UnparsedTenantConfig):
            ...
            for k, v in conf.projects.items():
                name = k
                # If canonicalize is requested we need to add the projects to
                ...
                if tenant is not None and canonicalize:
        ...
        for item in conf:
            ...
            key, value = list(item.items())[0]
            if key == 'project':
                # Put regex projects into a separate list. This makes it
                # possible to easily expand them later without filtering the
                # projects again.
                name = value.get('name')
                if not name.startswith('^'):
                    self.projects.setdefault(name, []).append(value)
                else:
                    self.projects_by_regex.setdefault(name, []).append(value)
            elif key == 'job':
