...
class FrozenJob(zkobject.ZKObject):
    ...
    def deserialize(self, raw, context):
        ...
        data['change'] = None
        if hasattr(self, 'buildset'):
            manager = self.buildset.item.pipeline.manager
            change = manager.resolveChangeReferences([data["ref"]])[0]
            if change is None:
                raise Exception("Unable to find change object for %s",
                                data['ref'])
            data['change'] = change
        return data
            ...
            for other_job in job_list[:]:
                ...
                if job.deduplicate == 'auto':
                    ...
                    if (not job.required_projects and
                        ...
                        continue
                # Deduplicate!  If a to-be-deduplicated job depends on
                # a non-deduplicated job, it will treat each (job,
                # ref) instance as a parent.
...
class QueueItem(zkobject.ZKObject):
    ...
    def new(klass, context, **kw):
        ...
        should_merge = any([isinstance(o, (Change, Tag))
                            for o in obj.changes])
        merge_state = (BuildSet.NEW if should_merge
                       else BuildSet.COMPLETE)
        should_files = any([o.files is None
                            for o in obj.changes])
        files_state = (BuildSet.NEW if should_files
                       else BuildSet.COMPLETE)
    ...
    def areAllChangesMerged(self):
        for change in self.changes:
            if not (hasattr(change, 'is_merged') and
                    change.is_merged):
                return False
        return True
