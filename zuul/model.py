...
class Job(ConfigObject):
    ...
    log = logging.getLogger("zuul.Job")
    ...
    def updateParentData(self, other_build):
        ...
        self.log.debug("Update parent data of %s with other vars of %s: %s",
                       self.name, other_build.job.name, str(other_vars))
        ...
        if 'zuul' in v:
            del v['zuul']
        self.log.debug("Update parent data of %s:\n From: %s\n   To: %s",
                       self.name, str(self.parent_data), str(v))
        self.parent_data = v
        vx = Job._deepUpdate(self.parent_data, self.variables)
        self.log.debug("Update variables of %s: \n    %s\n  + %s\n => %s",
                       self.name, str(self.parent_data), str(self.variables),
                       str(vx))
        self.variables = vx
...
class JobGraph(object):
    ...
    def addJob(self, job):
        ...
        self.jobs[job.name] = job
        self.log.debug("Adding Job: %s", job.name)
    ...
    def getParentJobsRecursively(self, dependent_job, layout=None,
        ...
        return [self.jobs[name] for name in
                self._getParentJobNamesRecursively(dependent_job, layout=layout,
                                                   skip_soft=skip_soft)]
...
class QueueItem(object):
    ...
    def findJobsToRun(self, semaphore_handler):
        torun = []
        if not self.live:
            self.log.debug("Found jobs to run: NONE (not live)")
            return []
        if not self.job_graph:
            self.log.debug("Found jobs to run: NONE (empty graph)")
            return []
        if self.item_ahead:
            # Only run jobs if any 'hold' jobs on the change ahead
            # have completed successfully.
            if self.item_ahead.isHoldingFollowingChanges():
                self.log.debug("Found jobs to run: NONE (XXX)")
                return []
        ...
        for job in self.job_graph.getJobs():
            if job not in jobs_not_started:
                self.log.debug("[Find to run] Job already started: %s",
                               job.name)
                continue
            if not self.jobRequirementsReady(job):
                self.log.debug("[Find to run] Job requirements not met: %s",
                               job.name)
                continue
            ...
            for parent_job in self.job_graph.getParentJobsRecursively(job.name):
                if parent_job.name in unexecuted_job_names \
                    ...
                    self.log.debug("Parent job %s of %s not started or failed",
                                   parent_job.name, job.name)
                    break
            ...
            for parent_job in self.job_graph.getParentJobsRecursively(
                ...
                if parent_job.name in ignored_job_names:
                    self.log.debug("Parent job %s of %s ignored",
                                   parent_job.name, job.name)
                    all_parent_jobs_successful = False
                    break
    ...
    def findJobsToRequest(self, semaphore_handler):
        ...
        toreq = []
        if not self.live:
            self.log.debug("Found jobs to request: NONE (not live)")
            return []
        if not self.job_graph:
            self.log.debug("Found jobs to request: NONE (empty graph)")
            return []
        if self.item_ahead:
            if self.item_ahead.isHoldingFollowingChanges():
                self.log.debug("Found jobs to request: NONE (XXX)")
                return []
        ...
        for job in self.job_graph.getJobs():
            if job not in jobs_not_requested:
                self.log.debug("[Find to request] Job already requested: %s",
                               job.name)
                continue
            if not self.jobRequirementsReady(job):
                self.log.debug("[Find to request] Job requirements not met: %s",
                               job.name)
                continue
            all_parent_jobs_successful = True
            for parent_job in self.job_graph.getParentJobsRecursively(job.name):
    ...
    def setResult(self, build):
        ...
        self.log.debug("Result: %s, Error: %s", str(build.result_data), str(build.error_detail))
        skipped = []
