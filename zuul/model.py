        ...
        for queue in self.queues:
            ...
            for e in queue.queue:
                if not e.item_ahead:
                    ...
                    j_changes = []
                j_changes.append(e.formatItemJSON())
    ...
    def formatItemJSON(self):
        ...
        for job in self.pipeline.getJobs(changeish):
            ...
            ret['jobs'].append({
                ...
                'worker_name': build.worker.name if build else None,
                'worker_hostname': build.worker.hostname if build else None,
                'worker_ips': build.worker.ips if build else None,
                'worker_fqdn': build.worker.fqdn if build else None,
                'worker_progam': build.worker.program if build else None,
                'worker_version': build.worker.version if build else None,
                'worker_extra': build.worker.extra if build else None
            })
