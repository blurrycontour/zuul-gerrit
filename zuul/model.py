...
class QueueItem(object):
    ...
    def formatJSON(self, websocket_url=None):
        ...
        for job in self.getJobs():
            ...
            # If we find a job dependency whose build is not finished yet (has a valid result),
            # we assume that the job is still waiting for this dependency
            dependency_builds = [self.current_build_set.getBuild(j.name) for j in job.dependencies]
            waiting_for_dependency = not all(b.result for b in dependency_builds if b is not None)
