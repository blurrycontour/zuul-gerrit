...
class JobGraph(object):
    ...
    def addJob(self, job, parent_job):
        ...
        if parent_job is not None:
            ...
            if job.name in self._dependencies[parent_job.name]:
                return False
    ...
    def getDirectParentJobs(self, dependent_job):
        ...
        return {self.jobs[parent_name] for parent_name in parent_job_names}

    def getParentJobsRecursively(self, dependent_job):
        all_parent_jobs = set()
        jobs_to_iterate = {dependent_job}
        while len(jobs_to_iterate) > 0:
            current_job = jobs_to_iterate.pop()
