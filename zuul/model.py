    ...
    def __setitem__(self, *args, **kw):
        if self._frozen:
            raise Exception("Unable to modify frozen dict %s" %
                            (repr(self),))
        super(FreezableDict, self).__setitem__(*args, **kw)

    def clear(self, *args, **kw):
        if self._frozen:
            raise Exception("Unable to modify frozen dict %s" %
                            (repr(self),))
        super(FreezableDict, self).clear(*args, **kw)

    def setdefault(self, *args, **kw):
        if self._frozen:
            raise Exception("Unable to modify frozen dict %s" %
                            (repr(self),))
        super(FreezableDict, self).setdefault(*args, **kw)

    def pop(self, *args, **kw):
        if self._frozen:
            raise Exception("Unable to modify frozen dict %s" %
                            (repr(self),))
        super(FreezableDict, self).pop(*args, **kw)

    def popitem(self, *args, **kw):
        if self._frozen:
            raise Exception("Unable to modify frozen dict %s" %
                            (repr(self),))
        super(FreezableDict, self).popitem(*args, **kw)

    def update(self, *args, **kw):
        if self._frozen:
            raise Exception("Unable to modify frozen dict %s" %
                            (repr(self),))
        super(FreezableDict, self).update(*args, **kw)
