...
class Build(object):
    ...
    def toDict(self):
        return {
            ...
            "build_request_ref": self.buld_request_ref,
            "uuid": self.uuid,
        }
    ...
    def updateFromDict(self, d):
        ...
        self.node_name = d["node_name"]
        # self.nodeset = d["nodeset"]
...
class BuildSet(object):
    ...
    def toDict(self):
        log = logging.getLogger("SWE")
        log.debug(f"SWE> nodesets: {self.nodesets}")
        return {
            ...
            # TODO: serialize the nodeset after we lock the nodes on the
            # executor side
            # "nodesets": {j: n.toDict() for j, n in self.nodesets.items()},
            # TODO: get the node request from the cached nodepool API
            "node_requests": {j: r.uid for j, r in self.node_requests.items()},
    ...
    def updateFromDict(self, d):
        ...
        # self.nodesets = {j: NodeSet.fromDict(n)
        #                  for j, n in d["nodesets"].items()}
        nodepool = self.item.pipeline.manager.sched.nodepool
        log = logging.getLogger("SWE")
        log.debug(f"SWE> node request ids: {d['node_requests']}")
        request_gen = [(j, nodepool.requests.get(i))
                       for j, i in d["node_requests"].items()]
        log.debug(f"SWE> node requests: {request_gen}")
        # self.node_requests = {j: r for j, r in request_gen if r is not None}
