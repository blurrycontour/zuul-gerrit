...
class Gerrit(object):
    ...
    def read_config(self):
        ...
        self.whitelist = []
        if self.config.has_section('gerrit_whitelist'):
            for k, v in self.config.items('gerrit_whitelist'):
                self.whitelist.append(v)
    ...
    def updateChange(self, change):
        ...
        if 'project' not in data:
            self.log.info("Change %s,%s not found" % (change.number,
                                                        change.patchset))
