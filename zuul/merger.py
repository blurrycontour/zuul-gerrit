...
class Merger(object):
    ...
    def mergeChanges(self, changes, target_ref=None, mode=None):
        ...
        commit = False
        siblings_filter = lambda c: (c.project == change.project and
                                     c.branch == change.branch)
        previous_changes = filter(siblings_filter, changes)
        if len(previous_changes) == 1:
            commit = self._mergeChange(changes[-1], change.branch,
            ...
            commit = self._mergeChange(changes[-1], last_change,
                                       target_ref=target_ref, mode=mode,
                                       branch=False)
        if not commit:
            return commit
