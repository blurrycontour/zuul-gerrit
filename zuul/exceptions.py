...
class AuthTokenForbiddenException(Exception):
    defaultMsg = 'Insufficient privileges'
    HTTPError = 403


class AuthTokenUnauthorizedException(Exception):
...
class IncorrectZuulActionsClaimError(AuthTokenUnauthorizedException):
    defaultMsg = 'The "zuul.actions" claim is expected to be a dictionary'


class UnauthorizedZuulActionsClaimError(AuthTokenUnauthorizedException):
    defaultMsg = 'Issuer is not allowed to set "zuul.actions" claim'
