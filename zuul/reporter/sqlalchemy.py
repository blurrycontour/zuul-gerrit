...
class SQLAlchemyReporter(BaseReporter):
    """Sends off reports to Gerrit."""

    name = 'sqlalchemy'
    log = logging.getLogger("zuul.reporter.mysql.SQLAlchemyReporter")

    def __init__(self, reporter_config={}, sched=None, connection=None):
        super(SQLAlchemyReporter, self).__init__(
    ...
    def report(self, source, pipeline, item, message=None, params=[]):
        ...
        for job in pipeline.getJobs(item):
            ...
            result = build.result
            url = self._createBuildURL(job, pipeline, job, item)
            if result == 'SUCCESS':
                result = True
            else:
                result = False
...
def getSchema():
    sqlalchemy_reporter = v.Schema({
        v.Required('connection'): str,
        'build_table': str,
        'build_metadata_table': str,
    })
    return sqlalchemy_reporter
