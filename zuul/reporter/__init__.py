...
class BaseReporter(object, metaclass=abc.ABCMeta):
    ...
    def _formatItemReport(self, item, with_jobs=True, action=None):
        ...
        self.log.debug(ret)
        self.log.debug(type(ret))
        duration = self._formatOverallDuration(item)
        self.log.debug(duration)
        self.log.debug(type(duration))
    ...
    def _formatOverallDuration(self, item):
        if item.report_time and item.event.timestamp:
            self.log.debug('................1')
            self.log.debug(item.report_time)
            self.log.debug(type(item.report_time))
            self.log.debug(item.event.timestamp)
            self.log.debug(type(item.event.timestamp))
            self.log.debug(self._seconds_2_string(
