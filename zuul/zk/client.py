...
from typing import Dict, Any, Union, ItemsView, TYPE_CHECKING, TypeVar,\
    Generic, Callable
...
L = Callable[[List[str], TreeEvent, Optional[T], Optional[T]], None]
...
class ZooKeeperTreeCacheClient(Generic[T]):
    def __init__(self, zk: 'ZooKeeper', root: str, multilevel: bool=False,
        ...
        self._cache = {}  # type: Dict[str, T]
        self.__multilevel = multilevel  # type: bool
        self.__listeners = [listener] if listener else []  # type: List[L]

    def __str__(self):
        return "<ZooKeeperTreeCacheClient root=%s, zk=%s>" % (
            self.__root, hex(hash(self._zk)))
    ...
    def _create_cached_value(self, path: str, content: Dict[str, Any],
                             stat: ZnodeStat) -> T:
        # return ZooKeeperCacheItem(content, stat)
    ...
    def __tree_cache_listener(self, event: TreeEvent) -> None:
        try:
            ...
            for listener in self.__listeners:
                listener(segments, event, previous, cached)
...
class ZooKeeperBuildTreeCacheClient(
    ...
    def __init__(self, zk: 'ZooKeeper', zone: Optional[str]=None,
                 multilevel: bool = False,
                 listener: Optional[Callable[[List[str], TreeEvent,
                                              Optional[T], Optional[T]],
                                             None]] = None):
        root = "%s/%s" % (zk.ZUUL_BUILDS_ROOT, zone)\
            if zone else zk.ZUUL_BUILDS_ROOT
        super().__init__(zk, root, multilevel, listener)
