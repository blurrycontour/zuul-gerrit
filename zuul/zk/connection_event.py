...
class ZooKeeperConnectionEvent(ZooKeeperBase):
    ...
    ELECTION_ROOT = "/zuul/events/election"
    ...
    def _readLock(self, connection_name: str) -> ReadLock:
        lock_node = "%s/%s" % (self.CONNECTION_ROOT, connection_name)
    ...
    def watch(self, connection_name: str, watch: Callable[[List[str]], None]):
        if connection_name not in self.event_watchers:
            ...
            path = "%s/%s/nodes" % (self.CONNECTION_ROOT, connection_name)
    ...
    def pop(self, connection_name: str)\
        ...
        try:
            with self.client.withLock(lock, timeout=20.0):
                ...
                for child in sorted(children):
                    path = "%s/%s/nodes/%s" % (
                        self.CONNECTION_ROOT, connection_name, child)
            ...
            self.log.exception("Could not acquire lock: %s", lock.path)
            yield []  # must yield, popping an empty array
