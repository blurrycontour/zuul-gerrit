...
class ZookeeperBuildJobWorker(ZookeeperWorker):
    # TODO JK: This worker does not need to watch the /zuul/builds node
    ...
    def _process_children(self, children: Optional[List[str]]) -> None:
        ...
        for node_path, build_item in items:
            self._zk.resumeBuildAttempt(build_item.path, self.__job_resume)
            self._zk.cancelBuildAttempt(build_item.path, self.__job_stop)
            if not self._zk.client:
                raise Exception("No zookeeper client!")
            lock = self._zk.client.Lock(build_item.path)
            if lock and not lock.is_aquired():
                del self.__build_items[node_path]

        try:
            next_item = self._zk.getNextBuild()
            if next_item:
