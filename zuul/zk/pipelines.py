...
class PipelineStore(ZooKeeperSimpleBase):
    ...
    def cleanup(self, pipeline):
        ...
        all_items = self._tree_cache.get_children(self.items_root, default=[])
        items_to_clean = all_items - valid_items
        for item_uuid in items_to_clean:
            self.kazoo_client.delete(self.queueItemPath(item_uuid),
                                     recursive=True)

        valid_queues = {q.uuid for q in pipeline.queues}
        all_queues = self._tree_cache.get_children(self.queue_root, default=[])
        queues_to_clean = all_queues - valid_queues
        for queue_uuid in queues_to_clean:
            self.kazoo_client.delete(self.queuePath(queue_uuid),
                                     recursive=True)
    ...
    def _restoreBuildSetState(self, build_set):
        ...
        self.log.debug(f"SWE> existing builds: {existing_builds}")
