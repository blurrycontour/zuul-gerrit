...
from zuul.zk.cache import ZooKeeperWorkItem, DataType
from zuul.zk.client import L, ZooKeeperTreeCacheClient
...
class ZooKeeperWorkBase(ZooKeeperBase, Generic[TreeCacheClient, WorkItem],
    ...
    def __init__(self, client: ZooKeeperClient):
        ...
        self._cache_listeners: List[L] = []
        self._locks: Dict[str, Lock] = {}
        self._registered_zones: Union[bool, List[str]] = [self.DEFAULT_ZONE]
        self._to_delete: List[str] = []

        def watchChildren(children):
            if self._registered_zones is True:
    ...
    def registerZone(self, zone: Optional[str] = None) -> None:
        ...
        if isinstance(self._registered_zones, list) \
                and zone not in self._registered_zones:
            self._registered_zones.append(zone)
    ...
    def next(self, names: Iterable[str]) -> Optional[WorkItem]:
        ...
        """
        for path, cached in self.inState(['REQUESTED', 'RUNNING', 'PAUSED'],
                                         names):
    ...
    def inState(self, state: Union[str, List[str]],
        ...
        :param state: One or more state the work should be in or "ALL" for
                      all states.
        :param names: Name filter.
        ...
        return sorted(items, key=lambda b: b[0].rsplit("/", 1)[::-1][0])
    ...
    def cleanup(self) -> None:
        ...
        for path, cached in self.inState(['RUNNING', 'PAUSED']):
            if self.kazoo_client.exists(path):
                lock = self.getLock(path)
        ...
        for path, cached in self.inState('REMOVED'):
            self.remove(path)
