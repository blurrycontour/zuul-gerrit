...
class ZooKeeperTreeCacheClient(Generic[CacheItem]):
    ...
    def __init__(self, client: KazooClient, root: str,
        ...
        self._listeners: List[TreeCacheListener] = [listener]\
            if listener else []
    ...
    def _getCachedStat(self, segments: List[str], cached: CacheItem)\
            -> ZnodeStat:
    ...
    def _setCachedStat(self, segments: List[str], cached: CacheItem,
                       stat: ZnodeStat) -> None:
        # Only first level props are relevant
        prop = 'stat' if self._multilevel or len(segments) == 1\
            else '%s_stat' % segments[1]
    ...
    def _setCachedValue(self, segments: List[str], cached: CacheItem,
                        value: Any) -> None:
        # Only first level props are relevant
