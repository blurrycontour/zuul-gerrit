...
class ZooKeeperNodepool(object):
    ...
    def __init__(self, client: ZooKeeperClient, enable_cache: bool=True):
        ...
        self.client = client
        client.on_connect_listeners.append(self.__on_connect)
        client.on_disconnect_listeners.append(self.__on_disconnect)
    ...
    def getNodes(self):
        ...
        """
        try:
            return self.kazoo_client.get_children(self.NODEPOOL_NODES_ROOT)
        except NoNodeError:
            return []
    ...
    def submitNodeRequest(self, node_request, watcher):
        ...
        path = self.kazoo_client.create(path, self._dictToStr(data),
    ...
    def storeNodeRequest(self, node_request):
        ...
        """
        path = '%s/%s' % (self.NODEPOOL_REQUEST_ROOT, node_request.id)
        self.kazoo_client.set(path, self._dictToStr(node_request.toDict()))
    ...
    def storeNode(self, node):
        ...
        self.kazoo_client.set(path, self._dictToStr(node.toDict()))
    ...
    def heldNodeCount(self, autohold_key):
        ...
        try:
            nodes = self.kazoo_client.get_children(self.NODEPOOL_NODES_ROOT)
        except NoNodeError:
            return 0
