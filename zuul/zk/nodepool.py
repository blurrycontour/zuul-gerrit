...
from zuul.model import Group, HoldRequest, Node, NodeRequest, NodeSet
...
class ZooKeeperNodepool(ZooKeeperBase):
    ...
    def __init__(self, client, enable_cache=True,
        ...
        # TODO (felix): Using the data watches only makes sense if we provide a
    ...
    def getNodeRequest(self, node_request_id, nodeset=None):
        ...
        if nodeset is None:
            ...
            for i, node_type in enumerate(json_data["node_types"]):
                # Restore the original node names from the requestor_data.
                # This is necessary as this nodeset will be provided to the
                # executor via job parameters.
                # TODO (felix): This works around the current nodepool protocol
                # by providing the additional information via the
                # requestor_data field. This is necessary as nodepool strips
                # all other parameters from the node request. Is there a better
                # way to fully restore a node request from ZK?
                node_name = json_data["requestor_data"]["node_names"][i]
                node = Node(name=node_name, label=node_type)
                nodeset.addNode(node)

            for g in json_data["requestor_data"]["node_groups"]:
                nodeset.addGroup(Group.fromDict(g))
