...
from zuul.model import HoldRequest, Node, NodeRequest, STATE_HOLD, STATE_USED
...
class ZooKeeperNodepool(ZooKeeperBase):
    ...
    def _nodePath(self, node: str) -> str:
    ...
    def _markHeldNodesAsUsed(self, request: HoldRequest) -> bool:
        ...
        for node_id in getHeldNodeIDs(request):
            node = self._getNode(node_id)
            if not node or node['state'] == STATE_USED:
                continue
    ...
    def updateNodeRequest(
        self, node_request: NodeRequest, data: Optional[bytes] = None
    ) -> None:
        """
    ...
    def lockNodeRequest(
        ...
        :param blocking: Whether or not to block on trying to acquire the lock
        :param timeout: When blocking, how long to wait for the lock
    ...
    def heldNodeCount(self, autohold_key: Set[str]) -> int:
        ...
        for nodeid in nodes:
            ...
            if not node_data:
                ...
                continue
            node_data = json.loads(node_data.decode('utf8'))
            if (node_data['state'] == STATE_HOLD and
