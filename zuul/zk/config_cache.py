...
class FilesCache(ZooKeeperSimpleBase, MutableMapping):
    ...
    def setValidFor(self, extra_config_files, extra_config_dirs, update_ltime):
    ...
    def isValidFor(self, tpc, cache_ltime):
        ...
        try:
            ...
            update_ltime = content["update_ltime"]
        except Exception:
            return False

        if update_ltime < cache_ltime:
            # Cache is outdated
            return False
    ...
    @property
    def update_ltime(self):
        try:
            data, _ = self.kazoo_client.get(self.root_path)
            content = json.loads(data)
            return content["update_ltime"]
        except Exception:
            return -1
