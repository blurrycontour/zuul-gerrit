...
class GlobalEventWatcher(ZooKeeperBase):
    ...
    def _event_watch(self, event_list):
        if event_list:
            self.callback()
...
class PipelineEventWatcher(ZooKeeperBase):
    ...
    def _tenant_watch(self, tenants):
    ...
    def _pipeline_watch(self, tenant_name, pipelines):
    ...
    def _event_watch(self, event_list, event=None):
        if event is None:
