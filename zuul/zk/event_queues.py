...
class GlobalEventWatcher(ZooKeeperBase):
    ...
    def _event_watch(self, event_list: List[str]) -> None:
        if event_list:
            self.callback()
...
class PipelineEventWatcher(ZooKeeperBase):
    ...
    def _tenant_watch(self, tenants: List[str]) -> None:
    ...
    def _pipeline_watch(self, tenant_name: str, pipelines: List[str]) -> None:
