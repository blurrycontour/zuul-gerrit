...
class ZooKeeperEventWatcher(ZooKeeperBase):
    ...
    def _queueWatch(
        ...
        if event is None:
            # Initial call when the watch is created. If there are already
            # events in the queue we trigger the callback.
            if event_list:
                self.callback()
...
class ZooKeeperEventQueueRegistry(defaultdict):
    def __init__(self, default_factory: Callable[[str], ZooKeeperEventQueue]):
