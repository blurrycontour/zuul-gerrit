...
from typing import Optional, List, Callable, Generator, Dict, Union
...
class ZooKeeperClient(object):
    ...
    def callElection(self, path: str, func: Callable[[], Optional[bool]],
                     repeat: Union[bool, Callable[[], bool]] = False)\
            -> None:
        """
        ...
        :param repeat: Whether the function should be called in an infinite
                       ...
                       each loop.
        """
        election = self.election(path, repeat)
        while next(election, default=False):
            try:
                election.send(func())
            except StopIteration:
                pass

    def election(self, path: str,
                 repeat: Union[bool, Callable[[], bool]] = False)\
            -> Generator[bool, Optional[bool], None]:
        """
        An extended version of kazoo.recipe.election.Election to provide
        a simple interface for (repeatable) generator calls.
        ...
        while True:
            try:
                with lock:
                    interrupt = yield True
                    if interrupt is True:
                        return
            except ConnectionClosedError:
                self.log.warning("Election: Connection to ZK closed")

            if repeat is False or callable(repeat) and not repeat():
                return
