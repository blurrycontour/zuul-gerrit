...
CacheStat = collections.namedtuple("CacheStat", ["key", "uuid", "version"])
...
class AbstractChangeCache(ZooKeeperSimpleBase, Iterable, abc.ABC):
    ...
    def _get(self, key, force=False):
        ...
        cache_stat = CacheStat(key, data_uuid, zstat.version)
        if change:
    ...
    def set(self, key, change):
        ...
        change.cache_stat = CacheStat(key, data_uuid, version)
