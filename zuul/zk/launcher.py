...
class ProviderCache(ZuulTreeCache):
    ...
    def parsePath(self, path):
        ...
        if len(parts) != 3:
            # FIXME: handle potential sub-nodes here
            return None
...
class LauncherClientApi:
    REQUEST_ROOT = "/zuul/nodeset"
    REQUEST_LOCK_ROOT = f"{REQUEST_ROOT}/locks"
    PROVIDER_ROOT = "/zuul/providers"
    ...
    def _getRequestPath(self, request):
        return f"{self.REQUEST_ROOT}/requests/{request.uuid}"

    def _getNodePath(self, provider, node):
        return f"{self.PROVIDER_ROOT}/{provider}/nodes/{node.uuid}"

    def _dictToBytes(self, data):
        return json.dumps(data).encode("utf-8")

    def _bytesToDict(self, raw_data):
    ...
    def _getNodeLockPath(self, node):
        return f"{self.PROVIDER_ROOT}/{node.provider}/locks/{node.uuid}"
    ...
    def updateNode(self, node, **attrs):
        ...
        zstat = self.zk_client.client.set(
            node.path,
            self._dictToBytes(node.toDict()),
            version=version
        )
        node.stat = zstat


class LauncherServerApi(LauncherClientApi):
    ...
    def __init__(self, zk_client, component_registry, component_info,
        ...
        self.request_cache = RequestCache(
            self.zk_client, self.REQUEST_ROOT, self.event_callback)
    ...
    def getMatchingRequests(self):
        # Q: Can all launchers dispatch nodeset requests for all providers
        #    or is there some info that might not be available?
        candidate_launchers = {
    ...
    def _getRequestLockPath(self, request):
        return f"{self.REQUEST_LOCK_ROOT}/{request.uuid}"
    ...
    def getMatchingProviderNodes(self):
        # FIXME: filter candidate launchers by provider
        all_launchers = {
            c.hostname: c for c in self.component_registry.all("launcher")}
    ...
    def updateNode(self, node, **attrs):
        ...
        super().updateNode(node, **attrs)

    def cleanupNodes(self):
        for node in self.provider_cache.getNodes():
            # FIXME: check here if the request still exists
            if node.state != ProviderNode.STATE_USED:
                continue
            if node.is_locked:
                continue
            try:
