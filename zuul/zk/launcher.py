...
class LauncherApi:
    ...
    def requestProviderNode(self, provider, node):
        path, zstat = self.zk_client.client.create(
            self._getNodePath(provider, node.uuid),
    ...
    def _getNodePath(self, provider, node_uuid):
        return (
            f"{self.PROVIDER_ROOT}/{provider}/"
            f"{ProviderCache.NODES_PATH}/{node_uuid}"
        )
...
class LauncherServerApi(LauncherApi):
    ...
    def __init__(self, zk_client, component_registry, component_info,
                 event_callback, provider_filter=None):
    ...
    def getMatchingRequests(self):
        ...
        for request in sorted(self.request_cache.getRequests(),
    ...
    def getProviderNodeByUuid(self, provider, node_uuid):
        path = self._getNodePath(provider, node_uuid)
        return self.provider_cache.getNode(path)
