...
def tenant_read_lock(client, tenant_name):
    return client.kazoo_client.ReadLock(f"{TENANT_LOCK_ROOT}/{tenant_name}")


def tenant_write_lock(client, tenant_name):
    return client.kazoo_client.WriteLock(f"{TENANT_LOCK_ROOT}/{tenant_name}")
...
def event_queue_lock(client, queue_name):
    return client.kazoo_client.Lock(f"/zuul/locks/events/{queue_name}")
