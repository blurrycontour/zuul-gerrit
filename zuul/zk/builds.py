...
class ZooKeeperBuilds(ZooKeeperBase):
    ...
    def __init__(self, client: ZooKeeperClient, enable_cache: bool):
        ...
        self.__builds_cache: Dict[str, ZooKeeperBuildTreeCacheClient] = {}
    ...
    def __in_state(self, condition: Callable[[str], bool])\
        ...
        # Make sure this is sorted by path (first item in the tuple) which
        # contains precedence and sequence in ascending order
        return sorted(builds, key=lambda b: b[0])
                    ...
                    try:
                        ...
                        self.kazoo_client.set(
                            ...
                            json.dumps(cached.content).encode(
                                encoding='UTF-8'),
                            version=cached.stat.version)
                        log.warning("Next [%s] %s: FAILED", path,
    ...
    def resume_attempt(self, path: str,
        ...
        if cached and isinstance(cached, ZooKeeperBuildItem)\
            ...
            self.log.warning("Resume attempt: Build node %s is not cached!",
                             path)
    ...
    def complete(self, path: str, result: Dict[str, Any],
        ...
        if content and lock.is_acquired\
            ...
            self.data(path, result, key='result' if success else 'exception')
            return True
        return False