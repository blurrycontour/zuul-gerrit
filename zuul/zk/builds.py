...
class ZooKeeperBuildTreeCacheClient(
    ...
    def __init__(self, client: KazooClient, zone: Optional[str]=None,
                 ...
                 listener: Optional[Callable[[List[str], TreeEvent,
                                              Optional[ZooKeeperBuildItem],
                                              Optional[ZooKeeperBuildItem]],
                                             None]] = None):
        root = "%s/%s" % (ZooKeeperBuildsMixin.ZUUL_BUILDS_ROOT, zone)\
            if zone else ZooKeeperBuildsMixin.ZUUL_BUILDS_ROOT
...
class ZooKeeperBuildsMixin(ZooKeeperBase):
    """
    ...
    def connect(self, hosts: str, read_only: bool=False, timeout: float=10.0,
        ...
        super().connect(hosts, read_only, timeout, tls_cert, tls_key, tls_ca)
    ...
    def registerBuildZone(self, zone: Optional[str]=None):
        ...
        if self.enable_cache \
            ...
            builds_cache = ZooKeeperBuildTreeCacheClient(self.client, zone)
