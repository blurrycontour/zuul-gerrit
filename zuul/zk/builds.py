...
class BuildState(Enum):
    # Waiting
    REQUESTED = 0
    # TODO (felix): Should we move the HOLD state to tests/zk/builds.py? AFAIK
    # it's only used for the tests.
    HOLD = 1
    # InProgress
    RUNNING = 2
    PAUSED = 3
...
class BuildItem:
    ...
    def __eq__(self, other) -> bool:
        return (
            ...
            and self._zstat.ctime == other._zstat.ctime
        )
...
class BuildQueue(ZooKeeperBase):
    ...
    def __init__(
        ...
        # TODO (felix): If use_cache is False, don't create the TreeCache.
        # In case of registering for all zones, we also don't need to create
        # the ChildWatch for new zone nodes.
        use_cache: bool = True,
    ):
    ...
    def register_all_zones(self) -> None:
        ...
        def watch_children(children):
            for zone in children:
                self.register_zone(zone)
                # TODO (felix): Start() the zone_cache in here if we are
                # already connected (to keep the register_zone method clean).
    ...
    def register_zone(self, zone: str) -> None:
        ...
        if zone not in self._zone_tree_caches:
            zone_cache = TreeCache(self.kazoo_client, zone_root)
            # TODO (felix): Start the listeners conditionally, e.g. in the
            # scheduler it's not needed.
    ...
    def _iter_builds(self):
        # TODO (felix): Shouldn't the executor provide some filter to the
        # next() method to only get builds from its own zone? Otherwise this
        # whole zone-namespace-thing doesn't make much sense to me.
        try:
            # TODO (felix): Only loop over all zones if the zone_filter is
            # None. Otherwise loop over the zones in zone_filter instead.
    ...
    def in_state(self, *states: BuildState) -> List[BuildItem]:
        if not states:
        ...
        if builds:
            self.log.debug(
                "FE: Found builds in state %s: %s",
                states,
                [(b.uuid, b.params["job"]) for b in list(builds)],
            )
    ...
    def update(self, build: BuildItem) -> None:
        ...
        try:
            ...
            # TODO (felix): Provide custom handling for build_queue API
            # TODO (felix): Raise the error or handle it in here? If handle,
            # how?
            raise NoNodeError("Could not update %s" % build.path)
    ...
    def lost_builds(self):
        ...
        """
        yield from filter(
            lambda b: self.is_locked(b), self.in_state(BuildState.RUNNING)
        )
        """

        # Get a list of builds which are running but not locked by any executor
        """
        for build in self.in_state(BuildState.RUNNING):
            # TODO (felix): How to properly check if the build is locked?
            if self.lock(build, blocking=False):
                yield build
        """

        """
        yield from filter(
            lambda b: b.lock is None, self.in_state(BuildState.RUNNING)
        )"""
    ...
    @staticmethod
    def _bytes_to_dict(data: Dict[str, Any]):
        return json.loads(data.decode("utf-8"))

    @staticmethod
    def _dict_to_bytes(data: Dict[str, Any]):
