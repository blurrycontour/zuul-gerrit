...
class ZooKeeperBuilds(ZooKeeperBase):
    ...
    def cleanup(self) -> None:
        ...
        self.log.debug("Cleanup: Cleanup builds with lost executors")
        for path, cached in builds:
            if self.kazoo_client.exists(path):
                lock = self.get_lock(path)
        ...
        self.log.debug("Cleanup: Cleanup nodes where deletion failed")
        ...
        self.log.debug("Cleanup: Cleanup nodes where deletion failed and"
                       " responsible client died")
        ...
        self.log.debug("Cleanup: Cleanup lock objects of removed nodes")
    ...
    def __send(self, path: str, key: str, data: Dict[str, Any]):
        ...
        if stat:
            log.debug("Send: [%s] UPDATE", node, data)
            self.kazoo_client.set(node, value, version=stat.version)
        else:
            try:
                log.debug("Send: [%s] CREATE", node, data)
