...
class ZooKeeperBuilds(ZooKeeperBase):
    ...
    def cleanup(self) -> None:
                ...
                try:
                    ...
                    try:
                        ...
                        self.kazoo_client.set(
                            ...
                            .encode(encoding='UTF-8'),
                            version=cached.stat.version)
                        self.log.debug("Cleaning up build [%s]: %s",
                                       path, json.dumps(cached.content))
                    finally:
                        lock.release()
        ...
        for path in self.__to_delete:
            try:
                self.remove(path, force=True)
            except NotEmptyError:
                self.log.exception(
                    "Cleaning up local removed node %s failed!", path)
        ...
        for path, cached in builds:
            try:
                ...
                self.log.exception(
                    "Cleaning up marked removed node %s failed!", path)
    ...
    def __delete_attempt(self, path: str, log=None):
        if not self.kazoo_client:
            raise NoClientException()
        ...
        try:
            ...
            log.debug("Node %s already deleted")
            if path in self.__to_delete:
                self.__to_delete.remove(path)
        except Exception:
            ...
            if content and content['state'] in ['COMPLETED', 'CANCELLED',
                ...
                try:
                    ...
                    self.kazoo_client.set(
                        path, json.dumps(content).encode(encoding='UTF-8'),
                        version=stat.version)
                    log.info("Marked as removed: %s", path)
                except Exception:
                    log.exception("Failed to mark %s as removed", path)
            elif content and content['state'] == 'REMOVED':
                if path not in self.__to_delete:
                    self.__to_delete.append(path)
                log.debug("Already marked as removed: %s", path)
            elif content:
                log.debug("Cannot mark %s in state %s as removed!", path,
                          content['state'])
            else:
                if path in self.__to_delete:
                    self.__to_delete.remove(path)
                log.debug("Could not mark as deleted non existing %s!", path)
    ...
    def remove(self, path: str, force: bool = False) -> None:
        ...
        if not lock or not lock.is_acquired:
            ...
            self.__delete_attempt(path, log)
        elif force:
            lock.release()
            ...
            raise LockException("Lock not released!")
