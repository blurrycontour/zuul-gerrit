...
class TenantParser(object):
    ...
    tenant_source = vs.Schema({'trusted-projects': [str],
    ...
    def fromYaml(base, project_key_dir, connections, scheduler, merger, conf,
        ...
        trusted_projects, untrusted_projects = \
            TenantParser._loadTenantProjects(
                project_key_dir, connections, conf)
        for project in trusted_projects:
            tenant.addTrustedProject(project)
        for project in untrusted_projects:
            tenant.addUntrustedProject(project)
        tenant.trusted_projects_config, tenant.untrusted_projects_config = \
            TenantParser._loadTenantInRepoLayouts(merger, connections,
                                                  tenant.trusted_projects,
                                                  tenant.untrusted_projects,
                                                  cached)
        unparsed_config.extend(tenant.trusted_projects_config)
    ...
    def _loadTenantProjects(project_key_dir, connections, conf_tenant):
        ...
        return trusted_projects, untrusted_projects

    @staticmethod
    def _loadTenantInRepoLayouts(merger, connections, trusted_projects,
                                 untrusted_projects, cached):
        trusted_projects_config = model.UnparsedTenantConfig()
        ...
        for project in trusted_projects:
            ...
            if cached and project.unparsed_config:
                TenantParser.log.info(
                    ...
                    (project,))
                trusted_projects_config.extend(project.unparsed_config)
                continue
            ...
            for fn in ['zuul.yaml', '.zuul.yaml']:
                if job.files.get(fn):
                    ...
                    if job.source_context.trusted:
                        incdata = TenantParser._parseTrustedProjectLayout(
                            job.files[fn], job.source_context)
                        trusted_projects_config.extend(incdata)
                    else:
        ...
        return trusted_projects_config, untrusted_projects_config

    @staticmethod
    def _parseTrustedProjectLayout(data, source_context):
...
class ConfigLoader(object):
    ...
    def createDynamicLayout(self, tenant, files,
                            include_trusted_projects=False):
        if include_trusted_projects:
            config = model.UnparsedTenantConfig()
            for project in tenant.trusted_projects:
                self._loadDynamicProjectData(config, project, files, True)
        else:
            config = tenant.trusted_projects_config.copy()
