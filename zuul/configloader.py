...
class TenantParser(object):
    ...
    def fromYaml(self, abide, conf, ansible_manager, executor, min_ltimes=None,
        ...
        for branch_future in as_completed(branch_futures.keys()):
            ...
            # We don't really care if the context here is for a trusted
            # or untrusted project, so we set the trusted flag to False
            # even though the project might indeed be trusted.
            source_context = model.SourceContext(
                tpc.project.canonical_name, tpc.project.name,
                tpc.project.connection_name, None, None, False)
