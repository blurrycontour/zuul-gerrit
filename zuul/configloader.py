...
class TenantParser(object):
    ...
    def _loadProjectKeys(project_key_dir, project):
        project.private_key_file = (
            project_key_dir + '/private/' + project.name + '.key')
        project.public_key_file = (
            project_key_dir + '/public/' + project.name + '.pem')
    ...
    def _generateKeys(project):
        ...
        for key_file in (project.private_key_file,
                         project.public_key_file):
    ...
    def _loadKeys(project):
        ...
        if not os.path.isfile(project.private_key_file):
            raise Exception(
                'Private key file {0} not valid'.format(
                    project.private_key_file))
        if not os.path.isfile(project.public_key_file):
            raise Exception(
                'Public key file {0} not valid'.format(
                    project.public_key_file))
