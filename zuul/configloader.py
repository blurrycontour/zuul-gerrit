...
class TenantParser(object):
    ...
    def fromYaml(self, base, project_key_dir, conf, old_tenant):
        ...
        tenant.layout.loading_errors.extend(loading_errors)
        return tenant
    ...
    def _parseLayoutItems(self, layout, tenant, data,
        ...
        for config_pragma in data.pragmas:
            try:
                ...
                self.log.exception(e)
                layout.loading_errors.append(e)
...
class ConfigLoader(object):
    ...
    def loadConfig(self, unparsed_abide, project_key_dir):
        ...
        for conf_tenant in unparsed_abide.tenants:
            ...
            if tenant.layout.loading_errors:
                self.log.warning(
    ...
    def reloadTenant(self, config_path, project_key_dir, abide, tenant):
        ...
        if new_tenant.layout.loading_errors:
            self.log.warning(
                "%s errors detected during %s tenant "
                "configuration loading" % (
    ...
    def createDynamicLayout(self, tenant, files,
        ...
        layout = model.Layout(tenant)
        layout.loading_errors.extend(loading_errors)
        self.log.debug("Created layout id %s", layout.uuid)
        ...
        if layout.loading_errors:
            self.log.warning(
                "%s errors detected during %s tenant configuration loading" % (
                    len(layout.loading_errors), tenant.name))
        return layout
