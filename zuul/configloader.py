...
class JobParser(object):
    ...
    def _getImpliedBranches(reference, job, project_pipeline,
        ...
        # for top-level job definitions and variants (including
        # project-templates).  In-project project-pipeline job
        # variants should more closely attach to their branch if they
        # appear in a project-repo.
        if (reference and
        ...
        if (job.source_context and
            ...
            (not project_template) and
            ((not same_branch) or project_pipeline)):
    ...
    def fromYaml(tenant, layout, conf, project_pipeline=False,
        ...
        if (project_pipeline or 'branches' not in conf):
            branches = JobParser._getImpliedBranches(
                reference, job, project_pipeline, project_template)
    ...
    def fromYaml(tenant, layout, conf_list):
        ...
        for conf in conf_list:
            ...
            with configuration_exceptions('project', conf):
                ...
                configs.extend([(layout.project_templates[name], implied_branch)
                                for name in conf_templates])
