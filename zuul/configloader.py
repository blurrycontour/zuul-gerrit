...
class ProjectParser(object):
    ...
    def fromYaml(self, conf):
        ...
        if not project_name:
            ...
            name = (conf['_source_context'].project.canonical_name)
...
class TenantParser(object):
    ...
    def _parseLayoutItems(self, layout, tenant, data,
        ...
        self._fixupProjectPipelineConfigs(layout)
        self._validateProjectPipelineConfigs(layout)

    def _fixupProjectPipelineConfigs(self, layout):
        # Set attributes on the reference definition if they show up
        # in later ones
        for project_name in layout.project_configs.keys():
            reference = in layout.project_configs.getProjectConfig(project_name)
            for project_config in layout.project_configs[project_name][1:]:
                if (reference.merge_mode is None and
                    project_config.merge_mode is not None):
                    reference.merge_mode = project_config.merge_mode
                if (reference.default_branch is None and
                    project_config.default_branch is not None):
                    reference.default_branch = project_config.default_branch
