...
class BaseMergeServer(metaclass=ABCMeta):
    ...
    def cat(self, job):
        ...
        with lock:
            (files, revision) = self.merger.getFiles(connection_name, project_name,
                                         args['branch'], args['files'],
                                         args.get('dirs'))
        self.log.debug("cat job get revision:{}".format(revision))
    ...
    def fileschangesrev(self, job):
        self.log.debug("Got fileschangesrev job: %s" % job.unique)
        args = json.loads(job.arguments)
        zuul_event_id = args.get('zuul_event_id')
