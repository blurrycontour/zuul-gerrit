...
class BaseMergeServer(metaclass=ABCMeta):
    ...
    def cat(self, job):
        ...
        self.log.debug("'cat' job %s payload size: %s",
    ...
    def merge(self, job):
        ...
        payload = json.dumps(result)
        self.log.debug("'merge' job %s payload size: %s",
                       job.unique, sys.getsizeof(payload))
        job.sendWorkComplete(payload)
    ...
    def refstate(self, job):
        ...
        result['zuul_event_id'] = zuul_event_id
        payload = json.dumps(result)
        self.log.debug("'refstate' job %s payload size: %s",
                       job.unique, sys.getsizeof(payload))
        job.sendWorkComplete(payload)
    ...
    def fileschanges(self, job):
        ...
        result['zuul_event_id'] = zuul_event_id
        payload = json.dumps(result)
        self.log.debug("'fileschanges' job %s payload size: %s",
                       job.unique, sys.getsizeof(payload))
        job.sendWorkComplete(payload)
