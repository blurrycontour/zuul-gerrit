...
class Repo(object):
    ...
    def _git_push(self, repo, remote, ref, zuul_event_id):
        log = get_annotated_logger(self.log, zuul_event_id)
        # Specify the remote ref, we want to push the changes to
        remote_ref = "HEAD:{}".format(ref)
    ...
    def push(self, ref, remote='origin', zuul_event_id=None):
        log = get_annotated_logger(self.log, zuul_event_id)
        repo = self.createRepoObject(zuul_event_id)
        log.debug("Pushing repository %s" % self.local_path)
        return self._git_push(repo, remote, ref, zuul_event_id)
...
class Merger(object):
    ...
    def pushChanges(self, items, repo_state=None, repo_locks=None,
        ...
        # and deal with the repo_state and repo_locks in here to do both, merge
        # and push, in one go. Otherwise, we might get conflicts in between
