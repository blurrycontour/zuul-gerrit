...
class Repo(object):
    ...
    def _reset(local_path, env, log=None):
        ...
        for ref in origin.refs:
            ...
            repo.create_head(ref.remote_head, ref, force=True)
        if log:
            log.debug("DEBUG _reset repo.head.commit %s", repo.head.commit)
        return messages
    ...
    def checkout(self, ref, zuul_event_id=None):
        ...
        log.debug("DEBUG checkout out %s %s", ref, repo.head.commit)
    ...
    def update(self, zuul_event_id=None, build=None):
        ...
        log.debug("repo.head.commit %s", repo.head.commit)
