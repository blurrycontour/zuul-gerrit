...
class Merger(object):
    ...
    def _mergeItem(self, item, recent, repo_state, zuul_event_id,
        ...
        # We are not sure where the remote refs got removed, so we list
        # remote branches in different stages to identify where it got lost.
        debug_messages = {}
        # Insert debug message for missing remote ref issue
        try:
            if repo.cache_path:
                debug_messages["Remote branch in cache"] = str(
                    git.cmd.Git(repo.cache_path).branch("-r"))
