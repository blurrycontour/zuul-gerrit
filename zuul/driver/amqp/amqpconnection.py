...
class AMQPClient(MessagingHandler):
    ...
    def on_start(self, event):
        ...
        self.conn = event.container.connect(
            urls=self.connection.urls, ssl_domain=self.connection.ssl_domain)
    ...
    def __init__(self, driver, connection_name, connection_config):
        ...
        if not self.address:
            raise ConfigurationError(
                ...
                self.connection_name)
        if self.connection_config.get('ca_file'):
            domain = SSLDomain(SSLDomain.MODE_CLIENT)
            cert_file = self.connection_config.get('cert_file')
            key_file = self.connection_config.get('key_file')
            if not cert_file or not key_file:
                raise ConfigurationError(
                    'TLS requires the cert_file and key_file options in %s' %
                    self.connection_name)
            domain.set_trusted_ca_db(self.connection_config.get('ca_file'))
            domain.set_credentials(cert_file, key_file, None)
