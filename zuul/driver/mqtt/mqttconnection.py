...
class MQTTConnection(BaseConnection):
    ...
    def __init__(self, driver, connection_name, connection_config):
        ...
        if self.connection_config.get('ca_certs'):
            if (self.connection_config.get('certfile') and
                not self.connection_config.get('keyfile')) or (
                    not self.connection_config.get('certfile') and
                    self.connection_config.get('keyfile')):
                self.log.error("keyfile and certfile should both be set.")
            self.client.tls_set(
                self.connection_config.get('ca_certs'),
