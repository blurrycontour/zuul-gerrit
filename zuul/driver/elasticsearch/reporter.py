...
# TODO(fbo): Check behavior is set in start reporter section
# TODO(fbo): Test a job in failure
...
class ElasticsearchReporter(BaseReporter):
    ...
    def __init__(self, driver, connection, pipeline, config=None):
        ...
        self.index_job_vars = self.config.get('index_job_vars', False)

    def _add_job_variables(self, doc, job):
    ...
    def report(self, item):
        ...
        for job in item.getJobs():
            ...
            if self.index_job_vars:
                self._add_job_variables(build_doc, job)
