...
class JWTAuthenticator(AuthenticatorInterface):
    ...
    def __init__(self, **conf):
        ...
        try:
            self.token_expiry = float(conf.get('token_expiry', math.inf))
        except ValueError:
            raise Exception('token_expiry must be an integer')
    ...
    def decodeToken(self, rawToken):
        ...
        if not isinstance(actions, dict):
            raise exceptions.IncorrectZuulActionsClaimError(realm=self.realm)
        if actions and not self.allow_authz_override:
            ...
            raise exceptions.UnauthorizedZuulActionsClaimError(
                realm=self.realm)
