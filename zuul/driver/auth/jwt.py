...
class JWTAuthenticator(AuthenticatorInterface):
    ...
    def __init__(self, **conf):
        ...
        try:
            self.max_token_age = float(conf.get('max_token_age', math.inf))
        except ValueError:
            raise ValueError('max_token_age must be a numerical value')
    ...
    def decodeToken(self, rawToken):
        ...
        if self.max_token_age < math.inf and 'iat' not in decoded:
        ...
        if now - issued_at > self.max_token_age:
            raise exceptions.TokenExpiredError(
