...
class JWTAuthenticator(AuthenticatorInterface):
    ...
    def decodeToken(self, rawToken):
        ...
        try:
            ...
            raise exceptions.AuthTokenUndecodedException(self.realm)
        except jwt.exceptions.ExpiredSignatureError:
            raise exceptions.TokenExpiredError(self.realm)
        except jwt.InvalidIssuerError:
            raise exceptions.IssuerUnknownError(self.realm)
        except jwt.InvalidAudienceError:
            raise exceptions.IncorrectAudienceError(self.realm)
        except Exception as e:
            raise exceptions.AuthTokenUnauthorizedException(
                self.realm,
                msg=e)
        if not all(x in decoded for x in ['aud', 'iss', 'exp', 'sub']):
            raise exceptions.MissingClaimError(self.realm)
        if self.uid_claim not in decoded:
            raise exceptions.MissingUIDClaimError(self.realm)
        expires = decoded.get('exp', 0)
        if expires < time.time():
            raise exceptions.TokenExpiredError(self.realm)
        actions = decoded.get('zuul.actions', {})
        if not isinstance(actions, dict):
            raise exceptions.IncorrectZuulActionsClaimError(self.realm)
        if actions and not self.allow_authz_override:
            msg = ('Issuer "%s" attempt to override User "%s" '
                   'authorization denied')
            logger.info(msg % (decoded['iss'], decoded[self.uid_claim]))
            logger.debug('%r' % actions)
            raise exceptions.UnauthorizedZuulActionsClaimError(self.realm)
        if actions and self.allow_authz_override:
            msg = ('Issuer "%s" attempt to override User "%s" '
...
class RS256withJWKSAuthenticator(JWTAuthenticator):
    ...
    def _decode(self, rawToken):
        ...
        try:
            ...
            raise exceptions.JWKSException(
                self.realm,
                'There was an error while fetching keys for Identity Provider')
