...
class GitlabAPIClient():
    ...
    def set_commit_status(self, project_name, sha, status,
        ...
        for attr in ['ref', 'name', 'target_url', 'description', 'coverage']:
            if hasattr(params, attr) and params['attr']:
                status_body[attr] = params[attr]
        log = get_annotated_logger(self.log, zuul_event_id)
        log.debug(
                "Setting commit status: %s ..." % (status_body))
...
class GitlabConnection(ZKChangeCacheMixin, ZKBranchCacheMixin, BaseConnection):
    ...
    def getCommitStatuses(self, project_name, sha,
                          zuul_event_id=None, **params):
        log = get_annotated_logger(self.log, zuul_event_id)
        statuses = self.gl_client.get_commit_statuses(
            project_name, sha, zuul_event_id, **params
        )
        log.debug("Got %s commit statuses for %s: %s",
                  project_name, sha, statuses)
