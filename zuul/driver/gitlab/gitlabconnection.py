...
class GitlabEventConnector(threading.Thread):
    ...
    def _event_merge_request(self, body):
        ...
        event.updated_at = datetime.strptime(
            attrs['updated_at'], '%Y-%m-%dT%H:%M:%S.%fZ').strftime('%s')
        event.created_at = datetime.strptime(
            attrs['created_at'], '%Y-%m-%dT%H:%M:%S.%fZ').strftime('%s')
...
class GitlabConnection(BaseConnection):
    ...
    def _updateChange(self, change, event):
        ...
        change.updated_at = datetime.strptime(
