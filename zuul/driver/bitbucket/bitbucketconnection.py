...
class BitbucketClient():
	...
	def get(self, path):
		...
		if r.status_code != 200:
			raise BitbucketConnectionError("Connection to server returned status {} path {}".format(r.status_code, url))
...
class BitbucketConnection(BaseConnection):
	...
	def __init__(self, driver, connection_name, connection_config):
		...
		self.server = self.connection_config.get('server', 'bitbucket.smaato.net')
		self.server_git_port = self.connection_config.get('server_git_port', 5999)
		self.server_rest_port = self.connection_config.get('server_rest_port', 443)
		self.server_user = self.connection_config.get('server_user')
		self.server_pass = self.connection_config.get('server_pass')

		self.base_git = 'ssh://git@{}:{}/'.format(self.server, self.server_git_port)
	...
	def _getBitbucketClient(self):
		...
		client = BitbucketClient('https://{}'.format(self.server), self.server_rest_port)
	...
	def getProjectBranches(self, project, tenant):
		...
		res = client.get('/rest/api/1.0/projects/{}/repos/{}/branches'.format(bb_project, repo))
		...
		return [ item.get('displayId') for item in res.get('values') if item.get('type') == 'BRANCH' ]

	def getPR(self, project, repo, id):
		return self._getBitbucketClient()\
		.get('/rest/api/1.0/projects/{}/repos/{}/pull-requests/{}'.format(project, repo, id))

	def getPRs(self, project, repo):
		return self._getBitbucketClient()\
		.get('/rest/api/1.0/projects/{}/repos/{}/pull-requests'.format(project, repo))

	def canMerge(self, change, allow_needs):
		bb_proj, repo = self._getProjectRepo(change.project.name)
		can_merge = self._getBitbucketClient()\
		.get('/rest/api/1.0/projects/{}/repos/{}/pull-requests/{}/merge'.format(bb_proj, repo, change.id))
