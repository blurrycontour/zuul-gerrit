...
class BitbucketWatcher(threading.Thread):
    ...
    def _handleBasePR(self, change):
        ...
        self.bitbucket_con.cachePR(change)
    ...
    def _handleNewPR(self, change):
        ...
        self.log.debug('New open pr event: {}'.format(event))
    ...
    def _handleUpdatePR(self, change, oldchange):
        ...
        self.log.debug('New update pr event {}'.format(event))

        self.bitbucket_con.sched.addEvent(event)

    def _handleOpenChange(self, change):
        self.log.debug('Handling open change {}-{}'
                       .format(change.project.name, change.id))
        if (self.isNew(change) and
                change.updatedDate > self.startup_time * 1000):
        ...
        if oldpr:
            ...
            if (change.canMerge or
                    oldpr.patchset != change.patchset):
    ...
    def _handleBranches(self, project, branches):
        ...
        for bo in branches:
            ...
            if com.get('committerTimestamp') < self.startup_time * 1000:
                self.log.debug('Branch update too old: {}'.format(head))
                continue
...
class BitbucketConnection(BaseConnection):
    ...
    def __init__(self, driver, connection_name, connection_config):
        ...
        canon_hostname = self.connection_config.get('canonical_hostname', None)

        up = urlparse(self.base_url)
        self.server = up.netloc

        if canon_hostname:
            self.canonical_hostname = canon_hostname
        else:
    ...
    def buildPR(self, project, repo, id, cache=True):
        ...
        if (cpr and pr.get('updatedDate') == cpr.updatedDate and
                pr.get('version') == cpr.pr_version and
                cpr.canMerge == self.canMerge(cpr, True)):
            return cpr
