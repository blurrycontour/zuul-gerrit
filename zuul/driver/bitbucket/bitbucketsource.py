...
class BitbucketSource(BaseSource):
    ...
    def getGitUrl(self, project):
        return '{}{}.git'.format(self.connection.base_git, project.name)
    ...
    def getProjectOpenChanges(self, project):
        ...
        return [ self.buildPR(bb_proj, repo, pr.get('id')) for pr in prs.get('values') ]

    def getChangesDependingOn(self, change, projects, tenant):
        return []

    change_re = re.compile(r".*/projects/(.*?)/repos/(.*?)/pull-requests/(\d+)[\w]*")
    ...
    def buildPR(self, project, repo, id):
    	...
    	fromProj = '{}/{}'.format(pr.get('fromRef').get('repository').get('project').get('key'),
    		pr.get('fromRef').get('repository').get('slug'))
    	pull.patchset = self.connection.getBranchSha(fromProj,
    		self.connection.getBranchSlug(fromProj, pr.get('fromRef').get('id')))
