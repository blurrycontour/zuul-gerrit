...
def upgrade(table_prefix=''):
    ...
    if dialect_name == 'mysql':
        binary = sa.BINARY
        unhex = mysql_unhex
        int_to_bin = mysql_int_to_bin
    elif dialect_name == 'postgresql':
        binary = sa.LargeBinary
        unhex = pg_unhex
        int_to_bin = pg_int_to_bin
    else:
        raise Exception("Unsupported dialect {dialect_name}")

    # Normalize data in buildset table
    connection.execute(sa.text(
        f"update {prefixed_buildset} set patchset=null where patchset = ''"
    ))
    connection.execute(sa.text(
        f"update {prefixed_buildset} set oldrev=null where oldrev = ''"
    ))
    connection.execute(sa.text(
        f"update {prefixed_buildset} set newrev=null where newrev = ''"
    ))
    connection.execute(sa.text(
        f"update {prefixed_buildset} set branch=null where branch = ''"
    ))
    connection.execute(sa.text(
        f"update {prefixed_buildset} set ref=null where ref = ''"
    ))
    connection.execute(sa.text(
        f"update {prefixed_buildset} set ref_url=null where ref_url = ''"
    ))
    ...
    op.create_table(
        ...
        sa.Column('patchset', binary(20)),
        sa.Column('oldrev', binary(20)),
        sa.Column('newrev', binary(20)),
        sa.Column('branch', sa.String(255)),
        sa.Column('patchset_temp', sa.String(255)),
        sa.Column('oldrev_temp', sa.String(255)),
        sa.Column('newrev_temp', sa.String(255)),
    )
    ...
    query = f"""
        insert into {prefixed_ref}
            (project, {quote('change')}, patchset_temp,
             ref, ref_url, oldrev_temp, newrev_temp)
        select distinct
            bs.project, bs.change, bs.patchset,
            bs.ref, bs.ref_url, bs.oldrev, bs.newrev
    ...
    query = f"""
        ...
        and ({prefixed_buildset}.patchset = {prefixed_ref}.patchset_temp
             or ({prefixed_buildset}.patchset is null
                 and {prefixed_ref}.patchset_temp is null))
        ...
        and ({prefixed_buildset}.oldrev = {prefixed_ref}.oldrev_temp
             or ({prefixed_buildset}.oldrev is null
                 and {prefixed_ref}.oldrev_temp is null))
        and ({prefixed_buildset}.newrev = {prefixed_ref}.newrev_temp
             or ({prefixed_buildset}.newrev is null
                 and {prefixed_ref}.newrev_temp is null))
    """
    connection.execute(sa.text(query))
    ...
    if dialect_name == 'mysql':
        ...
        query = f"""
        with t as (
            select {prefixed_build}.id as id,
                   {prefixed_buildset_ref}.ref_id as ref_id
            from {prefixed_build}
            inner join {prefixed_buildset}
              on {prefixed_build}.buildset_id = {prefixed_buildset}.id
            inner join {prefixed_buildset_ref}
        ...
        set ref_id = t.ref_id
        from t
        where {prefixed_build}.id=t.id
        """
    else:
        raise Exception("Unsupported dialect {dname}")
    connection.execute(sa.text(query))
    ...
    # Update zuul_ref table to use new column formats
    query = f"""
        update {prefixed_ref}
        set patchset =
            case
              when length(patchset_temp)=40
              then {unhex('patchset_temp')}
              else {int_to_bin('patchset_temp')}
            end,
        oldrev =
            case
              when length(oldrev_temp)=40
              then {unhex('oldrev_temp')}
              else {int_to_bin('oldrev_temp')}
            end,
        newrev =
            case
              when length(newrev_temp)=40
              then {unhex('newrev_temp')}
              else {int_to_bin('newrev_temp')}
            end
    """
    connection.execute(sa.text(query))
    ...
    with op.batch_alter_table(prefixed_ref) as batch_op:
        ...
        batch_op.drop_column('patchset_temp')
        batch_op.drop_column('oldrev_temp')
        batch_op.drop_column('newrev_temp')
        # Create replacement indexes
        batch_op.create_index(
            table_prefix + 'project_change_idx',
            ['project', 'change'])
        batch_op.create_index(
            table_prefix + 'change_idx',
            ['change'])
        # Create the unique constraint across our new columns
        batch_op.create_unique_constraint(
            table_prefix + 'ref_unique',
