...
class SQLReporter(BaseReporter):
    ...
    def createBuildEntry(self, item, job, db_buildset, build):
        ...
        # Create entries for retried builds in a different table with FK
        # relation to the original build's ID
        retry_builds = item.current_build_set.getRetryBuildsForJob(job.name)
        for retry_build in retry_builds:
            result, log_url, start, end = self._getBuildData(
                item, job, retry_build)
            db_buildset.createRetryBuild(
                uuid=build.uuid,
                job_name=build.job.name,
                result=result,
                start_time=start,
