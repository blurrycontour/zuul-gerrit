...
class GitWatcher(threading.Thread):
    ...
    def _run(self):
        ...
        try:
            for project in self.git_connection.projects:
                ...
                self.log.debug("Read refs %s for project %s" % (
                    self.projects_refs[project], project))
                ...
                for event in events:
                    ...
                    if event.type == 'ref-updated':
                        ...
                        self.log.debug(
                            "Change updates config %s" % change.updatesConfig()
                        )
                        if change.updatesConfig():
                            full_project_name = (
                                '/'.join([event.project_hostname,
                                          event.project_name]))
    ...
    def run(self):
        while not self._stopped:
            if not self.git_connection.watcher_pause:
                self._run()
                # Polling wait delay
