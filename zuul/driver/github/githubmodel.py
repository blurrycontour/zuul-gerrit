...
class GithubCommonFilter(object):
    ...
    def matchesReviews(self, change):
        if self.required_reviews or self.reject_reviews:
            ...
            if self.required_reviews and not change.reviews:
                ...
                return "Reviews %s does not match %s" % (
                    self.required_reviews, change.reviews)

        required_reviews_results = self.matchesRequiredReviews(change)
        if required_reviews_results is not True:
            return required_reviews_results
    ...
    def matchesRequiredReviews(self, change):
        for rreview in self.required_reviews:
            ...
            if not matches_review:
                return "Required reviews %s does not match %s" % (
                    self.required_reviews, change.reviews)
        return True
        ...
        for rreview in self.reject_reviews:
            for review in change.reviews:
                if self._match_review_required_review(rreview, review):
                    ...
                    return "Reject reviews %s matches %s" % (
    ...
    def matchesStatuses(self, change):
        if self.required_statuses or self.reject_statuses:
            if not hasattr(change, 'number'):
                # not a PR, no status
                return "Can't match statuses without PR"
            if self.required_statuses and not change.status:
                return "Required statuses %s does not match %s" % (
                    self.required_statuses, change.status)
        required_statuses_result = self.matchesRequiredStatuses(change)
        if required_statuses_result is not True:
            return required_statuses_result
            ...
            for required_status in self.required_statuses:
                for status in change.status:
                    if re2.fullmatch(required_status, status):
                        return True
            return "RequiredStatuses %s does not match %s" % (
                self.required_statuses, change.status)
        return True
        ...
        for rstatus in self.reject_statuses:
            for status in change.status:
                if re2.fullmatch(rstatus, status):
                    return "NoRejectStatuses %s matches %s" % (
...
class GithubEventFilter(EventFilter, GithubCommonFilter):
    ...
    def matches(self, event, change):
        ...
        if self.types and not matches_type:
            return "Types %s doesn't match %s" % (self.types, event.type)
        ...
        if self.branches and not matches_branch:
            return "Branches %s doesn't match %s" % (
        ...
        if self.refs and not matches_ref:
            return "Refs %s doesn't match %s" % (self.refs, event.ref)
        if self.ignore_deletes and event.newrev == EMPTY_GIT_REF:
            # If the updated ref has an empty git sha (all 0s),
            # then the ref is being deleted
            return "Ref deletion are ignored"
        ...
        if self.comments and not matches_comment_re:
            return "Comments %s doesn't match %s" % (
        ...
        if self.actions and not matches_action:
            return "Actions %s doesn't match %s" % (self.actions, event.action)

        # labels are ORed
        if self.labels and event.label not in self.labels:
            return "Labels %s doesn't match %s" % (self.labels, event.label)

        # unlabels are ORed
        if self.unlabels and event.unlabel not in self.unlabels:
            return "Unlabels %s doesn't match %s" % (
                self.unlabels, event.unlabel)

        # states are ORed
        if self.states and event.state not in self.states:
            return "States %s doesn't match %s" % (self.states, event.state)
        ...
        if self.statuses:
            ...
            for status in self.statuses:
                if re2.fullmatch(status, event.status):
                    ...
                    break
            if not status_found:
                return "Statuses %s doesn't match %s" % (
...
class GithubRefFilter(RefFilter, GithubCommonFilter):
    ...
    def matches(self, change):
        ...
        if statuses_result is not True:
