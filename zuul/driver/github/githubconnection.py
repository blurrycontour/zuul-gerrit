...
class CachedGithubTeam(object):
    ...
    def from_github3_team(cls,
        ...
        # Note(tobiash): We explicitly get the members via the API ourselves
        # because we need to also get the subteam members which are not
        # not returned when we use team.members().
        url = team.members_urlt.expand()
        # This header is needed to get the subteam members.
        headers = {'Accept': 'application/vnd.github.hellcat-preview+json'}
        params = {'per_page': 100}
        members = []
        while url:
            resp = team.session.get(
                url, headers=headers, params=params)
            url = resp.links.get('next', {}).get('url')
            members.extend([x['login'] for x in resp.json()])

        return cls(team.slug, members)
...
class GithubConnection(BaseConnection):
    ...
    def _getCachedGithubTeam(self,
        ...
        if cached_team_name not in self._team_cache:
            org_name, repo = project_name.split('/')
            ...
            for team in gh_repo.teams():
                ...
                if cached_team_name not in self._team_cache:
                    ...
                    full_team_name = '%s/%s' % (team_org, cached_team.name)
                    self._team_cache[full_team_name] = cached_team
    ...
    def _hasCodeownersReview(self,
        ...
        codeowners = self._getCodeowners(project_name, base_branch)
            ...
            """
                query {{
                  organization(login: "{org}") {{
                    # [X] Gets only teams containing a substring
                    ...
                    #teams(first: 100, {team_after}) {{
                      ...
                      edges {{
                        ...
                        node {{
                          slug
                          # [X] Gets only repos containing a substring
