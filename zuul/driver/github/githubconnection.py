...
from zuul.model import ConnectionEvent, Ref, Branch, Tag, Project
...
class GithubEventProcessor(object):
    def __init__(self, connector, event_tuple: EventTuple):
...
class GithubEventConnector:
    ...
    def _on_new_event(self) -> Optional[bool]:
        self._process_event.set()
        # Stop the data watch in case the connector was stopped
        return not self._stopped

    def run_event_dispatcher(self):
        self.event_queue.register_event_watch(self._on_new_event)
        while not self._stopped:
            try:
                self.event_queue.election.run(self._runEventDispatcherInner)
            except Exception:
    ...
    def _runEventDispatcherInner(self):
        while not self._stopped:
            for event in self.event_queue:
                try:
                    etuple = self._event_as_tuple(event)
                    ...
                    processor = GithubEventProcessor(self, etuple)
    ...
    @staticmethod
    def _event_as_tuple(event: ConnectionEvent) -> EventTuple:
        body = event.get("body")
