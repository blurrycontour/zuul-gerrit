...
class GithubConnection(BaseConnection):
    ...
    def _hasRequiredStatusChecks(self, allow_needs, protection, pull):
        ...
        successful = set([s.context for s in commit.status().statuses
                          if s.state == 'success'])
        # Successful check_runs are listed in Github's status section below the
        # comments of a PR, but not listed via its status API. So we have to
        # retrieve them via the checks API.
