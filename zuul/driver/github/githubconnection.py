...
class GithubConnection(BaseConnection):
    ...
    def _updateChange(self, change, zuul_event_id=None):
        ...
        change.status = self._get_statuses(
            change.project, change.patchset, zuul_event_id=zuul_event_id)
        change.reviews = self.getPullReviews(
            pr_obj, change.project, change.number, zuul_event_id=zuul_event_id)
    ...
    def getPullReviews(self, pr_obj, project, number, zuul_event_id=None):
        log = get_annotated_logger(self.log, zuul_event_id)
    ...
    def _get_statuses(self, project, sha, zuul_event_id=None):
        ...
        for status in self.getCommitStatuses(
                project.name, sha, zuul_event_id=zuul_event_id):
