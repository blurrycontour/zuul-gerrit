...
class GithubConnection(BaseConnection):
    ...
    def _fetchGithubTeam(self, project_name, team_slug, event):
        ...
        url = github.session.build_url(
            'graphql', base_url=self._github_client_manager.api_base_url)
    ...
    def canMerge(self, change, allow_needs, event=None):
        ...
        # Legacy codeowners handling is necessary of review_decision is None
        # (unsupported or no review required) and the branch protection
        # requires a review.
        if review_decision is None and canmerge_data.get(
                ...
                if member_name == user_name:
                    for team_repository in team_repositories:
                        ...
                        if repository_name == repository:
                            ...
                            key = 'u:{}'.format(member_name)
                            self._repo_access_cache[project][key] = perm

                    # Since query in "repositories(...)" is matching substrings
                    # so multiple hits may occur. The maximum number of items
                    # in a result is 100. There may be a theoretical case that
                    # a *complete* repository name may be a substring of more
                    # than 100 other repository name.
                    #
                    # E.g.:
                    # - repository-name
                    # - repository-name-a
    ...
    def get_project_member_permission(self, project_name, user_name):
        key = 'u:{}'.format(user_name)  # differentiate between user and team
            ...
            if team_name:
                for team_repository in team_repositories:
                    ...
                    if repository_name == repository:
                        ...
                        key = 't:{}'.format(team_slug)  # differentiate u/t
                        self._repo_access_cache[project][key] = perm
    ...
    def get_project_team_permission(self, project_name, team_slug):
        key = 't:{}'.format(team_slug)  # differentiate between user and team
        if project_name not in self._repo_access_cache:
            self._repo_access_cache[project_name] = {}
        if key not in self._repo_access_cache[project_name]:
            self._repo_access_cache[project_name][key] = 'none'
