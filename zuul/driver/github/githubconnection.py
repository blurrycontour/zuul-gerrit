...
class GithubRateLimitHandler:
    ...
    def handle_response(self, response, *args, **kwargs):
        ...
        # There is no rate limit information we have nothing to do here.
        if rate_limit:
...
class GithubRetryHandler:
    ...
    def handle_response(self, response, *args, **kwargs):
        ...
        self.log.warning('GET Request failed with %s (%s/%s retries), retrying '
                         'in %s seconds', response.status_code, retry_count,
...
class GithubConnection(BaseConnection):
    ...
    def getPullBySha(self, sha, project_name, event):
        ...
        try:
            ...
            log.debug('Found no PR matching sha %s', sha)
            return None
