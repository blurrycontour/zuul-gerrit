...
class GithubChangeCache(AbstractChangeCache):
    ...
    def __init__(self, client, connection):
        ...
        self.connection_registry = connection.sched.connections
    ...
    def _changeFromData(self, data):
        ...
        pr = PullRequest(project.name, self.connection_registry)
        pr.deserialize(data)
        pr.project = project
        return pr
...
class GithubConnection(CachedBranchConnection):
    ...
    def updateChangeAttr(self, change, attr_name, value):
        while True:
            cache_stat = change.cache_stat
            setattr(change, attr_name, value)
            try:
    ...
    def _getChange(self, project, number, patchset=None, refresh=False,
        ...
        if change and not refresh:
            return change
        if not change:
            change = PullRequest(project.name, self.sched.connections)
