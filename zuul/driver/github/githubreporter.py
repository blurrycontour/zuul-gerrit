        ...
        for i in range(1, max_retries + 1):
            try:
                ...
                log.exception('Merge attempt of change %s  %s/%s failed.',
                              item.change, i, max_retries, exc_info=True)
                error_message = str(e)
                if i < max_retries:
                    time.sleep(5)
