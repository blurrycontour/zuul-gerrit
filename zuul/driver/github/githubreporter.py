...
class GithubReporter(BaseReporter):
    ...
    def __init__(self, driver, connection, pipeline, config=None):
        ...
        self._checks_api = self.config.get('checks-api', False)
    ...
    def getSubmitAllowNeeds(self):
        ...
        # check if we report a status or a check_run, if not we can return an
        # empty list
        status = self.config.get('status')
        check_run = self.config.get("checks-api")
        if not any([status, check_run]):
            return []
...
def getSchema():
    github_reporter = v.Schema({
        ...
        'review-body': str,
        'checks-api': v.Any("in_progress", "success", "failure"),
    })
