...
class WebTriggerFilter(EventFilter):
    ...
    def matches(self, event, change):
        if event.type == 'web':
            # Store event's job filter for job freeze graph
            change.job_filter = event.job_filter
            # Generate a new rev for each trigger
            change.newrev = "%040d" % WebTriggerFilter.counter
            WebTriggerFilter.counter += 1
            return True
        return False
