...
class PagureZookeeperWorker(threading.Thread):
    ...
    def __init__(self, connection):
        ...
        self._stopped = False
        self._connection = connection  # PagureConnection
    ...
    def _processEvent(self) -> None:
        with self._zk.connection_event.pop(
            ...
            for event in events:
                ...
                self.log.info(
                    "Pagure Webhook Received (id: %(msg_id)s, topic:"
                    " %(topic)s)" % payload)

                try:
                    self._dispatchEvent(payload)
                except Exception as e:
                    self.log.exception(
                        "Exception handling Pagure event:", e)

    def _dispatchEvent(self, payload) -> None:
        event = payload['topic']
        try:
            self.log.info("Dispatching event %s" % event)
            self._connection.addEvent(payload, event)
...
class PagureConnection(BaseConnection):
    ...
    def __init__(self, driver, connection_name, connection_config):
        ...
        self.event_queue = NamedQueue('PagureEventQueue<%s>' % connection_name)
        self.metadata_notif = re.compile(
            r"^\*\*Metadata Update", re.MULTILINE)
        self.sched = None
