...
class PagureAPIClient():
    ...
    def get_pr_flags(self, project, number):
    ...
    # def set_status_on_pr_tip(self, project, pr, status):
    #     pr = self.get_pr(project, pr)
    #     tip_sha = pr['commit_start']
    #     self.set_commit_flag(project, tip_sha, status)
    #
    # def get_status_on_pr_tip(self, project, pr):
    #     pr = self.get_pr(project, pr)
    #     tip_sha = pr['commit_start']
    #     flags = self.get_commit_flags(project, tip_sha)
    #     if not flags:
    #         return
    #     last_flag = None
    #     for flag in flags:
    ...
    #             last_flag = flag
    #         else:
    #             if flag['date_created'] > last_flag['date_created']:
    #                 last_flag = flag
    #     return last_flag
...
class PagureConnection(BaseConnection):
    ...
    def _getChange(self, project, number, patchset=None, refresh=False):
        ...
        self.log.info("Getting change from cache or from pagure %s" % str(key))
    ...
    def getStatuses(self, project, number):
        statuses = self.getCommitStatuses(project.name, number)
        return statuses
    ...
    def getScore(self, pr):
        ...
        for comment in pr.get('comments', []):
            ...
            if comment['notification']:
                ...
                if date > last_pr_updated:
                    last_pr_updated = date
        ...
        for comment in pr.get('comments', []):
            ...
            if date >= last_pr_updated:
                ...
                if comment.get('comment', '').find(':thumbsup:') >= 0:
                    score_board.setdefault(author, 0)[author] += 1
    ...
    def _updateChange(self, change):
        ...
        self.log.info('PR score: %s' % change.score)
        # change.labels = change.pr.get('labels')
        # ensure message is at least an empty string
        change.message = change.pr.get('body') or ''
        # TODO(fbo): Not sure about the date un last_updated (seems weird)
    ...
    def getCommitStatuses(self, project, number):
        statuses = self.pagure.get_pr_flags(project, number)
        self.log.info("Got pull-request CI flags for PR %s on %s", (
            number, project))
        return statuses
