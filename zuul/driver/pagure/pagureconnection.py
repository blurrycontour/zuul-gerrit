...
# README:
#
# The actual state of the driver support a check/gate/post workflow for a
# single repository. See the pipeline below:
#
# - pipeline:
#     name: check
#     manager: independent
...
# CI flagged, and merged. A PR can be flagged (success/failure/pending) and this
# driver use that capability. Code review (evaluation) is done via comments
# that contains a :thumbsup: or :thumbsdown: emoji. Pagure computes a score
# based on that and allow or not the merge of PR if the "minimal score to merge"
# is set in repository settings. This driver uses that feature that need to be
# set. This driver expects to receive repository events via webhooks and expects
# to verify payload signature. Also it requires a repository API key to perform
# some authenticated calls like merging a PR.
#
...
# - API token, webhook target and hook payload signature are by repository
#   making hard to support multiple repositories like in Gerrit or Github.
#   An idea could be to group projects and set those settings ate group level.
#   RFE: https://pagure.io/pagure/issue/3948
# Cuurent Non blocking issues:
...
#   of the merged PR with files list details.
...
# TODO: Use BaseConnection.logEvent
...
class PagureEventConnector(threading.Thread):
    ...
    def _event_ref_updated(self, body):
        ...
        event.newrev = data.get('start_commit')
        # oldrev not sent by pagure, try that not sure how it is handle
        # in the scheduler
        # event.oldrev = data.get('start_commit')
        event.branch_updated = True

        # TODO(fbo): Pagure does not send an event when a branch is created
...
class PagureAPIClient():
    ...
    def __init__(self, baseurl, api_token=None):
    ...
    def get(self, url):
        self.log.debug("Getting resource %s, headers (%s) ..." % (
            url, self.headers))
    ...
    def post(self, url, params=None):
        self.log.info(
            "Posting on resource %s, params (%s), headers (%s) ..." % (
                url, params, self.headers))
    ...
    def create_project_api_token(self, project):
    ...
    def get_connectors(self, project):
        ...
        # Now following the patern zuul-token-{epoch} find the last one created
        api_token = None
        for token in tokens:
            if not token['description'].startswith('zuul-token-'):
                continue
            epoch = int(token['description'].split('-')[-1])
            if api_token:
                if epoch > int(api_token['description'].split('-')[-1]):
                    api_token = token
            else:
                api_token = token
        if not api_token:
            # Let's create one
            api_token = self.create_project_api_token(project)
...
class PagureConnection(BaseConnection):
    ...
    def __init__(self, driver, connection_name, connection_config):
        ...
        self.pagure = PagureAPIClient(self.baseurl, api_token=self.api_token)
    ...
    def _refresh_project_connectors(self, project):
        api_token, webhook_token = self.pagure.get_connectors(project)
        connector = self.connectors.setdefault(
            project, {'api_client': None, 'webhook_token': None})
        connector['api_client'] = PagureAPIClient(self.baseurl, api_token)
    ...
    def getChange(self, event, refresh=False):
        ...
        if event.change_number:
            ...
            change.uris = [
                '%s/%s/pull/%s' % (self.baseurl, project, change.number),
            ]
            ...
            # Instead refresh the repository and check for conf files
            # TODO(fbo): Propose upstream change to pagure upstream
            job = self.sched.merger.getFiles(
                self.connection_name, project.name, change.branch,
                files=['zuul.yaml', '.zuul.yaml'], dirs=['zuul.d', '.zuul.d'])
            self.log.debug("Submitting cat job %s for %s %s %s" % (
                job, self.connection_name, project, change.branch))
            self.log.debug("Waiting for cat job %s" % job)
            job.wait()
            if not job.updated:
                raise Exception("Cat job %s failed" % job)
            self.log.debug("Cat job %s got files %s" % (job, job.files.keys()))
            change.files = job.files.keys()
    ...
    def _hasRequiredStatusChecks(self, change):
        flag = self.pagure.get_pr_flags(
            change.project.name, change.number, last=True)
        return True if flag.get('status', '') == 'success' else False

    def canMerge(self, change, allow_needs):
        pr = self.pagure.get_pr(change.project.name, change.number)
    ...
    def getCommitStatus(self, project, number):
        flag = self.pagure.get_pr_flags(project, number, last=True)
        self.log.info(
