...
# comments that contains a :thumbsup: or :thumbsdown: emoji. Pagure computes a
# score based on that and allow or not the merge of PR if the "minimal score to
# merge" is set in repository settings. This driver uses that feature that need
# to be set. This driver expects to receive repository events via webhooks and
# expects to verify payload signature. Also it requires a repository API key to
# perform some authenticated calls like merging a PR.
#
...
# TODO: Fix issue: PR with .zuul.yaml changes
# on commit 1
# check: jobs [noop]
# on commit 2:
# check: jobs [noop2]
...
class PagureAPIClient():
    ...
    def __init__(self, baseurl, api_token, project, token_exp_date=None):
...
class PagureConnection(BaseConnection):
    ...
    def _refresh_project_connectors(self, project):
        pagure = PagureAPIClient(
            self.baseurl, api_token=self.admin_api_token, project=project)
    ...
    def get_project_api_client(self, project):
        ...
        self.log.debug("%s" % self.connectors)
    ...
    def _updateChange(self, change):
        ...
        self.log.debug("Change message: %s" % change.message)
