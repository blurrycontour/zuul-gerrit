...
class PagureEventConnector(threading.Thread):
    ...
    def _event_tag_created(self, body):
        ...
        event.oldrev = None
        # Pagure does not send the tag sha in the event
        # Instead use a gear job to fetch it from executor or merger
        event.newrev = self.connection.getCommitFromRef(
            event.project_name, event.tag)
        return event
...
class PagureConnection(BaseConnection):
    ...
    def getCommitFromRef(self, project_name, refname):
        job = self.sched.merger.getCommitFromRef(
            self.connection_name, project_name, refname)
        self.log.debug("Waiting for getcommitfromref job %s" % job)
        job.wait()
        if not job.updated:
            raise Exception("Getcommitfromref job %s failed" % job)
        self.log.debug("Getcommitfromref job %s got commit sha %s" %
                       (job, job.commit))
        return job.commit
