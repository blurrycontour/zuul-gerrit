...
class PagureConnection(BaseConnection):
    ...
    def _refresh_project_connectors(self, project):
        pagure = PagureAPIClient(
            self.baseurl, api_token=self.admin_api_token)
    ...
    def getChange(self, event, refresh=False):
        ...
        if event.change_number:
            ...
            files = [path for path, data in job.files.items() if
                     data is not None]
            self.log.debug("Cat job %s got files %s" % (job, files))
            change.files = files
    ...
    def getChangesDependingOn(self, change, projects, tenant):
        ...
        # a str in commit_stop commit msg or PR message body. Not a blocker
        # for now.
        return []
