...
class GerritKafkaEventListener:
    log = logging.getLogger("gerrit.KafkaEventListener")
    ...
    def __init__(self, gerrit_connection, connection_config):
        ...
        client_id = connection_config.get('kafka_client_id', None)
        if client_id:
            kafka_config['client.id'] = client_id
        group_id = connection_config.get('kafka_group_id', None)
        if group_id:
        ...
        self.topic = connection_config.get('kafka_topic')
            ...
            while not self._stopped:
                ...
                if msg.error():
                    if msg.error().code() == kafka.KafkaError._PARTITION_EOF:
                        self.log.info(
                            'Kafka topic %s partition %d reached end at offset %d',
