...
class GerritSource(BaseSource):
    ...
    def _parseURL(self, url):
        try:
            ...
            return None
        if parsed.hostname.lower() != self.canonical_hostname.lower():
            return None
    ...
    def getCanonicalProjectNameByURL(self, url):
        num = self._parseURL(url)
        if num is None:
            return None
        # If we have this number in the change cache, return its project
        patchsets = self.connection._change_cache.get(num)
        if patchsets:
            for change in patchsets.values():
                return change.project.canonical_name
        # This change hasn't appeared yet.  Since we're trying to find
        # the correct source to use to look up a change, we need to
        # query it here without saving any data (since this may not be
        # the correct source to use for the project).
        query = "change:%s" % (num,)
        results = self.connection.simpleQuery(query)
        if not results:
            return None
        for result in results:
            return '%s/%s' % (self.connection.canonical_hostname,
                              result['project'])
        return None
