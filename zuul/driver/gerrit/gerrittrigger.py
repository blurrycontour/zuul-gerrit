    ...
    def getEventFilters(self, connection_name, trigger_conf,
        ...
        for trigger in to_list(trigger_conf):
            ...
            with pcontext.confAttr(trigger, 'comment') as attr:
                if not attr:
                    with pcontext.confAttr(trigger, 'comment_filter') as attr:
                        comments = [make_regex(x, pcontext)
                                    for x in to_list(attr)]
            with pcontext.confAttr(trigger, 'email') as attr:
                if not attr:
                    with pcontext.confAttr(trigger, 'email_filter') as attr:
                        emails = [make_regex(x, pcontext)
                                  for x in to_list(attr)]
            with pcontext.confAttr(trigger, 'username') as attr:
                if not attr:
                    with pcontext.confAttr(
                            trigger, 'username_filter') as attr:
