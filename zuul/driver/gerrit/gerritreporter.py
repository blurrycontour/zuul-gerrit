...
class GerritReporter(BaseReporter):
    ...
    def report(self, item):
        ...
        # NOTE (felix): Is anything else from the config of interest for the
        # later gerrit action? Otherwise we can just evaluate the submit flag
        # here.
        ...
        if self.config.get("submit", False):
            ...
            if direct_push:
                ...
                # We don't want the gerrit review to automatically submit the
                # change, thus we simply overwrite the submit flag for the
                # gerrit review.
                action.pop("submit")
                #action["submit"] = False
