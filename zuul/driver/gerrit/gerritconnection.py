    ...
    def parseRelatedHTTP(self, data, related):
        ...
        for change in related['changes']:
            for parent in current_rev['commit']['parents']:
                if change['commit'] == parent['commit']:
...
class GerritConnection(BaseConnection):
    ...
    def queryChangeHTTP(self, number, event=None):
        data = self.get('changes/%s?o=DETAILED_ACCOUNTS&o=CURRENT_REVISION&o=CURRENT_COMMIT&o=CURRENT_FILES&o=LABELS&o=DETAILED_LABELS' % (number,))
        related = self.get('changes/%s/revisions/%s/related' % (number,data['current_revision']))
        return data, related
    ...
    def simpleQueryHTTP(self, query, event=None):
        ...
        while not done:
            ...
            q = 'changes/?n=500%s&o=CURRENT_REVISION&o=CURRENT_COMMIT&q=%s' % (sortkey, query)
