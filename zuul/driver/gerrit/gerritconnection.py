...
class GerritEventConnector(threading.Thread):
    ...
    def _getChange(self, event):
        ...
        if event.change_number:
            refresh = True
            if not self.connection.changeInCache(event.change_number):
                refresh = False
...
class GerritConnection(BaseConnection):
    ...
    def changeInCache(self, change_id: str) -> bool:
        # The cache stores a patchset with the key "change_id,patchset",
        # however sometimes we want to know if any version of a change exists
        # in the cache
        changes = set(key.split(',')[0] for key in self._change_cache.keys())
        return change_id in changes
