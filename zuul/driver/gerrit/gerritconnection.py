...
class GerritConnection(BaseConnection):
    ...
    def isMerged(self, change, head=None):
        ...
        if not hasattr(change, '_ref_sha'):
            log.error("Unable to confirm change in git repo: the change has "
                      "not been reported.  This pipeline may be "
                      "misconfigured (check for multiple Gerrit connections).")
            return False
