...
class GerritChangeCache(AbstractChangeCache):
    ...
    def _changeFromData(self, data):
        ...
        change = GerritChange(project, self.connection_registry)
...
class GerritEventConnector(threading.Thread):
    ...
    def _getChange(self, event):
        ...
        if event.change_number:
            refresh = True
            # FIXME: Clarify why we only check for the change number and not
            # for the patchset. Why are we ok with an old patchset of a change?
            # For the moment this will always lead to a refresh.
...
class GerritConnection(BaseConnection):
    ...
    def _getChange(self, number, patchset, refresh=False, history=None,
        ...
        if change and not refresh:
            return change
        if not change:
            change = GerritChange(None, self.sched.connections)
    ...
    def _updateChangeDependencies(self, log, change, data, event, history):
        ...
        self.updateChangeAttr(change, "git_needs_changes", git_needs_changes)
        ...
        for (dep_num, dep_ps) in self._getDependsOnFromCommit(
            ...
            if dep.open and dep not in needs_changes:
                ...
                needs_changes.add(dep)
        self.updateChangeAttr(change, "compat_needs_changes",
                              compat_needs_changes)
        ...
        for (dep_num, dep_ps) in data.needed_by:
            try:
                ...
                log.exception("Failed to get git-needed change %s,%s",
                              dep_num, dep_ps)
        self.updateChangeAttr(change, "git_needed_by_changes",
                              git_needed_by_changes)
        ...
        for (dep_num, dep_ps) in self._getNeededByFromCommit(
            ...
            try:
                ...
                log.exception("Failed to get commit-needed change %s,%s",
                              dep_num, dep_ps)
        self.updateChangeAttr(change, "compat_needed_by_changes",
                              compat_needed_by_changes)
