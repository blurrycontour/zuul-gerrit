...
class GerritChangeData(object):
    """Compatability layer for SSH/REST

    This class holds the raw data returned from a change query over
    SSH or REST.  Most of the work of parsing the data and storing it
...
class GerritConnection(BaseConnection):
    ...
    def __init__(self, driver, connection_name, connection_config):
        ...
        self.enable_stream_events = self.connection_config.get(
            'stream_events', True)
        if self.enable_stream_events not in ['true', 'True', '1', 1, 'TRUE']:
    ...
    def review_http(self, item, message, action={},
        ...
        changeid = "%s~%s~%s" % (
            ...
            change.id)
        if 'checks_plugin' in action:
            self.report_checks(log, item, changeid, action['checks_plugin'])
    ...
    def queryChangeSSH(self, query, event=None):
    ...
    def queryChangeHTTP(self, query, event=None):
        x = self.get('changes/?q=%s&o=DETAILED_ACCOUNTS&o=CURRENT_REVISION&o=CURRENT_COMMIT&o=CURRENT_FILES&o=LABELS&o=DETAILED_LABELS')

        # for submittable: iterate over labels
        # if optional, skip
        # if label is allow_needs: skip
        # if blocking: not submittable
        # if approved: submittable
