...
class Executor(zuul.cmd.ZuulDaemonApp):
    ...
    def start_log_streamer(self):
        ...
        if child_pid == 0:
            ...
            self.log.info("Starting log streamer on port %s",
                          self.finger_port)
    ...
    def run(self):
        ...
        with zuul.zk.ZooKeeperConnection.fromConfig(self.config) as zookeeper:

            self.executor_server = ExecutorServer(
                self.config, zookeeper, self.connections,
