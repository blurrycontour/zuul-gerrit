...
class Server(object):
    ...
    def test_config(self, function_list_path):
        ...
        layout = self.sched.testConfig(self.config.get('zuul',
                                                       'layout_config'))
        if not function_list_path:
            return False

        failure = False
        path = os.path.expanduser(function_list_path)
        if not os.path.exists(path):
            raise Exception("Unable to find function list: %s" % path)
        jobs = set()
