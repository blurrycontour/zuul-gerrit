...
class ZuulRESTClient(object):
    ...
    def __init__(self, address, port=None, auth_token=None):
...
class Client(zuul.cmd.ZuulApp):
    ...
    def main(self):
        ...
        if 'gearman' in conf_sections:
            ...
            self.client = zuul.rpcclient.RPCClient(
                self.server, self.port, self.ssl_key,
                self.ssl_cert, self.ssl_ca)
        elif 'web' in conf_sections:
            self.log.debug('web section found in config, using REST client')
            self.server = self.config.get('web', 'listen_address',
                                          'https://zuul.openstack.org')
            self.port = self.config.get('web', 'port', None)
            self.client = ZuulRESTClient(self.server, self.port,
    ...
    def autohold_list(self):
        autohold_requests = self.client.autohold_list(tenant=self.args.tenant)
    ...
    def create_auth_token(self):
        ...
        token = {'exp': time.time() + self.args.expires_in,
                 ...
                 'sub': self.args.user,
                 'zuul.actions': {}}
        if any(a not in ADMIN_ACTIONS for a in self.args.action):
            print('Actions must be any of %s' % ', '.join(ADMIN_ACTIONS))
            sys.exit(1)
        for action in self.args.action:
            token['zuul.actions'][action] = {
                self.args.tenant: self.args.project}
        driver = get_default(
            self.config, auth_section, 'driver')
        if driver == 'HS256':
            key = get_default(self.config, auth_section, 'secret')
        elif driver == 'RS256':
            public_key = get_default(self.config, auth_section, 'public_key')
    ...
    def show_running_jobs(self):
        running_items = self.client.get_running_jobs()
