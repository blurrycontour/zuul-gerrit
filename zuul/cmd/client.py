        ...
        if self.args.func == self.dequeue:
            ...
            if sum([1 and (x is not None) or 0
                ...
                parser.error(
                    ...
                    "mutually exclusive.")
            if any(x is None for x in (self.args.tenant,
                                       self.args.pipeline,
                                       self.args.project)) and\
               self.args.buildset_id is None:
                parser.error(
                    "Tenant, pipeline and project are required with a %s."
                    % (self.args.ref is not None and "ref" or "change id"))
