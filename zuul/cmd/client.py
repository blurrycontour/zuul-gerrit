...
class Client(zuul.cmd.ZuulApp):
    ...
    def get_client(self):
        ...
        if self.args.zuul_url:
            ...
            client = ZuulRESTClient(self.args.zuul_url, None,
                                    not self.args.insecure_ssl,
                                    self.args.auth_token)
        elif 'gearman' in conf_sections:
            ...
            server = get_default(self.config, 'web', 'listen_address',
                                 'https://zuul.openstack.org')
            port = get_default(self.config, 'web', 'port', None)
            # todo this is not an original option in the web section
            verify = get_default(self.config, 'web', 'verify',
                                 not self.args.insecure_ssl)
