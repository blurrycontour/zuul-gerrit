...
class Job:
    ...
    def _makeSCPTask(self, publisher):
        ...
        if artifacts:
            tasks.insert(0, collections.OrderedDict(
    ...
    def _makeBuilderTask(self, playbook_dir, builder, sequence):
        ...
        return collections.OrderedDict(
            name='Builder script {sequence} translated from {old}'.format(
                sequence=sequence, old=self.orig),
            script=script_fn,
    ...
    def toJobDict(self, artifacts=True):
    ...
    def writeJobs(self):
        ...
        for job in self.job_objects:
            if (job.name not in seen_jobs
                ...
                artifacts = job.emitPlaybooks(self.outdir)
                job_config.append({'job': job.toJobDict(artifacts)})
