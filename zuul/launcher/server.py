...
class Launcher:
    ...
    def _getWakeEventTimeout(self):
        if self.next_wake is None:
            ...
            return self.next_wake - time.time()
    ...
    def run(self):
        ...
        while self._running:
            ...
            timeout = self._getWakeEventTimeout()
            if timeout is not None:
                self.log.debug("Delaying next iteration by %ss", timeout)
    ...
    def _selectProviders(self, request, log):
        ...
        for i, label in enumerate(request.labels):
            provider_attempts = collections.Counter()
            if len(request.provider_nodes) > i:
                nodes = [
                    self.api.getProviderNode(n)
                    for n in request.provider_nodes[i]
                ]
                provider_attempts.update(n.provider for n in nodes)

            candidate_providers = [
                p for p in providers
                if p.hasLabel(label)
                and provider_attempts[p.name] < p.launch_retries
            ]
        ...
        for tenant_name, tenant_providers in self.tenant_providers.items():
            ...
            for provider in tenant_providers:
                for label in provider.labels.values():
                    ...
                    for i in range(len(label_unassigned), label.min_ready):
                        ...
                        for score, launcher_name in scores:
                            ...
                            if launcher.hostname == self.component_info.hostname:
                                # TODO: we need to create a provider node
                                node_uuid = uuid.uuid4().hex
                                # label = provider.labels[label_name]
                    ...
                    # Check if we have enough min-ready nodes
