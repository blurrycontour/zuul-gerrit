...
class JobDir(object):
    def __init__(self):
        ...
        self.callback_plugins_root = os.path.join(self.plugins_root, 'callback_plugins')
        os.makedirs(self.callback_plugins_root)
    ...
    def stopJob(self, job):
        ...
        try:
            if node.isAlive():
                ...
                self.log.debug("Node %s is not alive while aborting job" % (node.name,))
        except Exception:
            self.log.exception("Exception sending abort command to worker:")
...
class NodeWorker(object):
    ...
    def prepareAnsibleFiles(self, jobdir, gearman_job, args):
        ...
        with open(jobdir.playbook, 'w') as playbook:
            ...
            task = dict(file=dict(path=parameters['WORKSPACE'], state='directory'))
            tasks.append(task)
        ...
        with open(jobdir.config, 'w') as config:
            ...
            callback_path = '%s:/usr/share/ansible_plugins/callback_plugins' % (callback_path,)
            config.write('callback_plugins = %s\n' % callback_path)

            library_path = zuul.ansible.library.__file__
            library_path = os.path.abspath(library_path)
            library_path = os.path.dirname(library_path)
            library_path = '%s:/usr/share/ansible' % (library_path,)
    ...
    def runAnsiblePlaybook(self, jobdir, timeout):
        self.ansible_proc = subprocess.Popen(
            ['ansible-playbook', jobdir.playbook, '-e', 'timeout=%s' % timeout, '-v'],
