...
class RPCListener(object):
    ...
    def handle_project_freeze_jobs(self, gear_job):
        ...
        for job in item.job_graph.getJobs():
            ...
            job_output = job.toDict(tenant)
            for stage in ['pre_run', 'run', 'post_run']:
                job_output[stage] = []
                if job._get(stage) is not None:
                    for playbook in job._get(stage):
                        job_output[stage].append(
                            playbook.toDict(redact_secrets=True))
