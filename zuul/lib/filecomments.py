...
def extractLines(file_comments):
    ...
    lines = set()
    for path, data in file_comments.items():
        if 'line' in data:
            lines.add((path, int(data['line'])))
        if 'range' in data:
            rng = data['rng']
            for key in ['start_line', 'start_character', 'end_line',
                        'end_character']:
                if key in rng:
...
def updateLines(file_comments, lines):
    """Update the line numbers in file_commets with the supplied mapping"""

    for path, data in file_comments.items():
        if 'line' in data:
            data['line'] = lines.get((path, data['line']), data['line'])
        if 'range' in data:
            rng = data['rng']
            for key in ['start_line', 'start_character', 'end_line',
                        'end_character']:
