...
class KeyStorage(ZooKeeperBase):
    ...
    def getProjectSecretsKeys(self, connection_name, project_name):
        """Return the public and private keys of the project"""
    ...
    def rotateOidcSigningKeys(self, algorithm, rotation_interval, max_ttl):
        ...
        It creates a new key and/or remove the older keys if necessary.
        """

        self.log.debug("Rotating OIDC signing keys, algorithm: %s,"
                       "rotation_interval: %s, max_ttl: %s",
                       algorithm, rotation_interval, max_ttl)
        key_data = self.loadProjectsSecretsKeys("oidc", algorithm)

        if not key_data:
            self._createOidcSigningKeys(algorithm)
        else:
            ...
            for index in range(len(key_data['keys']) - 1, -1, -1):
                ...
                if (key['created'] < (int(time.time()) - max_ttl)
                    and index > 0):
    ...
    def getOidcSigningKeyData(self, algorithm, rotation_seconds=86400):
        """Return the public and private keys for OIDC"""

        # Here reuse loadProjectsSecretsKeys() and consider "oidc" as
        # a special connection_name and algorithm as a special project_name
        oidc_signing_keys = self.loadProjectsSecretsKeys(
            "oidc", algorithm)

        if not oidc_signing_keys:
            self._createOidcSigningKeys(algorithm)
            oidc_signing_keys = self.loadProjectsSecretsKeys("oidc", algorithm)

        return oidc_signing_keys

    def getLatestOidcSigningKeys(self, algorithm):
        """Return the latest public and private keys for OIDC"""
        signing_key_data = self.getOidcSigningKeyData(algorithm=algorithm)
        pem_private_key = \
            signing_key_data['keys'][0]["private_key"].encode("utf-8")
    ...
    def deleteOidcSigningKeys(self, algorithm):
        """
        Delete the OIDC signing keys.
        This is useful when the OIDC signing keys are compromised and needs to
        be deleted and recreated via zuul-admin command.
        """
        self.deleteProjectsSecretsKeys("oidc", algorithm)

    def _createOidcSigningKeys(self, algorithm):
        ...
        if keyDict:
            ...
            try:
                self.saveProjectsSecretsKeys("oidc", algorithm, keydata)
            except kazoo.exceptions.NodeExistsError:
