...
def getJobLogStreamAddress(executor_api, uuid, source_zone):
    ...
    worker_zone = _getWorkerZoneByHostname(worker_info["hostname"])
...
def _getWorkerZoneByHostname(hostname):
    for executor in COMPONENT_REGISTRY.registry.all("executor"):
        if executor.hostname == hostname:
            return executor.zone
    return None
