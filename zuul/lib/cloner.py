...
class Cloner(object):
    ...
    def execute(self):
        ...
        for project, dest in dests.iteritems():
            ...
            # ref-updates are lacking ZUUL_BRANCH which we need for branch
            # fallbacking.
            if self.zuul_branch is None and project == self.zuul_project:
                self.zuul_branch = repo.git.branch('--contains', self.zuul_ref, '-r')
                self.log.info("Ref % contained by %s branch of %s" %(
                              self.zuul_ref, self.zuul_branch, project))

        for project, dest in dests.iteritems():
            self.applyZuulState(project, dest)
    ...
    def applyZuulState(self, project, dest):
