    ...
    def prepare_repo(self, project, dest):
        ...
        if self.args.branch:
            override_zuul_ref = re.sub(self.args.zuul_branch, self.args.branch,
                                       self.args.zuul_ref)
            try:
                branch_commit = repo.getBranchHead(self.args.branch)
                self.log.debug("repo has branch %s (%s)",
                               self.args.branch, branch_commit)
                fallback_branch = self.args.branch
                fallback_zuul_ref = self.args.zuul_ref
            except IndexError:
                self.log.exception("repo is missing branch %s",
                                   self.args.branch)
        ...
        if (self.fetch_from_zuul(repo, project, override_zuul_ref)
            ...
            try:
                repo.getBranchHead(fallback_branch)
                repo.checkout(fallback_branch)
            except (ValueError, GitCommandError):
                self.log.debug("Fallback branch not found: %s",
                               fallback_branch)
