...
class Tracing:
    ...
    def __init__(self, config):
        ...
        if protocol == self.PROTOCOL_GRPC:
            ...
            raise Exception(f"Unknown tracing endpoint {protocol}")
    ...
    def startSpan(self, name, parent=None, start_time=None):
        """Start a span
        ...
        if parent:
            with self.useSpan(parent):
                return self.tracer.start_span(name, start_time=start_time)
        else:
            return self.tracer.start_span(name, start_time=start_time)
