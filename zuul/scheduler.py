...
class Scheduler(threading.Thread):
    ...
    def __init__(self, config):
        ...
        # These may be very similar to sources
        self.sources = dict()
    ...
    def _parseLayout(self, config_path, connections):
        ...
        for config_project in data.get('projects', []):
            ...
            #XXXlayout.projects[config_project['name']] = project
            mode = config_project.get('merge-mode', 'merge-resolve')
            for pipeline in layout.pipelines.values():
                if pipeline.name in config_project:
                    project = pipeline.source.getProject(config_project['name'])
    ...
    def process_event_queue(self):
        ...
        try:
            for tenant in self.abide.tenants.values():
                #XXXproject = tenant.layout.projects.get(event.project_name)
...
class DependentPipelineManager(BasePipelineManager):
    ...
    def getChangeQueue(self, change, existing=None):
        self.log.debug("xxx get change queue %s %s" % (change, existing))
        if existing:
