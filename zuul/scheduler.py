...
class Scheduler(threading.Thread):
    ...
    def _testReconfigure(self, tenant, event, change, project):
        ...
        if event.branch_created or event.branch_updated:
            reconfigure_tenant |= self._testReconfigureAdd(change, tenant,
                                                           project, event)
        if event.branch_deleted or event.branch_updated:
    ...
    def _testReconfigureAdd(self, change, tenant, project, event):
        reconfigure_tenant = False
        if event.branch:
    ...
    def _testReconfigureRemove(self, change, tenant, project, event):
        ...
        return self.abide.hasUsefulUnparsedBranchCache(project.canonical_name,
                                                       event.branch,
                                                       tenant)
