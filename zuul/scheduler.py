...
class Scheduler(threading.Thread):
    ...
    def _runStats(self):
        ...
        # TODO (felix): Remove the list cast here, inState() already returns a
        # list.
        execute_queue = len(self.zk_builds.inState(BuildState.REQUESTED))
        execute_running = len(self.zk_builds.inState(
    ...
    def _getBuildFromEvent(self, event):
        ...
        # TODO (felix): Get the build item directly from ZK, not from the cache
        # Implement a simple zk_builds.get(build_item_path), which then could
        # also update the cache.
        build_item = self.zk_builds.get(build_item_path)
    ...
    def _doBuildCompletedEvent(self, event):
        # TODO (felix): Special noop build handling
        build, build_item = self._getBuildFromEvent(event)
        if not build:
            return
        ...
        if build.zookeeper_node:
            ...
            try:
                # TODO (felix): Releasing the lock on scheduler side shouldn't
                # work if it was acquired on the executor server.
                self.zk_builds.remove(build.zookeeper_node)
                # TODO (felix): Use finally part instead
