...
class Scheduler(threading.Thread):
    ...
    def _doTenantReconfigureEvent(self, event):
        ...
        with self.layout_lock:
            ...
            # FIXME: check if it is necessary to reload the TPCs here as it
            # looks like the TPCs can only change in case the unparsed abide
            # changed. This is not the case for a tenant reconfiguration event.
            loader.loadTPCs(self.abide, self.unparsed_abide,
                            [event.tenant_name])
