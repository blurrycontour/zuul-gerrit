...
for change %s:" % (job, change))
    ...
    def onBuildCompleted(self, build):
        ...
        if change.areAllJobsComplete():
            ...
            self.log.debug("Cancelling builds behind change: %s due to" 
                        " failure." % change)
            while change_behind:
                for job in change_behind.findJobsToRun():
                    fakebuild = Build(job, None)
                    fakebuild.result = 'SKIPPED'
                    change_behind.addBuild(fakebuild)
                change_behind = change_behind.change_behind
        else:
