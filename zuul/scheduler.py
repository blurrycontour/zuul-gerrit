            ...
            for name, new_pipeline in layout.pipelines.items():
                ...
                if not old_pipeline:
                    if self.layout.pipelines:
                        ...
                        self.log.warning("No old pipeline matching %s found when "
                                         "reconfiguring." % name)
                    continue
                ...
                for shared_queue in old_pipeline.queues:
                    for item in shared_queue.queue + shared_queue.severed_heads:
                        item.item_ahead = None
                        item.item_behind = None
                        item.pipeline = None
                        project = layout.projects.get(item.change.project.name)
                        if not project:
                            self.log.warning("Unable to find project for change "
