...
from zuul.zk.builds import BuildState, ZooKeeperBuilds
...
class Scheduler(threading.Thread):
    ...
    def _runStats(self):
        ...
        execute_queue = len(list(self.zk_builds.inState(BuildState.REQUESTED)))
    ...
    def _getBuildFromEvent(self, event):
        build_item_path = event.build_item_path

        # Look up the build data from zookeper and the local executor client
        build_item = self.zk_builds.getCached(build_item_path)
        build_uuid = build_item.content["uuid"]
        build = self.executor.builds.get(build_uuid)
        if not build:
            self.log.error("Unable to find build %s", build_uuid)
            return None, None

        return build, build_item

    def _isCurrentBuild(self, build, cancel=False):
        log = get_annotated_logger(
    ...
    def _doBuildStatusEvent(self, event):
        build, build_item = self._getBuildFromEvent(event)
        if not build:
            return

        if not self._isCurrentBuild(build):
            return
    ...
    def _doBuildPausedEvent(self, event):
        ...
        if not self._isCurrentBuild(build, cancel=True):
            return

        # TODO (felix): Check if the build_item.data["paused"] is True here
        build.paused = True
        # TODO (felix): Previously this was something like the following, but
        # as we are in a BuildPausedEvent we simply could set this to True.
        # build.paused = build_item.data.get("paused", build.paused)
        # TODO (felix): Why do we have to look up the "paused" result data from
        # build_item.data while the "complete" result data is stored in
        # build_item.result?
        build.result_data = build_item.data.get("data", {})
    ...
    def _reportBuildStats(self, build, result):
    ...
    def _doNodesProvisionedEvent(self, event):
        # TODO (felix): Get the request + its buildset from zookeeper. Store
        # only the request_uid and request_id on the event and compare
        # request.id with request_id after retrieving it from zookeeper.
        ...
        if not build_set:
            log.warning("Build set %s is not current "
                        "for node request %s", build_set, request_id)
        ...
        # NOTE (felix): As a temporary solution, we look up the request from
        ...
        if not request:
            # TODO (felix): If request not found (is None) -> not current
            # anymore or resubmitted?
            return
