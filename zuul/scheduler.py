...
from zuul.model import Build, Tenant
...
class Scheduler(threading.Thread):
    ...
    def reconfigure(self, config, smart=False, check_config=False):
        ...
        if smart:
            ...
            event = ReconfigureEvent(config, check_config=check_config)
        self.management_event_queue.put(event)
        self.wake_event.set()
    ...
    def _doSmartReconfigureEvent(self, event):
        ...
        with self.layout_lock:
            ...
            tenant_names.update(
                {t['name'] for t in self.unparsed_abide.tenants})
            for tenant_name in tenant_names:
                ...
                if tenant is not None:
                    self._reconfigureTenant(tenant)
                self.abide = abide
        self.log.info("Smart reconfiguration complete")
