...
class SemaphoreHandler(object):
    ...
    def acquire(self, item, job):
        ...
        semaphore_key = self._semaphore_key(item, job.semaphore)
    ...
    def _max_count(self, item, semaphore_name):
    ...
    def _semaphore_key(self, item, semaphore_name):
        # search for first valid layout in buildsets
        buildset = item.current_build_set

        while not buildset.layout:
            if buildset.previous_build_set:
                buildset = buildset.previous_build_set
            else:
                # This should not occur as the layout of the item must have had
                # some valid layout at some point in time.
                raise Exception("Item {} has no layout".format(item))

        return buildset.layout.tenant.name, semaphore_name
