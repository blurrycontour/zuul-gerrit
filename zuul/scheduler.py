                ...
                for shared_queue in old_pipeline.queues:
                    for item in shared_queue.queue:
                        ...
                        if not item.change.project:
                            old_p = self.layout.projects.get(project_name)
                            if old_p.foreign:
                                self.log.debug("Inherit foreign project %s" %
                                               old_p)
                                layout.projects[project_name] = old_p
                                item.change.project = old_p
                            else:
                                self.log.warning("Unable to find project for "
                                                 "%s while reenqueueing" %
                                                 item.change)
