...
class BasePipelineManager(object):
    ...
    def prepareRef(self, item, base_items):
        ...
        if hasattr(item.change, 'refspec') and not ref:
            ...
            ref = item.current_build_set.ref
            commit = self.sched.merger.mergeChanges([item] + base_items, ref)
    ...
    def _processOneItem(self, item, base_items):
        ...
        if item.current_build_set.base_items != base_items:
            # Either a change ahead has failed or merged
            if not (item.current_build_set.base_items and
                    item.current_build_set.base_items[-1].change.is_merged):
                # The change didn't merge so something must have failed
                changed = True
                self.cancelJobs(item)
                merge_conflict = self.prepareRef(item, base_items)
        merge_failed = False
        ...
        if not (merge_failed or merge_conflict or
                self.pipeline.didAnyJobFail(item)):
