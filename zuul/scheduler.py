...
class Scheduler(threading.Thread):
    ...
    def stop(self):
        _callDriverMethod(self.connection_pairs['trigger'], 'stop')
        self._stopped = True
        self.wake_event.set()
    ...
    def _preParseConfig(self, data):
        ...
        for conf_pipeline in data.get('pipelines', []):
            for pair_type in self.connection_pairs.keys():
                # Build list of driver+connection pairs because of special
                # case involving reporters that may be in the layout as
                # 'start', 'failure' and/or 'success'
                if pair_type == 'reporter':
                    for rtype in ['start', 'failure', 'success']:
                        for d_name, conf_driver in \
                            conf_pipeline.get(rtype, {}).items():
    ...
    def _parseConfig(self, config_path):
        ...
        for conf_pipeline in data.get('pipelines', []):
            ...
            # TODO(jhesketh): Probably remove the need for this..
            _callDriverMethod(self.connection_pairs['trigger'],
                              '_registerSource', pipeline.source)
