...
class DependentPipelineManager(BasePipelineManager):
    ...
    def _launchJobs(self, change, jobs):
        ...
        if hasattr(change, 'refspec') and not ref:
            ...
            # of merging. If there is no chance at merging remvoe the change
            ...
            if not commit:
                ...
                if not self._potentiallyMergable(conflictable_changes, change):
                    # Keep the severed head so that this change is reported.
                    self.removeChange(change, keep_severed_heads=True)
                else:
                    self.possiblyReportChange(change)
                return
            change.current_build_set.commit = commit
    ...
    def removeChange(self, change, keep_severed_heads=False):
        ...
        self.dequeueChange(change, keep_severed_heads=keep_severed_heads)
