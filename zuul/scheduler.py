                ...
                if conf_pipeline.get(action):
                    for reporter_name, params \
                        ...
                        if reporter_name in self.reporters.keys():
                            action_reporters[action].addReporter(
...
class BasePipelineManager(object):
    ...
    def getSubmitAllowNeeds(self):
        ...
        allow_needs = set()
        for reporter in self.pipeline.success_action.getReporters():
            allow_needs.update(reporter['reporter'].getSubmitAllowNeeds(
                reporter['params']))
        return allow_needs
    ...
    def reportStart(self, change):
        try:
            ...
            ret = self.sendReport(self.pipeline.start_action.getReporters(),
                                  change, msg)
            if ret:
    ...
    def sendReport(self, reporters, change, message):
        """Sends the built message off to configured reporters.

        Takes the reporters, change, message and extra options and sends them
        ...
        if len(reporters) > 0:
            ...
            for reporter in reporters:
                ret = reporter['reporter'].report(change, message,
                                                  reporter['params'])
                if ret:
    ...
    def _reportItem(self, item):
        ...
        try:
            ...
            ret = self.sendReport(action.getReporters(), item.change, report)
            if ret:
