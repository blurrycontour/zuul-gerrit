...
class Scheduler(threading.Thread):
    ...
    def _reconfigureTenant(self, context, tenant, old_tenant=None):
        ...
        if self.statsd:
            try:
                for pipeline in tenant.layout.pipelines.values():
                    # We don't need to refresh the pipeline here
                    # because this is called after loading a tenant,
                    # and that refreshes the pipeline state.
                    items = len([
                        x for x in pipeline.getAllItems(include_old=True)
                        if x.live
                    ])
