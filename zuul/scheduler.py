    ...
    def _doDequeueByBuildSetUUIDEvent(self, event):
        ...
        if event.tenant_name:
            ...
            if event.pipeline_name:
                pipelines.append(tenant.layout.pipelines[event.pipeline_name])
            else:
                pipelines = list(tenant.layout.pipelines.values())
        else:
            for tenant_name in self.abide.tenants:
                tenant = self.abide.tenants.get(tenant_name)
                for pl_name in tenant.layout.pipelines:
                    pipelines.append(tenant.layout.pipelines[pl_name])
