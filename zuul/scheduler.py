...
class DependentPipelineManager(BasePipelineManager):
    ...
    def enqueueChangesAhead(self, change, quiet, ignore_requirements,
        ...
        self.log.debug("  Changes %s must be merged ahead of %s" %
                       (ret, change))
        for needed_change in ret:
            # change_queue is ignored, so that addChange must look up
            # the queue for a project each time.  This will prevent
            # unmerged cross-project dependencies from being enqueued
            # in the wrong queue.
