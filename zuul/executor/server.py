...
class AnsibleJob(object):
    ...
    def __init__(self, executor_server, job):
        ...
        # Look up the NodeRequest with the provided ID from ZooKeeper. If no
        # ID is provided, this most probably means that the NodeRequest wasn't
        # submitted to ZooKeeper.
        if self.arguments.get("noderequest_id"):
...
class ExecutorServer(BaseMergeServer):
    ...
    def executeJob(self, job):
        ...
        # Add the worker before locking the node; if the lock fails, the lock
        # method will ensure that the worker is removed.
    ...
    def lockNodes(self, job, event=None):
        ...
        node_request = ansible_job.node_request
        if not node_request:
            log.debug("No nodes to lock")
            # If the AnsibleJob doesn't provide a NodeRequest this probably
            # means that the NodeRequest didn't contain any nodes and thus was
            # never submitted to ZooKeeper. In that case we don't have anything
            # to lock here.
            return True

        try:
            log.debug("Locking nodeset")
            self.nodepool.acceptNodes(node_request)
            self.nodepool.useNodeSet(
                node_request.nodeset, ansible_job, event=event
            )
            return True
        except Exception:
            log.exception("Error locking nodeset %s", node_request.nodeset)
            data = dict(
        ...
        if ansible_job.node_request:
            try:
                self.nodepool.returnNodeSet(
                    ansible_job.node_request.nodeset,
                    ansible_job,
                    zuul_event_id=zuul_event_id,
                )
            except Exception:
                self.log.exception(
                    "Unable to return nodeset %s",
                    ansible_job.node_request.nodeset,
                )
