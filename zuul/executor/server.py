...
class AnsibleJob(object):
    ...
    def preparePlaybook(self, jobdir_playbook, playbook, args, required):
        ...
        # We create the symlink purely for debugging purposes; the
        # executor doesn't actually use it.  We could, but it's nice
        # to see the full playbook path when we execute it.
        os.symlink(jobdir_playbook.path, jobdir_playbook.project_link)
        ...
        for role in playbook['roles']:
            prepareRole(jobdir_playbook, role)
    ...
    def prepareRole(self, jobdir_playbook, role):
        if role['type'] == 'zuul':
            root = self.jobdir_playbook.addRole()
            self.prepareZuulRole(jobdir_playbook, role, root)
    ...
    def prepareZuulRole(self, jobdir_playbook, role, root):
