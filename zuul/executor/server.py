...
class JobDir(object):
    def __init__(self, root, keep, build_uuid):
        ...
        date_time_facts['iso8601_micro'] = jobtime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ")
        date_time_facts['iso8601'] = jobtime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
        date_time_facts['iso8601_basic'] = jobtime.strftime("%Y%m%dT%H%M%S%f")
        date_time_facts['iso8601_basic_short'] = jobtime.strftime("%Y%m%dT%H%M%S")
        ...
        # can have ansible skip populating the cache with information by writing
        # a file with the minimum facts we want.
        with open(localhost_facts, 'w') as f:
            json.dump(executor_facts)
