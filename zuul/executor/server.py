...
class AnsibleJob(object):
    ...
    def writeAnsibleConfig(self, jobdir_playbook):
        ...
        with open(jobdir_playbook.ansible_config, 'w') as config:
            ...
            # This is a hack for the sake of testing
            if self.ara_callbacks:
                # TODO: Temporary default config for ara 1.x, figure out where to store the config
                # https://ara.readthedocs.io/en/latest/ansible-plugins-and-use-cases.html#ara-default-callback-recording-playbooks-from-ansible
                ...
                config.write('[ara]\n')
                config.write('api_client = %s\n' % self.ara_config['api_client'])
                config.write('api_server = %s\n' % self.ara_config['api_server'])
                config.write('api_username = %s\n' % self.ara_config['api_username'])
                config.write('api_password = %s\n' % self.ara_config['api_password'])
                config.write('api_insecure = %s\n' % self.ara_config['api_insecure'])
                config.write('api_timeout = %s\n' % self.ara_config['api_timeout'])
                config.write('ignored_facts = %s\n' % self.ara_config['ignored_facts'])
                config.write('ignored_arguments = %s\n' % self.ara_config['ignored_arguments'])
