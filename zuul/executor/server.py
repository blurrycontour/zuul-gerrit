...
class ExecutorServer(object):
    ...
    def register(self):
        ...
        self.executor_worker.registerFunction("executor:stop:%s" % self.hostname)
    ...
    def _innerUpdateLoop(self):
        ...
        if task is None:
            ...
            return
        self.log.debug("update acquire lock")
        with self.merger_lock:
            self.log.debug("update got lock")
            ...
            self.log.debug("Finished updating repo %s/%s" %
                           (task.connection_name, task.project_name))
        self.log.debug("update release lock")
        task.setComplete()
