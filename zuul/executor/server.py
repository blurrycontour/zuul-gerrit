...
class ExecutorServer(BaseMergeServer):
    ...
    def completeBuild(self, build_request, result):
        ...
        found = self._retry(log, update_build_request, log, build_request)
        self._retry(log, self.executor_api.unlock, build_request)
        ...
        self._retry(log, put_complete_event, log, build_request, event)

    def _retry(self, log, fn, *args, **kw):
        while True:
            try:
                return fn(*args, **kw)
            except Exception:
