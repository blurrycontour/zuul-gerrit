...
class AnsibleJob(object):
    ...
    def pause(self):
        ...
        self.executor_server.zookeeper.setBuildData(self.build_item.lock,
                                                    'data', data)
    ...
    def execute(self):
        try:
            ...
            self.executor_server.zookeeper.setBuildData(
                self.build_item.lock, 'data', self._base_job_data())
            ...
            self.executor_server.zookeeper.setBuildData(
                self.build_item.lock, 'result',
                dict(result='ERROR', error_detail=e.args[0]))
            self.executor_server.zookeeper.completeBuild(
                self.build_item.lock, success=True)
        except Exception:
            self.log.exception("Exception while executing job")
            # self.job.sendWorkException(traceback.format_exc())
            self.executor_server.zookeeper.setBuildData(
                self.build_item.lock, 'exception', traceback.format_exc())
            self.executor_server.zookeeper.completeBuild(
                self.build_item.lock, success=False)
        finally:
            self.running = False
            if self.jobdir:
                try:
    ...
    def _send_aborted(self):
        ...
        self.executor_server.zookeeper.setBuildData(
            self.build_item.lock, 'result', result)
        self.executor_server.zookeeper.completeBuild(
            self.build_item.lock, success=True)
    ...
    def _execute(self):
        ...
        # self.job.sendWorkComplete(result_data)
        self.executor_server.zookeeper.setBuildData(
            self.build_item.lock, 'result', result_value)
        self.executor_server.zookeeper.completeBuild(
            self.build_item.lock, success=True)
