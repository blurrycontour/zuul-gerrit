...
class AnsibleJob(object):
    ...
    def resume(self):
        ...
        self.paused = False
        # TODO (felix): self.build.state = BuildState.RUNNING + update()?
...
class ExecutorServer(BaseMergeServer):
    ...
    def _tree_cache_listener(
        ...
        if build_event == BuildEvent.CANCELED:
            # TODO (felix): Would it harm to simply keep the cancel node? If
            ...
            # TODO (felix): Would it harm to simply keep the resume node? If
            # not we could avoid this ZK update. The cancel request can anyway
            # only be fulfilled by the executor that executes the job. So, if
            # that executor died, no other can pick up the request.
        ...
        while self._running:
            ...
            if self.accepting_work:
                for build in self.build_queue.next():
                    self.log.debug("FE: next build %s", build.uuid)
                    # TODO (felix): contextmanager for locking
        ...
        if self._running:
            ...
            if self.zk_client.connected:
                for build in self.build_queue.lost_builds():
                    self.log.debug("FE: Canceling lost build %s", build.uuid)
            ...
            for sensor in self.sensors:
                ...
                if not ok:
                    ...
                    try:
                        ...
                        # TODO (felix): Can't we handle this exception directly
                        # in the ZK component class?
