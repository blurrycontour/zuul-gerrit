...
class AnsibleJob(object):
    ...
    def _append_playbook(self, playbook):
        if not os.path.exists(self.jobdir.job_output_json):
            # The first playbook failed with something like no hosts
            # matched, therefore not writing a job_output.json file in the
            # first place.
            with open(self.jobdir.job_output_json, 'w') as outfile:
                outfile.write('[\n\n]\n')
