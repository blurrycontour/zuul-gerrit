...
class ExecuteServer(object):
    log = logging.getLogger("zuul.ExecuteServer")
    ...
    def start(self):
        ...
        if self.config.has_option('gearman', 'port'):
            ...
            port = 4730
        self.worker = gear.Worker('Zuul Execute Server')
    ...
    def run(self):
        self.log.debug("Starting execute listener")
        while self._running:
            try:
...
class AnsibleJob(object):
    ...
    def _execute(self):
        ...
        # TODOv3:
        # 'name': self.name,
        # 'manager': self.execute_server.hostname,
        # 'worker_name': 'My Worker',
