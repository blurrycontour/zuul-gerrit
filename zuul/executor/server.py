...
class AnsibleJob(object):
    ...
    def __init__(self, executor_server, job):
        ...
        for section_name in self.executor_server.config.sections():
            ...
            self.ansible_callbacks[callback_name] = dict(config.items(
                                                            section_name))
        ...
        with open(jobdir_playbook.ansible_config, 'w') as config:
            ...
            if self.ansible_callbacks:
                ...
                for callback in self.ansible_callbacks.keys():
                    config.write('    %s' % callback.replace('%', '%%'))
            ...
            if self.ansible_callbacks:
                for callback_name, config in self.ansible_callbacks.keys():
                    callback_config_section = "callback_{callback_name}".format(callback_name)
                    config.write("[%s]" % callback_config_section)
                    {config.write("%s = %s" % (k, config[k]))
