...
class NodeLockingError(Exception):
    pass
...
class AnsibleJob(object):
    ...
    def __init__(self, executor_server, job):
        ...
        # TODO (felix): Do we have to differentiate between
        #   1. no noderequest_id is provided
        #   2. a noderequest_id is provided but the corresponding node request
        #      is not in ZooKeeper anymore?
    ...
    def _execute(self):
        ...
        # We set the nodes to "in use" as late as possible. So in case the
        # build failed during the checkout or preparation phase, the node is
        # still untouched and nodepool can re-allocate it to a different node
        # request / build.
        if self.node_request:
