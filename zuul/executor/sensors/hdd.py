...
class HDDSensor(SensorInterface):
    ...
    def __init__(self, statsd, base_key, config=None):
        ...
        self.min_avail_hdd_inodes = float(
            get_default(config, 'executor', 'min_avail_hdd_inodes', '5.0'))
        self.state_dir = get_default(
            config, 'executor', 'state_dir', '/var/lib/zuul', expand_user=True)

    def isOk(self):
        avail_hdd_pct, avail_inodes_pct = get_avail_hdd_inode_pct(self.state_dir)
        ...
        if self.statsd:
            ...
            self.statsd.gauge(self.base_key + '.pct_used_hdd_inodes',
        ...
        if avail_inodes_pct < self.min_avail_hdd_inodes:
            return False, "low disk inodes {:3.1f}% < {}".format(
                avail_inodes_pct, self.min_avail_hdd_inodes)
