...
class LocalRunnerContextManager(AnsibleJobContextManager):
    ...
    def decodeParameters(self, description):
        parameters = []
        if not description:
            return parameters
        parameter = None
    ...
    def jobParameters(self, job=None):
        parameters = []
        history = set()
        if job is None:
        ...
        while stack:
            ...
            try:
                job = self._grabJob(current_job)[0]
            except Exception:
            ...
            job_parameters = self.decodeParameters(job.get('description'))
