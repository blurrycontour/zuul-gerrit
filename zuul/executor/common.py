...
class AnsibleJobBase(object):
    ...
    The AnsibleJobBase is responsible for preparing the workspace and
    executing a single job. The constructor requires an executor engine,
    the job argument and the Job object.

    The executor engine provides:
    - a getMerger procedure to return a merger object that can be shared by
      multiple jobs.
    - a connections list.
    - an optional statsd object to emit statsd metrics.

    The public methods are:
    - prepareRepositories(), preparePlaybooks() and prepareAnsibleFiles
    - runPlaybooks()
    """
    RESULT_NORMAL = 1
    RESULT_TIMED_OUT = 2
