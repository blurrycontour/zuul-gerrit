...
class AnsibleJobBase(object):
    ...
    def __init__(self, job_runner, arguments):
        ...
        self.merge_root = job_runner.merger_root
    ...
    def runPlaybooks(self, args):
        ...
        if setup_status != self.job.RESULT_NORMAL or setup_code != 0:
            return result
        ...
        for index, playbook in enumerate(self.jobdir.pre_playbooks):
            pre_status, pre_code = self.runAnsiblePlaybook(
                playbook, 60, phase='pre', index=index)
            if pre_status != self.job.RESULT_NORMAL or pre_code != 0:
                pre_failed = True
                break
        ...
        self.log.debug(
            ...
            (self.job.cpu_times['user'], self.job.cpu_times['system'],
             self.job.cpu_times['children_user'],
             self.job.cpu_times['children_system']))

        if not pre_failed:
            job_status, job_code = self.job.runAnsiblePlaybook(
                self.jobdir.playbook, 60, phase='run')
            if job_status == self.job.RESULT_ABORTED:
                return 'ABORTED'
            elif job_status == self.job.RESULT_TIMED_OUT:
                pre_failed = True
                result = 'TIMED_OUT'
            elif job_status == self.job.RESULT_NORMAL:
                success = (job_code == 0)
                if success:
                    result = 'SUCCESS'
                else:
        ...
        for index, playbook in enumerate(self.jobdir.post_playbooks):
            post_status, post_code = self.job.runAnsiblePlaybook(
                playbook, 60, success, phase='post', index=index)
            if post_status == self.job.RESULT_ABORTED:
                return 'ABORTED'
            if post_status == self.job.RESULT_UNREACHABLE:
                unreachable = True
            if post_status != self.job.RESULT_NORMAL or post_code != 0:
                success = False
                if not pre_failed:
                    result = 'POST_FAILURE'
    ...
    def runAnsibleSetup(self, playbook):
        cmd = ['ansible', '*', '-v', '-m', 'setup',
               '-i', self.job.jobdir.setup_inventory,
               '-a', 'gather_subset=!all']
        result, code = self.job.runAnsible(
            cmd=cmd, timeout=60, playbook=playbook,
            wrapped=False)
        self.log.debug("Ansible complete, result %s code %s" % (
            self.job.RESULT_MAP[result], code))
        return result, code
