...
class ExecutorClient(object):
    ...
    def __init__(self, config, sched, zk: ZooKeeper):
        ...
        self.zk.builds.registerCacheListener(self._treeCacheListener)
    ...
    def execute(self, job: Job, item: QueueItem, pipeline: Pipeline,
        ...
        self.zk.builds.registerZone(executor_zone)
    ...
    def cancel(self, build: Build) -> bool:
        ...
        if build_zk_node:
            ...
            lock = self.zk.builds.getLock(build_zk_node)
            if lock and self.zk.client.acquireLock(lock, blocking=False):
                try:
                    ...
                    self.zk.client.releaseLock(lock)
            else:
                self.zk.builds.cancelRequest(build_zk_node)
    ...
    def resumeBuild(self, build: Build) -> bool:
        ...
        if build.zookeeper_node:
            self.zk.builds.resumeRequest(build.zookeeper_node)
            return True
        return False
