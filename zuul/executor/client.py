...
class ExecutorClient(object):
    ...
    def __init__(self, config, sched, zk: ZooKeeper):
        ...
        self.zk.registerBuildTreeCacheListener(self.__tree_cache_listener)
    ...
    def execute(self, job: Job, item: QueueItem, pipeline: Pipeline,
        ...
        self.zk.registerBuildZone(executor_zone)

        self.builds[uuid] = build

        build.zookeeper_node = self.zk.submitBuild(
    ...
    def onWorkStatus(self, build_item: ZooKeeperBuildItem):
        self.log.debug("Build %s update %s on %s" %
                       (build_item.content, build_item.data, self))
        build = self.builds.get(build_item.content['uuid'])
        if build:
            started = (build.url is not None)
            # Allow URL to be updated
            self.log.debug("Build %s url update: %s -> %s on %s" %
                           (build, build.url, build_item.data.get('url'),
                            self))
    ...
    def cancelJobInQueue(self, build: Build) -> bool:
        if build.zookeeper_node:
            self.zk.cancelBuildInQueue(build.zookeeper_node)
            if build.uuid:
                try:
    ...
    def cancelRunningBuild(self, build: Build) -> bool:
        ...
        if build.zookeeper_node:
            self.zk.cancelBuildRequest(build.zookeeper_node)
            return True
        return False
    ...
    def resumeBuild(self, build: Build) -> bool:
        ...
        if build.zookeeper_node:
            self.zk.resumeBuildRequest(build.zookeeper_node)
            return True
        return False
