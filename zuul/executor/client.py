...
class ExecutorClient(ZooKeeperTreeCacheClient):
    ...
    def _tree_cache_listener(self, segments: List[str], event: TreeEvent,
        ...
        if isinstance(new, ZooKeeperBuildItem)\
                and event.event_type in (TreeEvent.NODE_ADDED,
        ...
        # if old.content['status'] != new.content['status'] and\
        #         new.content['status'] == 'COMPLETED':
        #     self.onBuildCompleted(new.params, new.content['result'])
    ...
    def execute(self, job, item, pipeline, dependent_changes=[],
        ...
        try:
            ...
            #                        timeout=300)
            build.__zk_node = self._zk.submitJob(
    ...
    def cancelRunningBuild(self, build):
        ...
        #                        timeout=300)
        self._zk.cancelBuildRequest(build.__zk_node)
        return True
    ...
    def resumeBuild(self, build):
        ...
        self._zk.resumeBuildRequest(build.__zk_node)
