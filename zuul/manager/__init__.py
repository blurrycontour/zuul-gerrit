...
class PipelineManager(object):
    ...
    def getItemForChangeInQueue(self, change, change_queue):
        for item in change_queue.queue:
            if change.equals(item.change):
                return item
    ...
    def addChange(self, change, event, quiet=False, enqueue_time=None,
        ...
        with self.getChangeQueue(change, event, change_queue) as change_queue:
            ...
            if not cycle:
                # Only enqueue changes behind if this is not a cycle, as
                # cycle changes will otherwise be partially enqueued
