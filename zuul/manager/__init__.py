...
class PipelineManager(metaclass=ABCMeta):
    ...
    def updateBundle(self, item, change_queue, cycle):
        ...
        for needed_change in (c for c in cycle if c is not item.change):
            ...
            if bundle is None and needed_item.bundle is not None:
                bundle = needed_item.bundle

        if bundle is None:
            bundle = model.Bundle()

        log.info("Adding cycle item %s to bundle %s", item, bundle)
        bundle.add_item(item)

        for needed_item in needed_items:
            if needed_item not in bundle.items:
                log.info("Adding cycle item %s to bundle %s",
                         needed_item, bundle)
                bundle.add_item(needed_item)
        ...
        if not item.live and not dequeued:
            if item.items_behind:
                ...
                if not dependents_behind:
                    ...
                    old_items_behind = item.items_behind
                    self.dequeueItem(item)
                    changed = dequeued = True
                    for item_behind in old_items_behind:
                        log.info("Resetting builds for change %s because the "
