...
class PipelineManager(metaclass=ABCMeta):
    ...
    def onBuildCompleted(self, build):
        ...
        for item in build_in_items:
            ...
            if ((build_set.fail_fast or build_set.fail_fast_true) and
                build.failed and build.job.voting and not build.retry):
                if build_set.fail_fast_soft:
                    # If fail-fast-soft is set and the build is not successful
                    # cancel all remaining not-yet-started jobs.
                    log.debug("Build %s failed and fail-fast soft enabled, "
                              "canceling not-yet-started builds", build)
                    self._cancelNotStartedBuilds(build_set)
                else:
                    # If fail-fast is set and the build is not successful
                    # cancel all remaining jobs.
                    log.debug("Build %s failed and fail-fast enabled, "
                              "canceling running builds", build)
