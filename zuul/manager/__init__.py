...
class PipelineManager(metaclass=ABCMeta):
    ...
    def scheduleMerge(self, item, files=None, dirs=None):
        ...
        branches = self._branchesForRepoState(projects=projects, tenant=tenant, items=items)
    ...
    def scheduleGlobalRepoState(self, item: QueueItem) -> bool:
        ...
        for connection in repo_state.keys():
            ...
            for project in repo_state[connection].keys():
                ...
                for affected_project in projects:
                    if canonical_project_name == affected_project.canonical_name:
                        projects_to_remove.add(affected_project)
        projects -= projects_to_remove

        if not projects:
            item.current_build_set.repo_state_state = item.current_build_set.COMPLETE
            return True
        ...
        self.sched.merger.getRepoState(items=new_items, build_set=item.current_build_set, event=item.event, branches=branches)
