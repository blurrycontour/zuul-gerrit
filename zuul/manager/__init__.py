...
class PipelineManager(metaclass=ABCMeta):
    ...
    def canProcessCycle(self, project):
        ...
        project_meta = layout.getProjectMetadata(project.canonical_name)
        # Project is not part of this tenant; assume cycles are allowed to
        # allow processig in independent piplines.
        # The dependent pipeline manager will stop the cycle later in case
        # this change is not merged before other changes in the cycle.
        if project_meta is None:
            return True
        queue_config = layout.queues.get(project_meta.queue_name)
        return (
