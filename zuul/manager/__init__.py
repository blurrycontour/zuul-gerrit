...
class PipelineManager(object):
    ...
    def enqueueChangesAhead(self, change, quiet, ignore_requirements,
                            change_queue, history=None, event=None):
        return True

    def enqueueChangesBehind(self, change, quiet, ignore_requirements,
                             change_queue, event=None):
        return True

    def checkForChangesNeededBy(self, change, change_queue, event=None):
        return True
    ...
    def addChange(self, change, quiet=False, enqueue_time=None,
                  ...
                  change_queue=None, history=None, event=None):
        ...
        with self.getChangeQueue(
                change, change_queue, event=event) as change_queue:
            if not change_queue:
            ...
            item = change_queue.enqueueChange(change, event=event)
            if enqueue_time:
            ...
            self.enqueueChangesBehind(change, quiet, ignore_requirements,
                                      change_queue, event=event)
            zuul_driver = self.sched.connections.drivers['zuul']
            tenant = self.pipeline.tenant
            zuul_driver.onChangeEnqueued(
                tenant, item.change, self.pipeline, event=event)
            return True
    ...
    def updateCommitDependencies(self, change, change_queue, event=None):
        log = get_annotated_logger(self.log, event)
