...
class PipelineManager(object):
    ...
    def scheduleMerge(self, item, files=None):
        ...
        if not hasattr(item.change, 'branch'):
            self.log.info("Change %s does not have an associated branch, "
    ...
    def _processOneItem(self, item, nnfi):
        ...
        if actionable and not item.active:
            # This change is entering the pipeline's window,
            # schedule a new merge because the previous merge
            # may not be up-to-date.
            ready = self.scheduleMerge(item, ['zuul.yaml', '.zuul.yaml'])
