...
class ZuulStreamer(object):
    ...
    def stop_on_signal(self, signal):
        self.event_loop.add_signal_handler(signal, self.stop, signal)

    def _terminate(self):
        self.term.set_result("DONE")

    def start(self):
        self.log.debug("ZuulStreamer starting")
        self.term = asyncio.Future()
        self.event_loop.run_until_complete(self.term)
        self.log.debug("OMG SUCCESS!!!")
    ...
    def stop(self, signal=None):
        msg = "ZuulStreamer stopping"
        if signal:
            msg = msg + " for signal %s" % signal
        self.log.debug(msg)
