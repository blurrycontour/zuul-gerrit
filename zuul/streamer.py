...
class ZuulStreamerProtocol(websocket.WebSocketServerProtocol):
    ...
    async def onMessage(self, payload, isBinary):
        ...
        # method 1: wait forever
        completed, pending = await asyncio.wait([gear_task])
        port_location = completed[0].result()

        # method 2: timeout
        #try:
        #    port_location = await asyncio.wait_for(gear_task, 5)
        #except asyncio.TimeoutError:
        #    return self.sendClose(4000, "Gearman timeout")

        if not port_location:
            return self.sendClose(4000, "Error with Gearman")
        ...
        try:
            ...
            print("Exception: %s" % e)
            return self.sendClose(4000, "Console streaming error")
