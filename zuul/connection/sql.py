...
class SQLConnection(BaseConnection):
    ...
    def _setup_tables(self):
        ...
        self.zuul_build = sqlalchemy.Table(
            ...
            sqlalchemy.UniqueConstraint('uuid', 'job_name',
                                        name='zuul_build_uuid_job_name'),
        )

        self.zuul_build_metadata = sqlalchemy.Table(
            self._build_metadata_table, metadata,
            sqlalchemy.Column(
                'build_uuid', sqlalchemy.String(255),
                sqlalchemy.ForeignKey(self._build_table + '.uuid')),
            sqlalchemy.Column(
                'build_job_name', sqlalchemy.String(255),
                sqlalchemy.ForeignKey(self._build_table + '.job_name')),
