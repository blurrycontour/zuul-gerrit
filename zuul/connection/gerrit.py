...
class GerritEventConnector(threading.Thread):
    ...
    def _handleEvent(self):
        ...
        if (event.change_number and
            ...
            # multiple times.
            for source in self.connection._attached_to['source']:
                source._getChange(event.change_number,
                                  event.patch_number,
                                  refresh=True)
                # We only need to do this once since the connection maintains
                # the cache (which is shared between all the sources)
                # NOTE(jhesketh): We may couple sources and connections again at
                # which point this becomes more sensible.
                break
