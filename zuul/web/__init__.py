...
class ZuulWebAPI(object):
    ...
    @cherrypy.tools.save_params()
    @cherrypy.tools.json_out(content_type='application/json; charset=utf-8')
    def jobs(self, tenant, full=None):
        full = str(full).lower() == 'true'
        job = self.rpc.submitJob('zuul:job_list',
                                 {'tenant': tenant, 'full': full})
        ret = json.loads(job.data[0])
        if ret is None:
    ...
    @cherrypy.tools.save_params()
    @cherrypy.tools.json_out(content_type='application/json; charset=utf-8')
    def roles(self, tenant):
        job = self.rpc.submitJob('zuul:role_list', {'tenant': tenant})
        ret = json.loads(job.data[0])
        if ret is None:
            raise cherrypy.HTTPError(404, 'Tenant %s does not exist.' % tenant)
        resp = cherrypy.response
...
class ZuulWeb(object):
    ...
    def __init__(self, listen_address, listen_port,
        ...
        route_map.connect('api', '/api/tenant/{tenant}/roles',
                          controller=api, action='roles')
