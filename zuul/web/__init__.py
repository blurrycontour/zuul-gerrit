...
class ZuulWeb(object):
    ...
    def run(self, loop=None):
        ...
        '''
        routes = [
            ('GET', '/console-stream', self._handleWebsocket, 'stream'),
            ('GET', '/info', self._handleInfo, 'info'),
        ]
        ...
        for method, path, handler, name in routes:
            app.router.add_route(method, path, handler, name)
        if not self.websocket_url:
            self.websocket_url = app.router['stream'].url_for()
