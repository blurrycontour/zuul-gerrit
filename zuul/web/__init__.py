...
class GearmanHandler(object):
    ...
    async def enqueue_ref(self, request):
        tenant = request.match_info["tenant"]
        pipeline = request.match_info["pipeline"]
        project = request.match_info["project"]
        body = await request.json()
    ...
    async def autohold(self, request):
        tenant = request.match_info["tenant"]
        project = request.match_info["project"]
    ...
    async def autohold_list(self, request):
        data = {}
...
class ZuulWeb(BaseZuulWeb):
    ...
    def _routes(self):
        routes = [
            ...
            ('GET', '/autohold.json', self._handleAutoholdListRequest),
        ]
