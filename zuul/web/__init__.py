...
class ZuulWebAPI(object):
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out(content_type='application/json; charset=utf-8')
    def tenant_autohold_by_request_id(self, tenant, request_id):
        if cherrypy.request.method == 'GET':
    ...
    def index(self):
        return {
            ...
            'tenant_connections': '/api/tenant/{tenant}/connections',
            ...
            'tenant_autohold_by_request_id': ('/api/tenant/{tenant}'
                                              '/autohold/{request_id}'),
            'tenant_autohold_delete': ('/api/tenant/{tenant}'
                                       '/autohold/{request_id}'),
    ...
    @cherrypy.expose
    @cherrypy.tools.json_out(content_type='application/json; charset=utf-8')
    def tenant_connections(self, tenant):
        # Connections are not tenant-bound
        return self._connections()
...
class ZuulWeb(object):
    ...
    def __init__(self, listen_address, listen_port, gear_server,
        ...
        route_map.connect('api', '/api/tenant/{tenant}/connections',
                          controller=api, action='tenant_connections')
        ...
        if self.authenticators.authenticators:
            ...
            route_map.connect('api', '/api/authorizations',
        ...
        route_map.connect('api', '/api/tenant/{tenant}/autohold/{request_id}',
                          controller=api,
                          action='tenant_autohold_by_request_id')
