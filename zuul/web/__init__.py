...
class CORSTool(cherrypy.Tool):
    ...
    def handle_CORS(self, allowed_methods=None):
        ...
        # This is a simplified check. We may want to be stricter
        # if need be.
        is_simple_request = (
            # allowed methods
            cherrypy.request.method in ['GET', 'HEAD', 'POST'] and
            # allowed headers
            all(h.lower() in ['accept',
                              'accept-language',
                              'content-language',
                              'content-type']
                for h in cherrypy.request.headers)
        )
        if self.CORS_enabled and not is_simple_request:
            if '*' in self.allowed_origins:
                ...
                if origin:
                    ...
                    error_message = ('CORS is strictly enforced, '
...
class ZuulWebAPI(object):
    ...
    def _handleInfo(self, info):
        ret = {
            'info': info.toDict(),
            'CORS': {
                'enabled': self.zuulweb.CORS_config['CORS_enabled']
            }
        }
        resp = cherrypy.response
...
class ZuulWeb(object):
    ...
    def __init__(self,
        ...
        allowed_origins = get_default(
            self.config, 'web',
            'allowed_origins', "http://localhost").split(',')
        if enable_cors:
            # inject a dummy allowed origin that will be used by the CLI
            allowed_origins.append('ZUUL-CLIENT')
