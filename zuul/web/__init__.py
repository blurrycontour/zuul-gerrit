...
def auth_header():
    ...
    bearer token. Remove the token from params if it was passed this way.
    """
    ...
    if token:
        ...
        if 'jwt' in params:
            token = params['jwt']
            del params['jwt']
            cherrypy.request.headers['Authorization'] = 'Bearer %s' % token
        else:
            raise cherrypy.HTTPError(
                400,
                'Authorization with bearer token required')
...
class ZuulWebAPI(object):
    ...
    def is_authorized(self, claims, action, tenant, project):
        # First, check for zuul.actions override
        override = claims.get('zuul.actions', {})
        if action in override:
            if override[action] == '*':
                return True
            if tenant in override[action]:
                if override[action][tenant] == "*":
                    return True
                if project in override[action][tenant]:
                    return True
        # Next, get the rules for tenant/project
        data = {'tenant': tenant,
                'project': project}
        ...
        for rule in rules:
            if rule not in self.zuulweb.authorizations.ruleset:
                ...
                v = self.zuulweb.authorizations.ruleset[rule].actions[action]
                if v(claims):
    ...
    def user_actions(self):
        ...
        if 'zuul.actions' in claims:
            return {'zuul.actions': claims['zuul.actions']}
        zuul_actions = {'enqueue': {},
                        'dequeue': {},
                        'autohold': {}, }
        ...
        for t, t_rules in rules_tree.items():
            base_rules = t_rules['base_rules']
            for a in ['autohold', 'enqueue', 'dequeue']:
                if any(ruleset[r].actions[a](claims) for r in base_rules):
                    zuul_actions[a][t] = '*'
                else:
                    zuul_actions[a][t] = []
                    for p, p_rules in t_rules['project_rules'].items():
                        if any(ruleset[r].actions[a](claims) for r in p_rules):
                            zuul_actions[a][t].append(p)
        return {'zuul.actions': zuul_actions}
    ...
    def dequeue(self, tenant, project):
        ...
        # content
        if not self.is_authorized(claims, 'dequeue', tenant, project):
    ...
    def enqueue(self, tenant, project):
        ...
        try:
            ...
            cherrypy.response.status = e.HTTPError
            return '<h1>%s</h1>' % e.error_description
        if not self.is_authorized(claims, 'enqueue', tenant, project):
            raise cherrypy.HTTPError(403)
        msg = 'User "%s" requesting "%s" on %s/%s'
        self.log.info(
    ...
    def autohold(self, tenant, project=None):
        ...
        if cherrypy.request.method == 'GET':
            ...
            try:
                ...
                cherrypy.response.status = e.HTTPError
                return '<h1>%s</h1>' % e.error_description
            if not self.is_authorized(claims, 'autohold', tenant, project):
                raise cherrypy.HTTPError(403)
            msg = 'User "%s" requesting "%s" on %s/%s'
            self.log.info(
...
class ZuulWeb(object):
    ...
    def __init__(self, listen_address, listen_port,
        ...
        route_map.connect('api', '/api/user/actions',
                          controller=api, action='user_actions')
