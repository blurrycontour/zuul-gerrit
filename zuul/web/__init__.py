...
class GearmanHandler(object):
    ...
    async def tenant_list(self, request, filt=None):
        job = self.rpc.submitJob('zuul:tenant_list', {})
        return web.json_response(json.loads(job.data[0]))

    async def status_get(self, request, filt=None):
        ...
        if filt:
            payload = filt.filterPayload(payload)
    ...
    async def job_get(self, request, filt=None):
        job_args = {
            'tenant': request.match_info["tenant"],
    ...
    async def job_list(self, request, filt=None):
        tenant = request.match_info["tenant"]
    ...
    async def project_get(self, request, filt=None):
        job_args = {
            'tenant': request.match_info["tenant"],
    ...
    async def project_list(self, request, filt=None):
        tenant = request.match_info["tenant"]
    ...
    async def pipeline_list(self, request, filt=None):
        tenant = request.match_info["tenant"]
        job = self.rpc.submitJob('zuul:pipeline_list', {'tenant': tenant})
        resp = web.json_response(json.loads(job.data[0]))
        return resp

    async def key_get(self, request, filt=None):
        tenant = request.match_info["tenant"]
    ...
    async def processRequest(self, request, action, filt=None):
        resp = None
        try:
            resp = await self.controllers[action](request, filt)
        except asyncio.CancelledError:
...
class ZuulWeb(object):
    ...
    def run(self, loop=None):
        ...
        routes = [
            ...
            ('GET', '/{tenant}/pipelines', self._handlePipelinesRequest),
            ('GET', '/{tenant}/console-stream', self._handleWebsocket),
            ('GET', '/{tenant}/{project:.*}.pub', self._handleKeyRequest),
        ]

        static_routes = [
            StaticHandler(self, '/{tenant}/index.html', 'status.html'),
