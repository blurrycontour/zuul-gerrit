...
class GearmanHandler(object):
    ...
    async def job_get(self, request, result_filter=None):
        job_args = {
            'tenant': request.match_info["tenant"],
        ...
        if not zuul_jobs:
            raise web.HTTPNotFound(
                text="Job %s does not exist." % job_args["job_name"])
    ...
    async def pipeline_list(self, request, result_filter=None):
        tenant = request.match_info["tenant"]
...
class ZuulWeb(object):
    ...
    def run(self, loop=None):
        ...
        routes = [
            ...
            ('GET', '/api/tenant/{tenant}/jobs/{job_name}',
            ...
            ('GET', '/api/tenant/{tenant}/pipelines',
             self._handlePipelinesRequest),
