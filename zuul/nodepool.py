...
class Nodepool(object):
    ...
    def emitStats(self, request: NodeRequest) -> None:
        ...
        if request.canceled:
            state = 'canceled'
        elif request.state in (STATE_FULFILLED, STATE_FAILED):
    ...
    def emitStatsResourceCounters(
        self, tenant, project, resources, duration
    ) -> None:
        if not self.sched.statsd:
            return
        statsd = self.sched.statsd
    ...
    def reviseRequest(
        self, request: NodeRequest, relative_priority: int = None
    ) -> None:
    ...
    def holdNodeSet(
        self, nodeset: NodeSet, request: HoldRequest, build: Build
    ) -> None:
        '''
    ...
    def returnNodeSet(self, nodeset: NodeSet, build: Optional[Build] = None,
        ...
        project = None
        tenant = None
        # TODO (felix): Remove the build_set check when the build_set is
        # mandatory.
