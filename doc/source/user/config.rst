...
**run**
  ...
  here.  The name of a playbook without file extension to run as the
  main body of a job.  The full path to the playbook in the repo where
  the job is defined is expected.  When a child
  ...
  attribute, in which case that value is used).::
