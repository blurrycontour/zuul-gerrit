...
*config-project*, Zuul looks for a file named `zuul.yaml` or a directory
named `zuul.d`. In the case of an *untrusted-project*, Zuul looks first
for `zuul.yaml` or `zuul.d` and if they are not found, `.zuul.yaml` or
`.zuul.d` (with a leading dot).  In the case of an *untrusted-project*,
the configuration from every branch is included, however, in the case of a
*config-project*, only the `master` branch is examined.
