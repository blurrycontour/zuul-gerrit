...
Zuul uses standard Python ``re`` module and Google RE2 library (via
``fb-re2`` binding) for regular expressions parsing. Due to security
considerations RE2 supports `restricted syntax`__.
If configuration parameter documentation doesn't explicitly specify
then ``re`` module is used, which means full Python `regular expression
syntax`__  is supported.
