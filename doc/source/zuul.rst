...
**fetch_url**
  ...
  fetched if different than the Gerrit server itself. If specified the
  project names are appended to this url to form the actual fetch
  location. If omitted items will be fetched from Gerrit. Defaults to
  ``None`` and items will be fetched from Gerrit.  This is potentially
  useful when using Gerrit's replication feature to replicate refs to
  remote servers which zuul mergers may have access to instead of Gerrit
  itself.
  Example: ``fetch_url=https://gitmirror.example.com/``
