...
**location_override**
  Optional: Base url from which Zuul items should be fetched if
  different than the Gerrit server itself. Defaults to ``None``
  and items will be fetched from Gerrit. Example:
  ``location_override=https://my.git.replica.com/``
...
**poll**
  ...
  indicates to the merger process that refs should be polled for as
