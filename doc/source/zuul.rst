...
**manager**
  ...
  *DependentPipelineManager*
    ...
    related, and they share a single virtual pipeline of changes.  Thus,
    if there is a job that performs integration testing on two
    projects, those two projects will automatically share a virtual
    change pipeline.  If a third project does not invoke that job, it
    will be part of a separate virtual change pipeline, and changes to
...
creating a virtual pipeline of dependent changes and performing
