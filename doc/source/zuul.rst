...
config repositories.  Since a per patch dynamic max doesn't make much
sense this cannot be defined in project repositories.  Semaphores
which are listed in ``jobs`` but not configured with a max value default to 1::
