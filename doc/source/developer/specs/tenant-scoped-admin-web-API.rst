...
.. code-block:: yaml
  ...
  - rule:
      ...
      claim_types:
        resources_access.account.roles: list
  - rule:
...
* **name** is how the authorization rule will be refered as in Zuul's tenants
  configuration.
* **conditions** is the list of conditions that must be matched in order for
  the rule to apply. A condition is a dictionary where keys are claims. The
  associated values must match the claims in the user's Authentication Token.
* **claim_types** (optional) allows an operator to specify the expected type
  of the claim; by default all claims are expected to be strings. This will
  impact how the value lookup will be performed. Therefore prior knowledge of the
...
.. code-block:: bash
    ...
    $ zuul create-token --auth-config zuul-operator --user venkman --tenant tenantA --expires-in 1800
