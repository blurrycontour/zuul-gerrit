...
will not be in scope of this spec.
...
possible in zookeeper. Thus we only store the per project-branch parsed config
...
watching for changes in the parsed project-branch-config. The configs will be
stored in the path ``/zuul/tenant/<tenant>/config/<project>/<branch>`` as json
blobs per branch. If this data gets large we can consider compressing these
...
is fulfilled the pipeline processor creates an execution-request below which
will be locked by an executor before processing the job. The executor will
accept the referenced node request, lock the nodes and run the job. If the job
needs to be canceled the pipeline processor just pulls the execution-request.
The executor will notice this, abort the job and return the nodes.
...
in ``/zuul/tenant/<tenant>/times/<project>/<branch>/<job>``. There is also the
possibility to make the sql database mandatory and store the times database
there. But this can be done separately and is not within the scope of this spec.
If sql is mandatory during the implementation phase it can be used otherwise
the times database will be stored in zk just like it is now on disk of the
scheduler.
...
As discribed above the executor will look for execution-requests within the
pipeline state. It will lock this, accept the referenced node-request and begin
job execution. This way the scheduler doesn't need to lock the nodes so it can
be restarted without loosing the nodes of all jobs.

During execution it can directly update the state of the job in the pipeline.
...
outstanding events of that pipeline.
...
symmetric cipher with a shared key that is known to all zuul service but not
