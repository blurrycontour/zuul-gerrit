...
``/zuul/images/opendev.org%2fopendev%2fimages/``.  This avoids
...
Just as with images, provider configuration stanzas should appear in
every tenant where that provider is expected to serve that tenant.
