...
modifies the *job-1* being run in the :term:`check` pipeline (*job-1*,
...
is used immediately in the :term:`check` and :term:`gate` pipeline, **before**
being merged.
...
This time, the change to *job-1* is not applied in the :term:`check` and :term:`gate`
...
were run as soon as the :term:`check` pipeline, nothing would prevent a malicious user
