...
Your first gated patch with Zuul
--------------------------------

In this article, we will create a project and explain how to configure a basic
:ref:`CI workflow in order to gate <project_gating>` your first patch with Zuul.
...
Note that we are using the :var:`zuul.project.src_dir` variable to set the task's working
directory to the repository's root. Zuul defines a fair amount of variables that
can be used when writing jobs; the full list and descriptions are available in the
:ref:`Job variables <user_jobs_job_variables>` section of the documentation.
...
Note that this time, we don't directly push the change to the repository but we
...
- Run the **unit-tests** job in the :term:`check` pipeline, i.e., whenever a new
  patch or a change to an existing patch is submitted to Gerrit.
- Run the **unit-tests** job in the :term:`gate` pipeline, ie right after a patch has
  ...
  follow-up article.
- Call the Gerrit API to merge the patch if the job execution in the :term:`gate`
  pipeline succeeded.
...
base job which handles all of the tedious details like setting up the test environment
and exporting logs. By inheriting from the base job, the unit-tests job is encapsulated
within it, by running a **pre** playbook before unit-tests, and a **post** playbook
after **unit-tests**, regardless of whether the latter ended in success or failure.
...
Use the Gerrit web interface to approve the change by leaving a **+2** Code-Review
and **+1** Workflow approval and let Zuul run the :term:`gate` job and merge the change.
...
Zuul to :term:`gate` patches on **test1** !

.. image:: /images/gate-first-patch-merged.png
   :align: center

Now, any new patch submitted to the **test1** repository will automatically trigger 
this same CI workflow.

If it didn't work, we encourage you to ask for help via the
`zuul-discuss <http://lists.zuul-ci.org/cgi-bin/mailman/listinfo/zuul-discuss>`_
mailing list or `Matrix Chat <https://matrix.to/#/#zuul:opendev.org>`_
