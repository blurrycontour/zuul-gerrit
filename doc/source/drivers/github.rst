...
* If you need access to projects the app is not installed for (e.g. to
  ...
  you may create an api_token as well (see this `article
  <https://help.github.com/articles/creating-an-access-token-for-command-line-use/>`_).
  This api_token is not strictly required as Zuul will fallback to
  anonymous access; however, anonymous access is severely limited by
  API rate limits.
