...
Zuul uses a separate component called Nodepool to provide the
...
language needs to support orchestrating tasks on multiple nodes.  That
is decidedly in Ansible's wheelhouse, and Ansible is well-known and
easy to learn and use.  Some existing Ansible playbooks and roles may
be able to be used directly with Zuul (but some restrictions apply, so
not all will).
