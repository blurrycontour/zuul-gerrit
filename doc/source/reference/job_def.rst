...
.. attr:: job
   ...
   .. attr:: roles
      ...
      used.  If the role repo is the playbook repo, that branch is
      used; otherwise there is no single branch context available for
      selecting an appropriate branch of the role's repo to check out,
      so only the following are considered: the ref specified by
