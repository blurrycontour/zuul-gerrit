...
.. attr:: job
   ...
   .. attr:: intermediate
      ...
      not be inherited into a final job.  All ``intermediate`` jobs
      *must* also be ``abstract``; a configuration error will be
      raised if not.

      For example, you may define a base abstract job `foo` and create
      two abstract variants `foo-production` and `foo-development`.
      If it would be an error to accidentally inherit from the base
      job `foo` instead of choosing one of the two variants, `foo`
      could be marked as ``intermediate``.
