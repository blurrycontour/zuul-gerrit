...
.. var:: zuul
   ...
   .. var:: child_jobs
      ...
      A list of the first level depdendent jobs to be run after this job
...
use by depdendent jobs.  To return a value, use the ``zuul_return``
...
Will tell zuul to only run the depdendent_jobA and depdendent_jobC for pre-configured
depdendent jobs. If dependente_jobB was configured, it would be now marked as SKIPPED. If
zuul.child_jobs is empty, all jobs will be marked as SKIPPED. Invalid depdendent jobs
