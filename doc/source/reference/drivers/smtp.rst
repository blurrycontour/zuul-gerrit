...
.. attr:: <smtp connection>
   ...
   .. attr:: certfile
      ...
      certificate. Ignored when ``use_starttls`` is not enabled.

   .. attr:: keyfile

      Optional path to a single file containing the private key of the client
      certificate. If not set, the private key will be taken from ``certfile``
      as well. Ignored when ``use_starttls`` is not enabled.
