...
branch segmented in ``/zuul/config/<project>/<branch>/jobs/``,
``/zuul/config/<project>/<branch>/nodeset/``, etc. directories of json
blobs. One zobject per job, nodeset, etc or their range not to end up with too
many zobjects that way. E.g. ``/zuul/config/<project>/<branch>/jobs/1`` be the
first 100 jobs, and ``/zuul/config/<project>/<branch>/jobs/2`` be the next 100.
...
This section contains proposals for future work. They are a logical step forward
but not part of this spec and will be specified thoroughly in a separate
document.
