...
.. command-output:: yarnpkg --non-interactive --cwd=web run
   :cwd: ../../../..
...
so for example, *run the test suite in py3*::
...
For example, to *run a single Zuul test*::
   ...
   tox -e py3 -- tests.unit.test_scheduler.TestScheduler.test_jobs_executed

To *run one test in the foreground* (after previously having run tox
to set up the virtualenv)::

   .tox/py3/bin/stestr run tests.unit.test_scheduler.TestScheduler.test_jobs_executed
