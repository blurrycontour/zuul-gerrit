...
cases, the egular expression matching starts at the beginning of the
string, as if there were an implicit ``^`` at the beginning of the
regular expression. To match at an arbitrary position, prepend ``.*``
