...
.. attr:: pipeline.<reporter>.<github source>
   ...
   .. attr:: merge.method
      ...
      If :attr:`merge` is a dictionary, :attr:`merge.method` allows to control
      the merge method GitHub uses to bring the PR into the base branch.
      Currently, GitHub allows the following methods: ``merge`` (use a merge
      commit), ``rebase`` (rebase the commits on top of the base branch) and
