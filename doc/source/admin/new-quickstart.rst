...
browser and click `Become` then click `New Account`.
...
Zuul recognizes two types of projects: *Config Projects* and
*Untrusted Projects*.  An :term:`Untrusted Project` is a normal
...
Zuul will run.  A :term:`Config Project` is a special project that
contains the Zuul's configuration.  Because it has access to normally
...
.. include:: zuul-config/zuul.d/pipelines.yaml
...
.. include:: zuul-config/zuul.d/projects.yaml
