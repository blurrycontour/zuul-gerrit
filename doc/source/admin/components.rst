...
connect to the host running the scheduler on the Gearman port which is
...
data, or services may emit to a centralized statsd collector.
...
results.  It also includes a Gearman server which is used to
communicate with other components of Zuul.  It is possible to use an
external Gearman server, but the built-in server is well-tested and
...
The scheduler must also be able to connect to the ZooKeeper cluster
used by Nodepool in order to request nodes.  It does not need to
...
the scheduler host), and if the SQL reporter is used, they need to be
