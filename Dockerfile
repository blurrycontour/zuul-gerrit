...
# Run tests with 'docker run --rm -ti -v $PWD:/zuul zuuldev'
FROM ubuntu
RUN apt-get update
...
VOLUME /zuul
WORKDIR /zuul
ENV run_tox_user=toxrun
ENV tox_args=""
ENV ZUUL_TEST_ROOT=/tmp
CMD /tools/start-daemons.sh && /tools/make-volume-user.sh $run_tox_user && sleep 1 && sudo -u $run_tox_user tox $tox_args
